...
MFG_NVRAM_MUTIPLE_DIR=/etc/wlan/nvram
...
mfg_program_nvram()
{
	...
	if [ $ZYXEL_MFG -ne 1 ]; then
		...
		if [ ! -e $MFG_NVRAM_DIR/$MFG_NVRAM_FILE ] ; then
            ...
            image_default_nvram_file=`cat /proc/nvram/boardid`
            if [ -f $MFG_NVRAM_MUTIPLE_DIR/$image_default_nvram_file/$MFG_NVRAM_FILE ] ; then
                cp -a $MFG_NVRAM_MUTIPLE_DIR/$image_default_nvram_file/$MFG_NVRAM_FILE $MFG_NVRAM_DIR/$MFG_NVRAM_FILE
                echo "[$0]: get $MFG_NVRAM_MUTIPLE_DIR/$image_default_nvram_file/$MFG_NVRAM_FILE"
            else
                cp -a $MFG_NVRAM_WLAN_DIR/$MFG_NVRAM_FILE $MFG_NVRAM_DIR/$MFG_NVRAM_FILE
                echo "[$0]: get $MFG_NVRAM_WLAN_DIR/$MFG_NVRAM_FILE"
            fi
		fi
	fi
    fi
...
mfg_init_flash_type()
{
    if [ /dev/root -ef /dev/rootfs1 ] || [ /dev/root -ef /dev/rootfs2 ]; then
        FLASHTYPE="eMMC"
        FSTYPE=ext4
# ZYXEL, fix Calibration data issue on JFFS2 filesystem
...
case "$1" in
	...
	if  [ ! -s $KERNEL_NVRAM_FILE ]; then
		echo "[$0] *** KERNEL_NVRAM_FILE does not exist or the size is empty, re-create it. ***"
