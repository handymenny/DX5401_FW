...
typedef struct LinkMetricPerDev_s {
...
} __attribute__((__packed__)) LinkMetricPerDev_t;
...
typedef struct LinkMetricsResp_s {
  ...
  LinkMetricPerDev_t Neighbor_LinkInfo[MAX_DEVICE-1];
...
typedef struct AssocSTABuffer_s {
  unsigned char STA_MAC[MAC_ADDR_LEN];
  unsigned int BytesSent;
  unsigned int BytesReceived;
  unsigned int PacketsSent;
  ...
  int bss_idx;
  int sta_idx;
} __attribute__((__packed__)) AssocSTABuffer_t;

typedef struct AssociStaTrafficStatsAndAssociStaLinkMetricBuffer_s {
  AssocSTABuffer_t AssocSta[MAX_STA_PER_BSS];
} __attribute__((__packed__)) AssocStaTrafficStatsAndAssocStaLinkMetricBuffer_t;
...
int tlv_AssociStaLinkMetric_extract(unsigned char *pvalue, ApMetricsResp_t *pApMetricsResp, AssocStaTrafficStatsAndAssocStaLinkMetricBuffer_t *pAssocStaTrafficStatsAndAssocStaLinkMetricBuffer);
...
int tlv_AssociStaTrafficStats_extract(unsigned char *pvalue, ApMetricsResp_t *pApMetricsResp, int sta_cnts, AssocStaTrafficStatsAndAssocStaLinkMetricBuffer_t *pAssocStaTrafficStatsAndAssocStaLinkMetricBuffer);
#endif
