...
typedef struct rdm_NatPortMapping_s {   // RDM_OID_NAT_PORT_MAPPING 
	...
	char RemoteHost[46];
	char X_ZYXEL_RemoteHostEndRange[46];
...
typedef struct rdm_Dhcpv4Srv_s {   // RDM_OID_DHCPV4_SRV 
	...
	uint32_t X_ZYXEL_SubnetNumberOfEntries;
} rdm_Dhcpv4Srv_t;
...
typedef struct rdm_Dhcpv6Client_s {   // RDM_OID_DHCPV6_CLIENT 
	...
	char X_ZYXEL_DhcpcPidFilePath[33];
	bool SNTPEnable;
	uint32_t X_ZYXEL_MaxRetransTime;
} rdm_Dhcpv6Client_t;
...
typedef struct rdm_RebootSchedule_s {   // RDM_OID_REBOOT_SCHEDULE 
	bool Enable;
	uint32_t Mode;
	uint32_t Week;
	uint32_t Day;
