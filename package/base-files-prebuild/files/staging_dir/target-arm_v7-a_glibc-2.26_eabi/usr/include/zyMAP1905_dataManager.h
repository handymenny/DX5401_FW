...
#ifdef ABZQ_CUSTOMIZATION_DNS_RESPONSE
#define ABZQ_DNScfg "/tmp/.ABZQ_DNScfg"
#define ABZQ_DNS_KO "zy_nfhook_dns.ko"
#endif //ABZQ_CUSTOMIZATION_DNS_RESPONSE
...
#if defined(ZY1905_BROADCOM_WRAPPER) && defined(ZYXEL_OPAL_EXTENDER)
int zyUtilIGetBaseMACwithColon(char *);
#endif

#if defined(ZYXEL_ZY1905_VER_30_SUPPORT_FWUPGRADE)
RetCode getZyFWUpgradeStatus(int *st);
#endif

void zyMAP1905_MainLoop();

RetCode zyMAP1905_param_update();

void registerMAPNeighbor(UINT8 *alMac);

#ifdef ABZQ_CUSTOMIZATION_DNS_RESPONSE
void InsertDNSRedirectionModule(UINT8 *controllerIP);
#endif //ABZQ_CUSTOMIZATION_DNS_RESPONSE
