...
#define zyTlvBssConfigType			0xD0

#if defined(ZYXEL_ZY1905_VER_30_SUPPORT_FWUPGRADE)
#define FWCTRL_START_LIST		"/etc/zy1905/zy1905_fwctrl_start_list"
#define FWCTRLST_DL_DONE		"/etc/zy1905/zy1905_fwctrlst_dl_done"
#define FWCTRLST_DL_NOT_DONE	"/etc/zy1905/zy1905_fwctrlst_dl_not_done"
#endif
...
typedef struct zyBSS_Proprietary_s {
...
typedef struct zyBss_Config_Proprietary_s {
	unsigned int 			InterfaceNumberOfEntries;
	zyBSS_Proprietary_t 	BSS[IEEE1905_MAX_BSS_PER_DEV];
} zyBss_Config_Proprietary_t;
...
typedef enum {
	...
	VEND_MSG_TYPE_FW_CTRL_ST_REQ       = 0x0033,	/**< A message to control the FW control status of a specific neighbor interface */
	VEND_MSG_TYPE_FW_CTRL_ST_RESP      = 0x0034,	/**< A message to response to interface FW control status request */
...
#if defined(ZYXEL_ZY1905_VER_30_SUPPORT_FWUPGRADE)
	UINT16		USERcount;	/**< FW USER count*/
	UINT8		USER[64];	/**< FW URL*/
	UINT16		PASSWDcount;	/**< FW URL count*/
	UINT8		PASSWD[64];	/**< FW URL*/
#endif
...
#if defined(ZYXEL_ZY1905_VER_30_SUPPORT_FWUPGRADE)
//FW control status
/** A structure to represent the vendor specific message data of FW control status query message */
typedef struct {
	UINT8 		alMac[6];	/**< 1905.1 AL MAC address of the receiving device */
} FWCtrlStReqMsgData;

/** A structure to represent the vendor specific message data of FW control status response message */
typedef struct {
	RetCode		retCode;	/**< Result code for the FW control status request message */
} FWCtrlStRespMsgData;
#endif
...
int zyMAP1905_TlvBss_Config_insert(unsigned char *pbuf, unsigned int *tlv_data_len, zyBss_Config_Proprietary_t* pBssPropietary);
