...
#if 1
int faifa_get_hwaddr(faifa_t *faifa, unsigned char *hwaddr)
{
	int fd,i;
	struct ifreq ifr;
...
void faifa_set_src_addr(faifa_t *faifa, const u_int8_t *addr)
{
	memcpy(faifa->src_addr, addr, ETHER_ADDR_LEN);
}
