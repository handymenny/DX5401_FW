...
static int dump_result_to_ram(struct cm_net_info *net_info, char *ifname)
{
    FILE *fp;
	struct HPdump_t data;
	int macaddrlen = 0;
    u_int8_t *mac = net_info->macaddr;


    if((macaddrlen=strlen(net_info->macaddr))==0) {
        faifa_printf(out_stream, "Homeplug mac address not found\n");
        return -1;
    }
	memcpy(data.ifname, ifname, strlen(data.ifname));
    //fprintf(fp, "%s,", ifname);
    sprintf(data.hwaddr, "%02hX:%02hX:%02hX:%02hX:%02hX:%02hX",mac[0],mac[1],mac[2],mac[3],mac[4],mac[5]);

	pthread_mutex_lock(&filelck);
    fp = fopen(HPAV_RES_FILE, "a+");

    if(fp == NULL) {
        faifa_printf(out_stream, "open file %s error\n", HPAV_RES_FILE);
		pthread_mutex_unlock(&filelck);
        return -1;
    }

	fwrite(&data, sizeof(struct HPdump_t), 1, fp);
    //fprintf(fp,"\n");
    fclose(fp);
	pthread_mutex_unlock(&filelck);
    return 0;
...
static int hpav_dump_cm_get_network_infos_confirm(void *buf, int len, struct ether_header *UNUSED(hdr))
{
	...
	for (i = 0; i < mm->net.count; i++) {
		#ifdef ZYXEL_HOMEPLUG_DEVICE_DISCOVER 
		dump_result_to_ram(&(mm->net.infos[i]),ifname);
		#else
...
{
    //faifa_printf(out_stream,"\033[1;33m""%s,%d\n""\033[m",__FUNCTION__, __LINE__); //frank
 ...
 */
static int hpav_dump_frame(u_int8_t *frame_ptr, int frame_len, struct ether_header *hdr, u_int8_t *ifname)
{
	...
	if( (frame->header.mmtype & HPAV_MM_CATEGORY_MASK) == HPAV_MM_VENDOR_SPEC ) {
		...
		memcpy(frame_buf, frame->payload.vendor.data, frame_len);
	} else {
		frame_ptr = frame->payload.public.data;
		frame_len -= sizeof(frame->payload.public);
		memcpy(frame_buf, frame->payload.public.data, frame_len);
	}
	memcpy(&frame_buf[frame_len], ifname, 4);
	frame_len += 4;
...
void do_receive_frame(faifa_t *faifa, void *buf, int len, void *UNUSED(user))
{
	...
	if (*eth_type == ntohs(ETHERTYPE_HOMEPLUG))
		...
		hpav_dump_frame(payload_ptr, payload_len, eth_header,faifa->ifname); //add interface name to parameter
...
void *receive_loop(faifa_t *faifa)
{
	...
	if(res !=0) {
		faifa_printf(out_stream,"thread cancel state failed\n"); //frank
	}
	res = pthread_setcanceltype(PTHREAD_CANCEL_DEFERRED, NULL);
	if(res!=0) {
		faifa_printf(out_stream,"thread cancel fail\n"); //frank
	}		
...
void menu(faifa_t *faifa)
{
	...
	if (pthread_create(&(faifa->receive_thread), NULL, (void *)receive_loop, faifa)) {
	...
	while (ask_for_frame(&mmtype)) {
		do_frame(faifa, mmtype, faifa->dst_addr, faifa->src_addr, NULL);
		sleep(1);
	}
	#endif
...
int cancel_thread(faifa_t *faifa) {
	int ret =0 ;
	pthread_cancel(faifa->receive_thread);
	//faifa_printf(out_stream, "waiting for thread to finish\n");
	ret = pthread_join(faifa->receive_thread, NULL);
	return ret;
}