...
dal_param_t NETWORK_MAP_param[] = 
{
	...
	{"showPPPSettingCard",	    		dalType_boolean,	0,	0,	NULL},
	{NULL,		0,	0,	0,	NULL}
};
	...
	*Jarray = json_object_new_array();
	...
	for(i=0;i<len;i++){
       ...
       // wlan real status ; in case disable by wlan scheduler
...
#if defined(ZYXEL_RADIO_MAIN_SSID_INDEPENDENT) && !defined(ACEN_CUSTOMIZATION)
		...
		replaceParam(Jobj, "X_ZYXEL_GuestAPs", wifiSsidObj, "X_ZYXEL_GuestAPs");
...
zcfgRet_t zcfgFeDalCardPageStatusGet(struct json_object *Jobj, struct json_object *Jarray, char *replyMsg)
{
	...
	bool onessid = false;
#endif
	objIndex_t iid, pppoeInfoObjIid;
	int count = 0, currentBitRate = 0, isSFP =0;
	const char *lanport = NULL, *lanMacAddr = NULL, *duplexMode = NULL;
	char ethConnectionStatus[16] = {0};  
	char WanType[16] = {0};
	struct json_object *pppoeInfoObj = NULL, *ipInfoObj = NULL;
	const char *pppoeStatus = NULL, *svrName = NULL, *lowerLayers = NULL;
	...
	for (i = 0; i < len; i++) {
		...
		if (MainSSID) {
			...
			if(strcmp(json_object_get_string(json_object_object_get(MainSSIDobj, "X_ZYXEL_GuestAPs")),"")) {
				...
				GuestSSIDObj = json_object_array_get_idx(ssidJarray, i);
				json_object_array_add(newssidJarray, JSON_OBJ_COPY(GuestSSIDObj));
				guestOperatingFrequencyBand = json_object_get_string (json_object_object_get(GuestSSIDObj, "OperatingFrequencyBand"));
				if (!strcmp (guestOperatingFrequencyBand, "2.4GHz")) {
					support24G = true;
					Guest24GSSIDEnable = false;
				}
				else if (!strcmp (guestOperatingFrequencyBand, "5GHz")) {
					support5G = true;
					Guest5GSSIDEnable = false;
				}
			}
		}
	}
...
#endif
	IID_INIT(iid);
	IID_INIT(pppoeInfoObjIid);
	while(zcfgFeObjJsonGetNext(RDM_OID_IP_IFACE, &iid, &ipInfoObj) == ZCFG_SUCCESS){
		isDfGateway = json_object_get_boolean(json_object_object_get(ipInfoObj, "X_ZYXEL_DefaultGatewayIface"));
		svrName = json_object_get_string(json_object_object_get(ipInfoObj, "X_ZYXEL_SrvName"));
		if(isDfGateway && strcmp(svrName, "WWAN")){
			lowerLayers = json_object_get_string(json_object_object_get(ipInfoObj, "LowerLayers"));
			pppoeInfoObjIid.level = 1;
			sscanf(lowerLayers, "PPP.Interface.%hhu", &pppoeInfoObjIid.idx[0]);
			if((zcfgFeObjJsonGetWithoutUpdate(RDM_OID_PPP_IFACE, &pppoeInfoObjIid, &pppoeInfoObj)) == ZCFG_SUCCESS){
				json_object_object_add(statusJobj, "showPPPSettingCard", json_object_new_boolean(true));
				json_object_object_add(statusJobj, "pppUsername", JSON_OBJ_COPY(json_object_object_get(pppoeInfoObj, "Username")));
				json_object_object_add(statusJobj, "pppPassword", JSON_OBJ_COPY(json_object_object_get(pppoeInfoObj, "Password")));
				zcfgFeJsonObjFree(pppoeInfoObj);
				zcfgFeJsonObjFree(ipInfoObj);
				break;
			}
		}
		zcfgFeJsonObjFree(ipInfoObj);
