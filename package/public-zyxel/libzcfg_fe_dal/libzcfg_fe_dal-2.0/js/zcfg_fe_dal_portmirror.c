...
dal_param_t TT_PORT_MIRROR_param[]={		
	{"Index", 	 				dalType_int,	0,	0,	NULL},
...
zcfgRet_t zcfgFeDal_TTPortMirror_Edit(struct json_object *Jobj, char *replyMsg){
	...
	if(action == NULL)
		...
		//set TT mirror parameters
		IID_INIT(setttmirrorIid);
		if(zcfgFeObjJsonGet(RDM_OID_TT_PORT_MIRROR, &setttmirrorIid, &setttmirrorObj) == ZCFG_SUCCESS) {
			...
			if(!strcmp(action,"mirrorset"))
			   zcfgFeObjJsonSetWithoutApply(RDM_OID_TT_PORT_MIRROR, &setttmirrorIid, setttmirrorObj, NULL);
			else
			   zcfgFeObjJsonSet(RDM_OID_TT_PORT_MIRROR, &setttmirrorIid, setttmirrorObj, NULL);
...
zcfgRet_t zcfgFeDal_TTPortMirror_Get(struct json_object *Jobj, struct json_object *Jarray, char *replyMsg){
	...
	isethwan = guiisEthWan();
		...
		zcfgFeObjJsonGet(RDM_OID_TT_PORT_MIRROR, &ttmirrorIid, &ttmirrorObj);
...
zcfgRet_t zcfgFeDal_TTPortMirror_Edit(struct json_object *Jobj, char *replyMsg){
	zcfgRet_t ret = ZCFG_SUCCESS;
    objIndex_t setttmirrorIid = {0};
	struct json_object *setttmirrorObj = NULL, *ttmirrorObj = NULL;
	const char *action = NULL;
	const char *ttmonIface = NULL, *ttdirection = NULL, *ttkey = NULL;
	bool ttmirrorenable = false;
	int ttmirrorIface = 0, ttmirrortimeout = 0;
	
	action = Jgets(Jobj,"Action");

	if(action == NULL)
			return ZCFG_INVALID_PARAM_VALUE;
	
		ttmirrorenable = Jgetb(Jobj,"MirrorEnable");
		ttmonIface = Jgets(Jobj,"MonitorInterface");
		ttdirection = Jgets(Jobj,"Direction");
		ttmirrorIface = Jgeti(Jobj,"MirrorInterface");
		ttmirrortimeout = Jgeti(Jobj,"Timeout");	
		ttkey = Jgets(Jobj,"Key");
		...
		if(zcfgFeObjJsonGet(RDM_OID_TT_PORT_MIRROR, &setttmirrorIid, &setttmirrorObj) == ZCFG_SUCCESS) {
			...
			json_object_object_add(setttmirrorObj, "MirrorInterface", json_object_new_int(ttmirrorIface));
			json_object_object_add(setttmirrorObj, "MonitorInterface", json_object_new_string(ttmonIface));
			json_object_object_add(setttmirrorObj, "Key", json_object_new_string(ttkey));
			if(!strcmp(action,"mirrorset"))
			   zcfgFeObjJsonSetWithoutApply(RDM_OID_TT_PORT_MIRROR, &setttmirrorIid, setttmirrorObj, NULL);
			else
			   zcfgFeObjJsonSet(RDM_OID_TT_PORT_MIRROR, &setttmirrorIid, setttmirrorObj, NULL);
...
zcfgRet_t zcfgFeDal_TTPortMirror_Get(struct json_object *Jobj, struct json_object *Jarray, char *replyMsg){
	...
	char ipIfacePath[32] = {0};
	char loginName[32] = {0};
	struct json_object *accountobj = NULL, *ttaccountobj = NULL;
	objIndex_t accountiid = {0};
	objIndex_t ttaccountiid = {0};
	const char *curusername = NULL, *ttcurusername = NULL, *curpassword = NULL, *defpassword = NULL;
	const char *X_ZYXEL_SrvName = NULL, *X_ZYXEL_ConnectionType = NULL, *LowerLayers =NULL;
	int inifcount = 0;
	...
	tmp_array = json_object_new_object();
		...
		tmp_string = strrchr(X_ZYXEL_SrvName,'_');
		strncpy(newName, X_ZYXEL_SrvName, strlen(X_ZYXEL_SrvName)-strlen(tmp_string));	
		newName[strlen(X_ZYXEL_SrvName)-strlen(tmp_string)] = '\0';

		paramJobj = json_object_new_object();
		
		json_object_object_add(paramJobj, "Index", json_object_new_int(ipIfaceIid.idx[0]));

		json_object_object_add(paramJobj, "MonInterface", json_object_new_string(newName));

        snprintf(ipIfacePath, sizeof(ipIfacePath), "IP.Interface.%u", ipIfaceIid.idx[0]);
	...
	if(getLoginUserInfo(Jobj, loginName, NULL, NULL) != ZCFG_SUCCESS){
		if(replyMsg) strncpy(replyMsg, "Internal Error, can't get account information", sizeof(replyMsg));
		return ZCFG_INTERNAL_ERROR;
	}
	...
	if(!strcmp(loginName,"root"))
	{
		...
		while(zcfgFeObjJsonGetNextWithoutUpdate(RDM_OID_TT_USER, &ttaccountiid, &ttaccountobj) == ZCFG_SUCCESS) {
			ttcurusername = json_object_get_string(json_object_object_get(ttaccountobj, "Username"));
			if(!strcmp(ttcurusername,"root"))
			{
				strncpy(password, json_object_get_string(json_object_object_get(ttaccountobj, "Password")), sizeof(password));
				if(password != NULL)
				{
			...
			zcfgFeJsonObjFree(ttaccountobj);
		}	
			...
			while(zcfgFeObjJsonGetNextWithoutUpdate(RDM_OID_ZY_LOG_CFG_GP_ACCOUNT, &accountiid, &accountobj) == ZCFG_SUCCESS) {
				...
				if(!strcmp(curusername,"root")){
				   defpassword = json_object_get_string(json_object_object_get(accountobj, "DefaultPassword"));
				   if(!strcmp(decopassword,defpassword))
		           {
						   json_object_object_add(paramJobj, "UserPwdChanged", json_object_new_boolean(false));
		           }
		           else
...
zcfgRet_t zcfgFeDal_PacketCapture_Get(struct json_object *Jobj, struct json_object *Jarray, char *replyMsg){
	...
	if(zcfgFeObjJsonGet(RDM_OID_PACKET_CAPTURE, &PacketCaptureIid, &PacketCaptureJobj) == ZCFG_SUCCESS){
		...
		if(strstr(New_SrvName,"_ADSL")!= NULL){
			IID_INIT(ipIfaceIid);
			while(zcfgFeObjJsonGetNextWithoutUpdate(RDM_OID_IP_IFACE, &ipIfaceIid, &ipIfaceObj) == ZCFG_SUCCESS) {
				if(!strcmp(Jgets(ipIfaceObj, "X_TT_SrvName"),New_TT_SrvName) && strcmp(Jgets(ipIfaceObj, "X_ZYXEL_SrvName"),New_SrvName) && strstr(Jgets(ipIfaceObj, "X_ZYXEL_SrvName"),"_VDSL"))
				{
						sprintf(finalintf, "IP.Interface.%u", ipIfaceIid.idx[0]);
						zcfgFeJsonObjFree(ipIfaceObj);
						break;
	...
	if(!strcmp(loginName,"root"))
	{
		...
		while(zcfgFeObjJsonGetNextWithoutUpdate(RDM_OID_TT_USER, &ttaccountiid, &ttaccountobj) == ZCFG_SUCCESS) {
			...
			if(!strcmp(ttcurusername,"root"))
			{
				strcpy(password, json_object_get_string(json_object_object_get(ttaccountobj, "Password")));
				if(password != NULL)
				{
					int len = strlen(password);
					decopassword = base64_decode((unsigned char*)password, &len);
