...
zcfgRet_t zcfgFeDal8023ahCfg(struct json_object *Jobj)
{
	...
	struct json_object *ieee8023ahobj = NULL;
	objIndex_t ieee8023ahIid;
	bool Enabled = false;
	unsigned int OAMID = 0;
	char *Interface = NULL;
	bool VariableRetrievalEnabled = false, LinkEventsEnabled = false, RemoteLoopbackEnabled = false, ActiveModeEnabled = false, AutoEventEnabled = false;
...
zcfgRet_t zcfgFeDal8023ahGet(struct json_object *Jobj, struct json_object *Jarray, char *replyMsg, uint32_t flag1)
{
	zcfgRet_t ret = ZCFG_SUCCESS;
	struct json_object *obj = NULL, *interfaceList = NULL, *ieee8023ahobj = NULL;
	char pathString[30] = "";
	objIndex_t objIid;
	objIndex_t ieee8023ahIid;
...
zcfgRet_t zcfgFe8023ah(const char *method, struct json_object *Jobj, struct json_object *Jarray, char *replyMsg)
{
	...
	if(!strcmp(method, "GET")) {
		ret = zcfgFeDal8023ahGet(Jobj, Jarray, NULL, flag1);
	}
