...
unsigned int getPingTestType(struct json_object *Jobj)
{
	unsigned int type = 0;
...
zcfgRet_t zcfgFeDalIpDiagIPPingEdit(struct json_object *Jobj)
{
	...
	const char* ProtocolVersion = NULL;
...
exit:
	if(diagIPPingJobj)
		json_object_put(diagIPPingJobj);
...
zcfgRet_t zcfgFeDalIpDiagTraceRTEdit(struct json_object *Jobj)
{
	...
	struct json_object *diagTraceRTobj = NULL;
	const char* ProtocolVersion = NULL;
	const char* Host = NULL;
	const char* DiagnosticsState = NULL;
	unsigned int NumberOfRepetitions = 0;
...
exit:
	if(diagTraceRTobj)
		json_object_put(diagTraceRTobj);
...
zcfgRet_t zcfgFeDalDNSDiagNSLookUpEdit(struct json_object *Jobj)
{
	...
	struct json_object *dnsNSLookUpobj = NULL;
	const char* DiagnosticsState = NULL;
...
exit:
	if(dnsNSLookUpobj)
		json_object_put(dnsNSLookUpobj);
...
{
	zcfgRet_t ret = ZCFG_SUCCESS;
	objIndex_t IpDiagIPPingIid, IpDiagTraceRTIid, DNSDiagNSLookUpIid;
	struct json_object *diagIPPingJobj = NULL, *diagTraceRTobj = NULL, *dnsNSLookUpobj = NULL;
	unsigned int type = 0;
