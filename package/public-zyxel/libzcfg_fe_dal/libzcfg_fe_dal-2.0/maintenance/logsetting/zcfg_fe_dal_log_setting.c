...
void getMailEventBasicInfo(struct json_object *Jobj, logsetting_t *logsetting){
	if(NULL !=json_object_object_get(Jobj, "MailEnable")){
		logsetting->MailEnable = json_object_get_boolean(json_object_object_get(Jobj, "MailEnable"));
		logsetting->LogEmailSubject = json_object_get_string(json_object_object_get(Jobj, "MailSubject"));
		logsetting->AlarmEmailSubject = json_object_get_string(json_object_object_get(Jobj, "AlarmSubject"));
		logsetting->LogEmailTo = json_object_get_string(json_object_object_get(Jobj, "MailTo"));
		logsetting->AlarmEmailTo = json_object_get_string(json_object_object_get(Jobj, "AlarmTo"));
		logsetting->LogSericeRef = json_object_get_string(json_object_object_get(Jobj, "MailRef"));
	}else{
		logsetting->LogEnable = json_object_get_boolean(json_object_object_get(Jobj, "LogEnable"));
		logsetting->LogEmailSubject = json_object_get_string(json_object_object_get(Jobj, "LogEmailSubject"));
		logsetting->AlarmEmailSubject = json_object_get_string(json_object_object_get(Jobj, "AlarmEmailSubject"));
		logsetting->LogEmailTo = json_object_get_string(json_object_object_get(Jobj, "LogEmailTo"));
		logsetting->AlarmEmailTo = json_object_get_string(json_object_object_get(Jobj, "AlarmEmailTo"));
		logsetting->LogSericeRef = json_object_get_string(json_object_object_get(Jobj, "LogSericeRef"));
	}

#ifdef ZYXEL_NAT_LOG
	logsetting->NatEmailSubject = json_object_get_string(json_object_object_get(Jobj, "NatEmailSubject"));
	logsetting->NatEmailTo = json_object_get_string(json_object_object_get(Jobj, "NatEmailTo"));
#endif
	return;
}


zcfgRet_t zcfgFeDaleMailEventLogCfg(struct json_object *Jobj, logsetting_t *logsetting){
	...
	if(emailLog){
		if(zcfgFeObjJsonGet(RDM_OID_MAIL_EVENT_CFG, &Iid, &emailLogObj) == ZCFG_SUCCESS){
			...
			if(json_object_object_get(Jobj, "MailEnable")){ //come from webgui Vue
				if(logsetting->MailEnable){
					json_object_object_add(emailLogObj, "Enable", json_object_new_boolean(logsetting->MailEnable));
					if(json_object_object_get(Jobj, "MailTo"))
						json_object_object_add(emailLogObj, "EmailTo", json_object_new_string(logsetting->LogEmailTo));
					if(json_object_object_get(Jobj, "MailSubject"))
						json_object_object_add(emailLogObj, "EmailSubject", json_object_new_string(logsetting->LogEmailSubject));
					if(json_object_object_get(Jobj, "MailRef"))
						json_object_object_add(emailLogObj, "SericeRef", json_object_new_string(logsetting->LogSericeRef));
				}
				else{
					json_object_object_add(emailLogObj, "Enable", json_object_new_boolean(logsetting->MailEnable));
				...
				if(logsetting->LogEnable){
					json_object_object_add(emailLogObj, "Enable", json_object_new_boolean(logsetting->LogEnable));
...
zcfgRet_t zcfgFeDaleMailEventAlarmCfg(struct json_object *Jobj, logsetting_t *logsetting){
	zcfgRet_t ret = ZCFG_SUCCESS;
	...
	if(emailAlarm){
		if(zcfgFeObjJsonGet(RDM_OID_MAIL_EVENT_CFG, &Iid, &emailAlarmObj) == ZCFG_SUCCESS){
			...
			if(json_object_object_get(Jobj, "MailEnable")){ //come from webgui Vue
				if(logsetting->MailEnable){
					json_object_object_add(emailAlarmObj, "Enable", json_object_new_boolean(logsetting->MailEnable));
					if(json_object_object_get(Jobj, "AlarmTo"))
						json_object_object_add(emailAlarmObj, "EmailTo", json_object_new_string(logsetting->AlarmEmailTo));
					if(json_object_object_get(Jobj, "AlarmSubject"))
						json_object_object_add(emailAlarmObj, "EmailSubject", json_object_new_string(logsetting->AlarmEmailSubject));
					if(json_object_object_get(Jobj, "MailRef"))
						json_object_object_add(emailAlarmObj, "SericeRef", json_object_new_string(logsetting->LogSericeRef));
				}
				else{
					json_object_object_add(emailAlarmObj, "Enable", json_object_new_boolean(logsetting->MailEnable));
				...
				if(logsetting->LogEnable){
					json_object_object_add(emailAlarmObj, "Enable", json_object_new_boolean(logsetting->LogEnable));
...
zcfgRet_t zcfgFeDaleMailEventNotificationCfg(struct json_object *Jobj, logsetting_t *logsetting){
	zcfgRet_t ret = ZCFG_SUCCESS;
	...
	if(emailNotification){
		if(zcfgFeObjJsonGet(RDM_OID_MAIL_EVENT_CFG, &Iid, &emailNotificationObj) == ZCFG_SUCCESS){
			...
			if(json_object_object_get(Jobj, "MailEnable")){ //come from webgui Vue
				if(logsetting->MailEnable){
					json_object_object_add(emailNotificationObj, "Enable", json_object_new_boolean(logsetting->MailEnable));
					if(json_object_object_get(Jobj, "AlarmTo"))
						json_object_object_add(emailNotificationObj, "EmailTo", json_object_new_string(logsetting->AlarmEmailTo));
					if(json_object_object_get(Jobj, "MailRef"))
						json_object_object_add(emailNotificationObj, "SericeRef", json_object_new_string(logsetting->LogSericeRef));
				}
				else{
					json_object_object_add(emailNotificationObj, "Enable", json_object_new_boolean(logsetting->MailEnable));
				...
				if(logsetting->LogEnable){
					json_object_object_add(emailNotificationObj, "Enable", json_object_new_boolean(logsetting->LogEnable));
					if(json_object_object_get(Jobj, "AlarmTo"))
						json_object_object_add(emailNotificationObj, "EmailTo", json_object_new_string(logsetting->AlarmEmailTo));
					if(json_object_object_get(Jobj, "MailRef"))
						json_object_object_add(emailNotificationObj, "SericeRef", json_object_new_string(logsetting->LogSericeRef));
...
#ifdef ZYXEL_NAT_LOG
zcfgRet_t zcfgFeDaleMailEventNatLogCfg(struct json_object *Jobj, logsetting_t *logsetting){
	zcfgRet_t ret = ZCFG_SUCCESS;
	...
	if(emailNat){
		if(zcfgFeObjJsonGet(RDM_OID_MAIL_EVENT_CFG, &Iid, &emailNatObj) == ZCFG_SUCCESS){
			...
			if(json_object_object_get(Jobj, "LogEnable")){
				if(logsetting->LogEnable){
					json_object_object_add(emailNatObj, "Enable", json_object_new_boolean(logsetting->LogEnable));
					if(json_object_object_get(Jobj, "NatEmailTo"))
						json_object_object_add(emailNatObj, "EmailTo", json_object_new_string(logsetting->NatEmailTo));
					if(json_object_object_get(Jobj, "NatEmailSubject"))
						json_object_object_add(emailNatObj, "EmailSubject", json_object_new_string(logsetting->NatEmailSubject));
					if(json_object_object_get(Jobj, "LogSericeRef"))
						json_object_object_add(emailNatObj, "SericeRef", json_object_new_string(logsetting->LogSericeRef));
				}
				else{
					json_object_object_add(emailNatObj, "Enable", json_object_new_boolean(logsetting->LogEnable));
...
zcfgRet_t zcfgFeDalLogSettingcfg(struct json_object *Jobj)
{
	...
	bool Enable = false;
	const char* LogMode = NULL;
	const char* LogServer = NULL;
...
{
	zcfgRet_t ret = ZCFG_SUCCESS;
	char *logCat = NULL, *tmp_ptr = NULL, *category = NULL;
	int classIdx = 0;
	int categoryIdx = 0;
	bool enable = false;
...
{
	zcfgRet_t ret = ZCFG_SUCCESS;
	logsetting_t logsetting;

	memset(&logsetting, 0, sizeof(logsetting_t));

	getMailEventBasicInfo(Jobj, &logsetting);

	if((ret = zcfgFeDaleMailEventLogCfg(Jobj, &logsetting)) != ZCFG_SUCCESS) {
		printf("%s: Set log category fail\n", __FUNCTION__);
		return ZCFG_INTERNAL_ERROR;
	}
	if((ret = zcfgFeDaleMailEventAlarmCfg(Jobj, &logsetting)) != ZCFG_SUCCESS) {
		printf("%s: Set log category fail\n", __FUNCTION__);
		return ZCFG_INTERNAL_ERROR;
	}
#ifdef ZYXEL_SEND_NEW_CONNECTION_NOTIFICATION //send notification if the device never connected before
		if((ret = zcfgFeDaleMailEventNotificationCfg(Jobj, &logsetting)) != ZCFG_SUCCESS) {
			printf("%s: Set log category fail\n", __FUNCTION__);
			return ZCFG_INTERNAL_ERROR;
		}
#endif
#ifdef ZYXEL_NAT_LOG
	if((ret = zcfgFeDaleMailEventNatLogCfg(Jobj, &logsetting)) != ZCFG_SUCCESS) {
		printf("%s: Set log category fail\n", __FUNCTION__);
		return ZCFG_INTERNAL_ERROR;
	}
#endif
...
//MTKSOC Patches
	...
	IID_INIT(categoryIid);
	if(json_object_object_get(Jobj, "EasyMesh")){
		while(zcfgFeObjJsonGetNext(RDM_OID_LOG_CATEGORY, &categoryIid, &categoryObj) == ZCFG_SUCCESS){
			if(!strcmp(json_object_get_string(json_object_object_get(categoryObj, "Describe")),"EasyMesh")){
				json_object_object_add(categoryObj, "Enable", JSON_OBJ_COPY(json_object_object_get(Jobj, "EasyMesh")));
				zcfgFeObjJsonSet(RDM_OID_LOG_CATEGORY, &categoryIid, categoryObj, NULL);
				zcfgFeJsonObjFree(categoryObj);
			}
			zcfgFeJsonObjFree(categoryObj);
