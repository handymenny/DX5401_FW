...
{
	zcfgRet_t ret = ZCFG_SUCCESS;
	struct json_object *mailNotificationJobj = NULL;
	objIndex_t objIid;
	unsigned int SMTPServerPort = 0;
	const char* SMTPServerAddress = NULL;
	const char* AuthUser = NULL;
	const char* AuthPass = NULL;
	const char* EmailAddress = NULL;
	const char* SMTPSecurity = NULL;
	const char* DeleteMail = NULL;
...
exit:
	...
	if(mailNotificationJobj)
		json_object_put(mailNotificationJobj);
	return ret;
}
