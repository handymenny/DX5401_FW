...
static zcfgRet_t zcfgFeDalMgmtGet(struct json_object *Jobj, struct json_object *Jarray, char *replyMsg)
{
	...
	if(replyMsg && !strcmp(replyMsg, "resetdefault")){
		if((ret = zcfgFeObjJsonGetWithoutUpdate(RDM_OID_T_R69_DEAFAULT, &objIid, &mgmtJobj)) != ZCFG_SUCCESS) {
...
#ifdef ACBD_CUSTOMIZATION
zcfgRet_t zcfgFeDalMgmtDefault(const char *method, struct json_object *Jobj, struct json_object *Jarray, char *replyMsg)
{
	zcfgRet_t ret = ZCFG_SUCCESS;
	...
	if(!strcmp(method, "GET")) {
		ret = zcfgFeDalMgmtGet(Jobj, Jarray, "resetdefault");
	}
	return ret;
}
#endif
