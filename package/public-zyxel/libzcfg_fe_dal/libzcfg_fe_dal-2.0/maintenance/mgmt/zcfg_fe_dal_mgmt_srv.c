...
#ifdef MSTC_TTNET_MULTI_USER //__MSTC__, Bruce for item 169
...
#endif
#ifdef CONFIG_ZYXEL_TT_MGMTPORT_SEPARETE
    int SPport;
#endif
#ifdef MSTC_TTNET_MULTI_USER //__MSTC__, Bruce for item 169
...
#ifdef MSTC_TTNET_MULTI_USER //__NSTC__, Bruce for item 169
		while (!found && (zcfgFeObjJsonGetNext(RDM_OID_TT_USER, &UsersUser_iidStack, &UsersUserObj) == ZCFG_SUCCESS))	  
		{
...
zcfgRet_t dalSec_updateUsersUserObject(struct json_object *Jobj)
{
	...
	while (!found && zcfgFeObjJsonGetNext(RDM_OID_TT_USER, &UsersUser_iidStack, &UsersUserObj) == ZCFG_SUCCESS)    
	{
		...
		{	
			...
			if ((ret = zcfgFeObjJsonSet(RDM_OID_TT_USER, &UsersUser_iidStack, UsersUserObj, NULL)) != ZCFG_SUCCESS)
			{
				printf("failed to set <RDM_OID_TT_USER> object. ret:%d", ret);
			}
			found = true;
		}
		zcfgFeJsonObjFree(UsersUserObj);
...
zcfgRet_t zcfgFeDal_mgmtSrv_Edit(struct json_object *Jobj, char *replyMsg){
	...
	getSysLogInfo(Jobj, reqObject);
			...
			if((ret = dalSec_updateUsersUserObject(Jobj)) != ZCFG_SUCCESS){
				printf("failed to update <RDM_OID_TT_USER> object. ret=%d", ret);
			}
#endif
