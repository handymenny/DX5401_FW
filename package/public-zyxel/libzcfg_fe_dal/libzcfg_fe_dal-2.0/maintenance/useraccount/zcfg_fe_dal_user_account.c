...
#else
	{"AccountLockTime", 	dalType_int,	 0, 	5400,	NULL,	NULL,	0},
#endif
#ifdef ABUU_CUSTOMIZATION
...
#elif defined(MSTC_TTNET_MULTI_USER) /*__MSTC__, Linda, CPE User Definitions and Rights. */
...
#elif defined(ABZQ_CUSTOMIZATION_GUI_USER_ACCOUNT_SHOW_ALL)//#168877 Add group "Supervisor"
	{"group",				dalType_string,  0, 	0,		NULL,	"Supervisor|Administrator|User", dal_Add|dal_Edit_ignore}, //depend on login acocunt, see paramPrevilegeChange(), defatul is "User"
#else
...
zcfgRet_t zcfgFeDal_User_Account_Get(struct json_object *Jobj, struct json_object *Jarray, char *replyMsg){
...
#elif defined(ABZQ_CUSTOMIZATION_GUI_USER_ACCOUNT_SHOW_ALL)
			//#168877 do not ignore high and medium group
#else
			if(!strcmp(loginLevel, "high") || !strcmp(loginLevel, "medium")){
	...
	#ifdef ABZQ_CUSTOMIZATION_GUI_USER_ACCOUNT_SHOW_ALL //#168877 add group "Supervisor" in user account page
				json_object_object_add(pramJobj, "group",json_object_new_string("Supervisor"));
	#else
...
#ifndef MSTC_TTNET_MULTI_USER /*__MSTC__, Linda, CPE User Definitions and Rights. */					
...
#if defined(MSTC_TTNET_MULTI_USER) || defined(ABZQ_CUSTOMIZATION_GUI_USER_ACCOUNT_SHOW_ALL)
		//#168877 able to modify highGP account infomation
...
#ifdef ABZQ_CUSTOMIZATION_NOT_CHECK_MEDIUMGP_OLD_PASSWORD
//#168878
//Do not check old password if account is in medium group 
		if(ISSET("Password")){
			if(strcmp("", Jgets(Jobj ,"Password"))){//if psw is not empty
				if(ISSET("oldPw") || accountIid.idx[0] == mediumGpIdx){
					if(ISSET("verifyNewpassword")){//if verify psw is empty
						if(!strcmp("", Jgets(Jobj ,"verifyNewpassword"))){
							if(replyMsg) {
								ZOS_STRNCPY(replyMsg, "Need --verifyNewpassword to modify Password", sizeof(replyMsg));
							}
						...
						else if(strcmp(Jgets(Jobj, "Password"), Jgets(Jobj, "verifyNewpassword"))){
							if(replyMsg) 
								ZOS_STRNCPY(replyMsg, "--Password and --verifyNewpassword don't match", sizeof(replyMsg));
							json_object_object_add(Jobj, "__multi_lang_replyMsg", 
						...
						if(accountIid.idx[0] != mediumGpIdx){
							...
							if(!strcmp("", Jgets(Jobj ,"oldPw"))){//if old psw is empty
								...
								if(strcmp(curPassword, Jgets(Jobj ,"oldPw"))!= 0){
									if(replyMsg) 
										ZOS_STRNCPY(replyMsg, "Wrong --oldPw value", sizeof(replyMsg));
									json_object_object_add(Jobj, "__multi_lang_replyMsg", 
