...
dal_param_t WAN_param[]={
	...
	//ppp Iface
#if defined(ABUU_CUSTOMIZATION) || defined(ZYXEL_WEB_GUI_ALLOW_PPPUSERNAME_EMPTY)
	...
	if(CurrIpDnsStatic && ipDnsStatic){ //enable to enable
		...
		if(dnsClientSrv4Obj){
			json_object_object_add(dnsClientSrv4Obj, "Enable", json_object_new_boolean(true));
				...
				int upwantype = 0;
				const char *curTTSrvName;
				const char *tmpTTSrvName;
				char tempipIfacePath[32];
		
			upwantype = UpWanTypeGet();   
			curTTSrvName = json_object_get_string(json_object_object_get(ipIfaceObj, "X_TT_SrvName"));
			
			if(upwantype == 2 && strstr(curTTSrvName,"_DSL")!=NULL){
				
				IID_INIT(tmpiid);
				while(zcfgFeObjJsonGetNextWithoutUpdate(RDM_OID_IP_IFACE, &tmpiid, &tmpipIfObj) == ZCFG_SUCCESS){
					   tmpTTSrvName = json_object_get_string(json_object_object_get(tmpipIfObj, "X_TT_SrvName"));
					   if(!strcmp(curTTSrvName,tmpTTSrvName) && (tmpiid.idx[0] != ipIfaceIid.idx[0])){
...
zcfgRet_t addLayer3Objects(struct json_object *Jobj){
   ...
   char *tmp_string = NULL;
    ...
    if(TTSrvName){
       	if(!strcmp(Type, "ATM") || !strcmp(Type, "PTM") ){
			strncpy(copy_name, TTSrvName, sizeof(copy_name));
			strncat(copy_name,"_DSL", sizeof(copy_name));
		    json_object_object_add(ipIfaceObj, "X_TT_SrvName", json_object_new_string(copy_name));
	    }
	    else if(!strcmp(Type, "ETH")){
...
#ifdef TT_HGW
	replaceParam(ipIfaceObj, "X_TTNET_VLANID", Jobj, "VLANID");
	replaceParam(ipIfaceObj, "X_TTNET_802_1p", Jobj, "VLANPriority");
#endif
...
#ifdef ZYXEL_TTNET_FIREWALL_WANINTERFACE
...
#else
	TTSrvName = json_object_get_string(json_object_object_get(Jobj, "TTSrvName"));
#endif
#endif
	...
	while(count < 4){
		...
		if(!strncmp(HigherLayer, "IP.Interface.", 13)){
			...
			if(!strncmp("Ethernet.VLANTermination.", LowerLayer, 25)){
				...
				zcfgFeObjJsonGetWithoutUpdate(RDM_OID_ATM_LINK, &atmLinkIid, &atmLinkObj);
#ifdef ZYXEL_PPTPD_RELAY
				strcpy(CurrEncapsulation, json_object_get_string(json_object_object_get(atmLinkObj, "LinkType")));
#else
...
zcfgRet_t UnnecessarymodifyObj(struct json_object *Jobj){	
...
#ifdef TT_HGW
	struct json_object *InternetVLANTermobj = NULL;
	objIndex_t InternetVLANTermIid;
	struct json_object *ManagentVLANTermobj = NULL;
	objIndex_t ManagentVLANTermIid;
	struct json_object *IPTVVLANTermobj = NULL;
	objIndex_t IPTVVLANTermIid;
	struct json_object *VoIPVLANTermobj = NULL;
	objIndex_t VoIPVLANTermIid;
#else
...
#ifdef TT_HGW
	bool currInternetVlanEnable = false;
	bool currManagentVlanEnable = false;
	bool currVoIPVlanEnable = false;
	bool currIPTVVlanEnable = false;
#else
...
#ifdef TT_HGW
	int currInternetVlanID = 0;
	int currManagentVlanID = 0;
	int currIPTVVlanID = 0;
	int currVoIPVlanID = 0;
	int currInternetVlanPri = 0;
	int currManagentVlanPri = 0;
	int currIPTVVlanPri = 0;
	int currVoIPVlanPri = 0;
#else
...
#endif
#ifdef TT_HGW
	struct json_object *Internetintfobj = NULL;
	objIndex_t InternetintfIid;
	struct json_object *Managentintfobj = NULL;
	objIndex_t ManagentintfIid;
	struct json_object *VoIPintfobj = NULL;
	objIndex_t VoIPintfIid;
	struct json_object *IPTVintfobj = NULL;
	objIndex_t IPTVintfIid;
	uint32_t MaxMTUSize = json_object_get_int(json_object_object_get(Jobj, "MaxMTUSize"));
	uint32_t currInternetMaxMTUSize = 0;
	uint32_t currManagentMaxMTUSize = 0;
	uint32_t currVoIPMaxMTUSize = 0;
	uint32_t currIPTVMaxMTUSize = 0;
#else
	...
	bool InternetLimitation = true;
	bool managementLimitation = true;
	bool VoIPLimitation = true;
	bool IPTVLimitation = true;
	bool Enable = json_object_get_boolean(json_object_object_get(Jobj, "Enable"));
	struct json_object *ipifaceobj = NULL;
	objIndex_t ipifaceIid;
	int currInternetEnable = 0;
	int currmanagentEnable = 0;	
	int currVoIPEnable = 0;
	int currIPTVEnable = 0;

	strcpy(Name, json_object_get_string(json_object_object_get(Jobj, "Name")));
	...
	if ( zcfgFeObjJsonGet(RDM_OID_INTERNET, &InternetIid,&InternetJarray) == ZCFG_SUCCESS)
	{
		...
		zcfgFeJsonObjFree(InternetJarray);
	}
	IID_INIT(managementIid);
...
#else
		if(((index == 3 && InternetLimitation == true) || (index == 4 && managementLimitation == true) || (index == 5 && VoIPLimitation == true) || (index == 6 && IPTVLimitation == true)) && !isAdd){
			if(index == 3){
				IID_INIT(ipifaceIid);
				ipifaceIid.level = 1;
				ipifaceIid.idx[0] = 3;
				...
				currInternetEnable = json_object_get_boolean(json_object_object_get(ipifaceobj, "Enable"));
#ifndef TT_HGW
				if(Enable != currInternetEnable){
					Enable = currInternetEnable;
				...
				currmanagentEnable = json_object_get_boolean(json_object_object_get(ipifaceobj, "Enable"));

				if(Enable != currmanagentEnable){
					Enable = currmanagentEnable;
					json_object_object_add(Jobj, "Enable", json_object_new_boolean(Enable));
				}
				zcfgFeJsonObjFree(ipifaceobj);
			}
				...
				currVoIPEnable = json_object_get_boolean(json_object_object_get(ipifaceobj, "Enable"));

				if(Enable != currVoIPEnable){
					Enable = currVoIPEnable;
					json_object_object_add(Jobj, "Enable", json_object_new_boolean(Enable));
				}
				zcfgFeJsonObjFree(ipifaceobj);
			}
				...
				currIPTVEnable = json_object_get_boolean(json_object_object_get(ipifaceobj, "Enable"));

				if(Enable != currIPTVEnable){
					Enable = currIPTVEnable;
					json_object_object_add(Jobj, "Enable", json_object_new_boolean(Enable));
				}
				zcfgFeJsonObjFree(ipifaceobj);
			}
			...
			IID_INIT(InternetintfIid);
			InternetintfIid.level = 1;
			InternetintfIid.idx[0] = 3;
			if(zcfgFeObjJsonGet(RDM_OID_IP_IFACE, &InternetintfIid,&Internetintfobj) != ZCFG_SUCCESS){
				printf("cannot get RDM_OID_IP_IFACE obj");
			}

			IID_INIT(ManagentintfIid);
			ManagentintfIid.level = 1;
			ManagentintfIid.idx[0] = 4;
			if(zcfgFeObjJsonGet(RDM_OID_IP_IFACE, &ManagentintfIid,&Managentintfobj) != ZCFG_SUCCESS){
				printf("cannot get RDM_OID_IP_IFACE obj");
			}

			IID_INIT(VoIPintfIid);
			VoIPintfIid.level = 1;
			VoIPintfIid.idx[0] = 5;
			if(zcfgFeObjJsonGet(RDM_OID_IP_IFACE, &VoIPintfIid,&VoIPintfobj) != ZCFG_SUCCESS){
				printf("cannot get RDM_OID_IP_IFACE obj");
			}

			IID_INIT(IPTVintfIid);
			IPTVintfIid.level = 1;
			IPTVintfIid.idx[0] = 6;
			if(zcfgFeObjJsonGet(RDM_OID_IP_IFACE, &IPTVintfIid,&IPTVintfobj) != ZCFG_SUCCESS){
				printf("cannot get RDM_OID_IP_IFACE obj");
			}
			currInternetMaxMTUSize = json_object_get_int(json_object_object_get(Internetintfobj, "MaxMTUSize"));
			currManagentMaxMTUSize = json_object_get_int(json_object_object_get(Managentintfobj, "MaxMTUSize"));		
			currVoIPMaxMTUSize = json_object_get_int(json_object_object_get(VoIPintfobj, "MaxMTUSize"));
			currIPTVMaxMTUSize = json_object_get_int(json_object_object_get(IPTVintfobj, "MaxMTUSize"));
                        if(index == 4){
				if(MaxMTUSize != currManagentMaxMTUSize){
					MaxMTUSize = currManagentMaxMTUSize;
					json_object_object_add(Jobj, "MaxMTUSize", json_object_new_int(MaxMTUSize));
				}
			}
			else if(index == 5){
				if(MaxMTUSize != currVoIPMaxMTUSize){
					MaxMTUSize = currVoIPMaxMTUSize;
					json_object_object_add(Jobj, "MaxMTUSize", json_object_new_int(MaxMTUSize));
				}
			}
			else if(index == 6){
				if(MaxMTUSize != currIPTVMaxMTUSize){
					MaxMTUSize = currIPTVMaxMTUSize;
					json_object_object_add(Jobj, "MaxMTUSize", json_object_new_int(MaxMTUSize));
				}
			}
			zcfgFeJsonObjFree(Internetintfobj);
			zcfgFeJsonObjFree(Managentintfobj);
			zcfgFeJsonObjFree(VoIPintfobj);
			zcfgFeJsonObjFree(IPTVintfobj);
		}
...
#ifdef TT_HGW
			if(index == 3){
				IID_INIT(InternetVLANTermIid);
				InternetVLANTermIid.level = 1;
				InternetVLANTermIid.idx[0] = 1;
				if(zcfgFeObjJsonGet(RDM_OID_ETH_VLAN_TERM, &InternetVLANTermIid,&InternetVLANTermobj) != ZCFG_SUCCESS){
					printf("cannot get RDM_OID_ETH_VLAN_TERM obj");
				}
				currInternetVlanEnable = json_object_get_boolean(json_object_object_get(InternetVLANTermobj, "X_ZYXEL_VLANEnable"));
				currInternetVlanID = json_object_get_int(json_object_object_get(InternetVLANTermobj, "VLANID"));
				currInternetVlanPri =  json_object_get_int(json_object_object_get(InternetVLANTermobj, "X_ZYXEL_VLANPriority"));

#ifndef TT_HGW
				if(VlanEnable != currInternetVlanEnable){
					VlanEnable = currInternetVlanEnable;
					json_object_object_add(Jobj, "VLANEnable", json_object_new_boolean(VlanEnable));
				}	
				if(VlanID != -1){
					if(VlanID != currInternetVlanID || VlanPri != currInternetVlanPri){
						VlanID = currInternetVlanID;
						VlanPri = currInternetVlanPri;	
...
#endif				
				zcfgFeJsonObjFree(InternetVLANTermobj);
			}
			else if(index == 4){
				IID_INIT(ManagentVLANTermIid);
				ManagentVLANTermIid.level = 1;
				ManagentVLANTermIid.idx[0] = 2;
				if(zcfgFeObjJsonGet(RDM_OID_ETH_VLAN_TERM, &ManagentVLANTermIid,&ManagentVLANTermobj) != ZCFG_SUCCESS){
					printf("cannot get RDM_OID_ETH_VLAN_TERM obj");
				}
				currManagentVlanEnable = json_object_get_boolean(json_object_object_get(ManagentVLANTermobj, "X_ZYXEL_VLANEnable"));
				currManagentVlanID = json_object_get_int(json_object_object_get(ManagentVLANTermobj, "VLANID"));
				currManagentVlanPri =  json_object_get_int(json_object_object_get(ManagentVLANTermobj, "X_ZYXEL_VLANPriority"));

				if(VlanEnable != currManagentVlanEnable){
					VlanEnable = currManagentVlanEnable;
					json_object_object_add(Jobj, "VLANEnable", json_object_new_boolean(VlanEnable));
				}	
				if(VlanID != -1){
					if(VlanID != currManagentVlanID || VlanPri != currManagentVlanPri){
						VlanID = currManagentVlanID;
						VlanPri = currManagentVlanPri;	
						json_object_object_add(Jobj, "VLANID", json_object_new_int(VlanID));
						json_object_object_add(Jobj, "VLANPriority", json_object_new_int(VlanPri));
					}
				}
				zcfgFeJsonObjFree(ManagentVLANTermobj);
			}
			else if(index == 5){
				IID_INIT(VoIPVLANTermIid);
				VoIPVLANTermIid.level = 1;
				VoIPVLANTermIid.idx[0] = 3;
				if(zcfgFeObjJsonGet(RDM_OID_ETH_VLAN_TERM, &VoIPVLANTermIid,&VoIPVLANTermobj) != ZCFG_SUCCESS){
					printf("cannot get RDM_OID_ETH_VLAN_TERM obj");
				}
				currVoIPVlanEnable = json_object_get_boolean(json_object_object_get(VoIPVLANTermobj, "X_ZYXEL_VLANEnable"));
				currVoIPVlanID = json_object_get_int(json_object_object_get(VoIPVLANTermobj, "VLANID"));
				currVoIPVlanPri =  json_object_get_int(json_object_object_get(VoIPVLANTermobj, "X_ZYXEL_VLANPriority"));

				if(VlanEnable != currVoIPVlanEnable){
					VlanEnable = currVoIPVlanEnable;
					json_object_object_add(Jobj, "VLANEnable", json_object_new_boolean(VlanEnable));
				}	
				if(VlanID != -1){
					if(VlanID != currVoIPVlanID || VlanPri != currVoIPVlanPri){
						VlanID = currVoIPVlanID;
						VlanPri = currVoIPVlanPri;	
						json_object_object_add(Jobj, "VLANID", json_object_new_int(VlanID));
						json_object_object_add(Jobj, "VLANPriority", json_object_new_int(VlanPri));
					}
				}
				zcfgFeJsonObjFree(VoIPVLANTermobj);
			}
			else if(index == 6){
				IID_INIT(IPTVVLANTermIid);
				IPTVVLANTermIid.level = 1;
				IPTVVLANTermIid.idx[0] = 4;
				if(zcfgFeObjJsonGet(RDM_OID_ETH_VLAN_TERM, &IPTVVLANTermIid,&IPTVVLANTermobj) != ZCFG_SUCCESS){
					printf("cannot get RDM_OID_ETH_VLAN_TERM obj");
				}
				currIPTVVlanEnable = json_object_get_boolean(json_object_object_get(IPTVVLANTermobj, "X_ZYXEL_VLANEnable"));
				currIPTVVlanID = json_object_get_int(json_object_object_get(IPTVVLANTermobj, "VLANID"));
				currIPTVVlanPri =  json_object_get_int(json_object_object_get(IPTVVLANTermobj, "X_ZYXEL_VLANPriority"));

				if(VlanEnable != currIPTVVlanEnable){
					VlanEnable = currIPTVVlanEnable;
					json_object_object_add(Jobj, "VLANEnable", json_object_new_boolean(VlanEnable));
				}	
				if(VlanID != -1){
					if(VlanID != currIPTVVlanID || VlanPri != currIPTVVlanPri){
						VlanID = currIPTVVlanID;
						VlanPri = currIPTVVlanPri;						
						json_object_object_add(Jobj, "VLANID", json_object_new_int(VlanID));
						json_object_object_add(Jobj, "VLANPriority", json_object_new_int(VlanPri));
					}
				}
				zcfgFeJsonObjFree(IPTVVLANTermobj);
			}
#else
			if(index == 1){
                ...
                if(!strncmp(CurrMode, "IP_Routed", strlen("IP_Routed"))){
				if(VlanID != -1){
...
static zcfgRet_t getWanInfo(struct json_object *pramJobj,objIndex_t ipIfaceIid, struct json_object *ipIfaceObj){
	...
	if(!strcmp(CurrEncapsulation, "IPoE") || !strcmp(CurrEncapsulation, "IPoA")){
		v4enable = json_object_get_boolean(json_object_object_get(ipIfaceObj, "IPv4Enable"));
		v6enable = json_object_get_boolean(json_object_object_get(ipIfaceObj, "IPv6Enable"));
		replaceParam(pramJobj, "MaxMTUSize", ipIfaceObj, "MaxMTUSize");
	}
	else{ //PPPoE or PPPoA
		v4enable = json_object_get_boolean(json_object_object_get(pppIfaceObj, "IPCPEnable"));
		v6enable = json_object_get_boolean(json_object_object_get(pppIfaceObj, "IPv6CPEnable"));
		...
		if( ret == ZCFG_SUCCESS )
		{
			...
			if(json_object_object_get(Jobj, "Index")){
				...
				if(!strcmp(BindName,TT_SrvName)){
					tmp_string = strrchr(TT_SrvName,'_');
...
#ifdef TT_HGW
                    char newName[128] = {0};
					strncpy(newName, TT_SrvName, strlen(TT_SrvName)-strlen(tmp_string));	
                    newName[strlen(TT_SrvName)-strlen(tmp_string)] = '\0';
					json_object_object_add(pramJobj, "HGWStyleName", json_object_new_string(newName));	
#endif
					}
					json_object_object_add(pramJobj, "TTSrvName", json_object_new_string(TT_SrvName));	
					json_object_array_add(Jarray, JSON_OBJ_COPY(pramJobj));
				}
#if defined(MSTC_TTNET_MULTI_USER) || defined(MSTC_RETAIL_MULTI_USER)
