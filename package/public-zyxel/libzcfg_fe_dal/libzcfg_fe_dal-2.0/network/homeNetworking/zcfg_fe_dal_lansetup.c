...
zcfgRet_t zcfgFeDal_LanSetup_Edit(struct json_object *Jobj, char *replyMsg){
	...
	const char *IPv6_PrefixType = NULL, *dnsList = NULL, *dnsServer = NULL;
	struct json_object *raObj = NULL;
	objIndex_t raIid = {0};
#ifdef CONFIG_ABZQ_CUSTOMIZATION
	struct json_object *radvdObj = NULL;
	objIndex_t radvdIid = {0};
...
*/
		...
		if(!ip6PreTypeStatic){
			...
			dnsServer = json_object_get_string(json_object_object_get(Jobj, "Ipv6_DNSServers"));
			IID_INIT(raIid);
			while(zcfgFeObjJsonGetNext(RDM_OID_RT_ADV_INTF_SET, &raIid, &raObj) == ZCFG_SUCCESS)
			{
				json_object_object_add(raObj, "X_ZYXEL_DNSList", json_object_new_string(dnsList));
				json_object_object_add(raObj, "X_ZYXEL_DNSServers", json_object_new_string(dnsServer));
	
				if(json_object_get_int(json_object_object_get(curIpIfaceObj, "X_ZYXEL_AdvManagedFlag")) == 1)
					json_object_object_add(raObj, "AdvManagedFlag", json_object_new_boolean(true));
				else
					json_object_object_add(raObj, "AdvManagedFlag", json_object_new_boolean(false));

				if(json_object_get_int(json_object_object_get(curIpIfaceObj, "X_ZYXEL_AdvOtherConfigFlag")) == 1)
					json_object_object_add(raObj, "AdvOtherConfigFlag", json_object_new_boolean(true));
				else
					json_object_object_add(raObj, "AdvOtherConfigFlag", json_object_new_boolean(false));
	
				if(json_object_get_boolean(json_object_object_get(curIpIfaceObj, "X_ZYXEL_RAandDHCP6S")))
					json_object_object_add(raObj, "X_ZYXEL_RAandDHCP6S", json_object_new_boolean(true));
				else
					json_object_object_add(raObj, "X_ZYXEL_RAandDHCP6S", json_object_new_boolean(false));
	
			}
			zcfgFeObjJsonSet(RDM_OID_RT_ADV_INTF_SET, &raIid, raObj, NULL);
			zcfgFeJsonObjFree(raObj);
