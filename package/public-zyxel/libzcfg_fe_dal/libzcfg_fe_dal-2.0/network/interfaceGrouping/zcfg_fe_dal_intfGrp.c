...
#ifdef ZYXEL_AUTO_INTF_GRP_QOS_PRI_ADJUSTMENT
	{"autopriorityEnable",	dalType_boolean,0, 0, NULL,NULL,	0},
#endif
...
#ifdef ZYXEL_AUTO_INTF_GRP_QOS_PRI_ADJUSTMENT
	bool enable_autopriority = false;
#endif
	int len = 0, i, idx = 0;
#if defined(ZYXEL_EASYMESH_R2) && defined(BROADCOM_PLATFORM)
	bool TSenable = 0, Meshenable = 0;
	objIndex_t TSObjIid = {0};
	objIndex_t easyMeshIid = {0};
	rdm_EasyMeshTs_t *TSObj = NULL;
	rdm_EasyMesh_t *easymMeshObj = NULL;

	IID_INIT(TSObjIid);
	if(zcfgFeObjStructGetWithoutUpdate(RDM_OID_EASY_MESH_TS, &TSObjIid, (void **)&TSObj) == ZCFG_SUCCESS){
		TSenable = TSObj->Enable;
		free(TSObj);
	}
	IID_INIT(easyMeshIid);
	if(zcfgFeObjStructGetWithoutUpdate(RDM_OID_EASY_MESH, &easyMeshIid, (void **)&easymMeshObj) == ZCFG_SUCCESS) {
		Meshenable = easymMeshObj->Enable;
		free(easymMeshObj);
	}
#endif
...
#if defined(ZYXEL_EASYMESH_R2) && defined(BROADCOM_PLATFORM)
			if(TSenable && Meshenable){
				/* Follow EasyMesh DS, in Mesh R2, Wi-Fi interface grouping are controller by BCM Mesh daemon */
				if(!strncmp(json_object_get_string(json_object_object_get(bridgeportObj, "LowerLayers")), "WiFi", 4)){
					continue;
				}
			}
#endif
						...
						if(!strcmp(tmp, bridge)){
...
#ifdef ZYXEL_AUTO_INTF_GRP_QOS_PRI_ADJUSTMENT
							enable_autopriority = json_object_get_boolean(json_object_object_get(bridgefilterObj, "X_ZYXEL_Enable_AutoHighPriority"));
#endif
#ifdef ZYXEL_ANY_PORT_ANY_SERVICE_INTFGRP
							...
							if(strcmp(SourceMACFromVSIFilter,"")){
								...
								if(strlen(criinfo)>0)
									strcat(criinfo,";");
#ifdef ZYXEL_AUTO_INTF_GRP_QOS_PRI_ADJUSTMENT
								snprintf(criinfo+strlen(criinfo), sizeof(criinfo), "op60<%s %s>",SourceMACFromVendorClassIDFilter, (enable_autopriority)? "Y":"N");
#else
...
void dalcmdcirteriaToObject(json_object *BrFilterObj, char *brPathName, char *type, char *macvalue, char *op60value, char *op61value, char *op125tmp, char *wildcardEnable, char *apasSrcMac, char *autopriorityEnable);
zcfgRet_t dalcmdIntrGrpAdd(struct json_object *Jobj, char *replyMsg) {
	...
	const char *autopriorityEnable = NULL;
	const char *cmd = NULL;
	const char *act = NULL;
		...
		if(!strcmp(type, "vlangroup")) {
			...
			if(!strcmp(cmd, "GUI")){
...
#ifdef ZYXEL_AUTO_INTF_GRP_QOS_PRI_ADJUSTMENT
				autopriorityEnable = json_object_get_string(json_object_object_get(obj, "autopriorityEnable"));
#endif
			}
				...
				if(wildcard)
					...
					wildcardenable = "N";
#ifdef ZYXEL_AUTO_INTF_GRP_QOS_PRI_ADJUSTMENT
				autopriority = json_object_get_boolean(json_object_object_get(obj, "autopriorityEnable"));					
				if(autopriority)
					autopriorityEnable = "Y";
				else
					autopriorityEnable = "N";
#endif
			}
			IID_INIT(newBrFilterIid);
			...
			dalcmdcirteriaToObject(newBrFilterObj, newbrPathName, type, (char *)macvalue, (char *)op60value, (char *)op61value, (char *)op125value, (char *)wildcardenable, (char *)apasSrcMac, (char *)autopriorityEnable);
...
zcfgRet_t dalcmdIntrGrpEdit(struct json_object *Jobj, char *replyMsg, char *method){
	...
	//int ethidx = 0;
	const char *name = NULL, *wanintf = NULL, *lanintf = NULL, *type = NULL, *macvalue = NULL, *op60value = NULL, *op61value = NULL, *apasSrcMac = NULL, *autopriorityEnable = NULL;
		...
		while(obj != NULL) {
			...
			else
            {
				...
				if(!strcmp(act, "add")) {
					...
					dalcmdcirteriaToObject(BridgeFilterObj, brPath, (char *)type, (char *)macvalue, (char *)op60value, (char *)op61value, (char *)op125value, (char *)wildcardenable, (char *)apasSrcMac, (char *)autopriorityEnable);
					zcfgFeObjJsonBlockedSet(RDM_OID_BRIDGING_FILTER, &BridgeFilterIid, BridgeFilterObj, NULL);
					zcfgFeJsonObjFree(BridgeFilterObj);
					reset++;
				}	
...
void dalcmdcirteriaToObject(json_object *BrFilterObj, char *brPathName, char *type, char *macvalue, char *op60value, char *op61value, char *op125tmp, char *wildcardEnable, char *apasSrcMac, char *autopriorityEnable)
{
	...
	if(!strcmp(type, "mac")) {
		...
		if(!strcmp(wildcardEnable, "Y"))
			...
			json_object_object_add(BrFilterObj, "SourceMACFromVendorClassIDMode", json_object_new_string("Exact"));
#ifdef ZYXEL_AUTO_INTF_GRP_QOS_PRI_ADJUSTMENT
		if(autopriorityEnable && !strcmp(autopriorityEnable, "Y"))
			json_object_object_add(BrFilterObj, "X_ZYXEL_Enable_AutoHighPriority", json_object_new_boolean(true));
		else
			json_object_object_add(BrFilterObj, "X_ZYXEL_Enable_AutoHighPriority", json_object_new_boolean(false));
#endif
		}
...
#ifdef ZYXEL_AUTO_INTF_GRP_QOS_PRI_ADJUSTMENT
		json_object_object_add(criteria, "autopriorityEnable", JSON_OBJ_COPY(json_object_object_get(Jobj, "autopriorityEnable")));
#endif
		json_object_object_add(criteria, "act", json_object_new_string(act));
		json_object_object_add(criteria, "cmd", json_object_new_string("CLI"));
		json_object_object_add(criteria, "idx", json_object_new_int(index));
...
#ifdef ZYXEL_AUTO_INTF_GRP_QOS_PRI_ADJUSTMENT
		json_object_object_del(Jobj, "autopriorityEnable");
#endif
		json_object_object_add(Jobj, "0", JSON_OBJ_COPY(criteria));
	}
	else if(!strcmp(type, "op61")){
...
#ifdef ZYXEL_AUTO_INTF_GRP_QOS_PRI_ADJUSTMENT
			autopriority = strtok_r(NULL, ":", &ptr);
			autopriority = strtok_r(NULL, ",", &ptr);
#endif
			wildcard = strtok_r(NULL, ":", &ptr);
			wildcard = strtok_r(NULL, ",", &ptr);
#ifdef ZYXEL_AUTO_INTF_GRP_QOS_PRI_ADJUSTMENT
			if( (op60_value == NULL) || (autopriority == NULL) || (wildcard == NULL))
#else
			if( (op60_value == NULL) || (wildcard == NULL))
#endif
...
#ifdef ZYXEL_AUTO_INTF_GRP_QOS_PRI_ADJUSTMENT
			json_object_object_add(opt60, "autopriorityEnable", json_object_new_string(autopriority));
#endif
			json_object_object_add(opt60, "wildcardEnable", json_object_new_string(wildcard));
