	...
	while(zcfgFeObjJsonGetNextWithoutUpdate(RDM_OID_ZY_LOG_CFG_GP_ACCOUNT, &accountIid, &accountJobj) == ZCFG_SUCCESS){
		...
		*/
		...
		if(json_object_object_get(accountJobj, "Enabled") && json_object_object_get(accountJobj, "Username")){
			...
			if(accountIid.idx[0] == 2)
				json_object_object_add(paramJobj, "AccountGroup", json_object_new_string("Administrator"));
			else if(accountIid.idx[0] == 3)
				json_object_object_add(paramJobj, "AccountGroup", json_object_new_string("User"));
