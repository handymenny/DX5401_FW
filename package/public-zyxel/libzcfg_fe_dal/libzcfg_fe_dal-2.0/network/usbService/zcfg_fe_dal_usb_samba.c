...
#ifdef ZYXEL_ADVANCE_SMB_PRIVILEGE
	{"ValidGroup",		dalType_string,	0,	0,	NULL},
	{"WriteList",		dalType_string,	0,	0,	NULL},
	{"WriteGroup",		dalType_string,	0,	0,	NULL},
#endif //ZYXEL_ADVANCE_SMB_PRIVILEGE
...
#ifdef ZYXEL_ADVANCE_SMB_PRIVILEGE
	char *test[128] = {0};
	const char *ValidGroup = NULL;
	const char *WriteList = NULL;
	const char *WriteGroup = NULL;
#endif //ZYXEL_ADVANCE_SMB_PRIVILEGE
...
#ifdef ZYXEL_ADVANCE_SMB_PRIVILEGE
	if(json_object_object_get(Jobj, "WriteList")){
		WriteList = json_object_get_string(json_object_object_get(Jobj, "WriteList"));
		strncpy(test, WriteList, sizeof(test));
		char *s = strtok(test, " ");
		while(s != NULL) {
			if(strstr(Validuser_gui, s) == NULL)
				return ZCFG_INVALID_PARAM_VALUE;
			s = strtok(NULL, " ");
		}
	}
...
#ifdef ZYXEL_ADVANCE_SMB_PRIVILEGE
	if(json_object_object_get(Jobj, "ValidGroup"))
		ValidGroup = json_object_get_string(json_object_object_get(Jobj, "ValidGroup"));

	if(json_object_object_get(Jobj, "WriteGroup")){
		WriteGroup = json_object_get_string(json_object_object_get(Jobj, "WriteGroup"));
		strncpy(test, WriteGroup, sizeof(test));
		char *s = strtok(test, " ");
		while(s != NULL) {
			if(strstr(ValidGroup, s) == NULL)
				return ZCFG_INVALID_PARAM_VALUE;
			s = strtok(NULL, " ");
		}
	}
...
#ifdef ZYXEL_ADVANCE_SMB_PRIVILEGE
			if(!Public){
				if(json_object_object_get(Jobj, "ValidGroup") && strcmp(ValidGroup,""))
					json_object_object_add(sambadirObj, "X_ZYXEL_ValidGroup", json_object_new_string(ValidGroup));
				if(json_object_object_get(Jobj, "WriteGroup") && strcmp(WriteGroup,""))
					json_object_object_add(sambadirObj, "X_ZYXEL_WriteGroup", json_object_new_string(WriteGroup));
				if(json_object_object_get(Jobj, "ValidUsers") && strcmp(ValidUser,""))
					json_object_object_add(sambadirObj, "X_ZYXEL_ValidUsers", json_object_new_string(ValidUser));
				if(json_object_object_get(Jobj, "WriteList") && strcmp(WriteList,""))
					json_object_object_add(sambadirObj, "X_ZYXEL_WriteList", json_object_new_string(WriteList));
 			}
#else
			if(json_object_object_get(Jobj, "ValidUsers")){
				if(strcmp(ValidUser,""))
...
zcfgRet_t zcfgFeDalUsbSambaEdit(struct json_object *Jobj, char *replyMsg){
	...
	if(json_object_object_get(Jobj, "Public")){
		if(!Public){
			if(!json_object_object_get(Jobj, "ValidUsers")){
				strncpy(replyMsg, sizeof(replyMsg), "Please enter the valid users.");
				return ZCFG_INVALID_PARAM_VALUE;
			}
		}
	}
