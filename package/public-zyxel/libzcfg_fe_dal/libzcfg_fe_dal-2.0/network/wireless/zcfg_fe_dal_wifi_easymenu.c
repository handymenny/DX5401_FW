...
dal_param_t WanBackUp_param[]={
	{"VLANEnable",		dalType_boolean,	0,	0,	NULL,		NULL,			0},
	{"VLANID",			dalType_int,		0,	0,	NULL,		NULL,			0},
	{"VLANPriority",		dalType_int,		0,	0,	NULL,		NULL,			0},
	{"Username",				dalType_string,	 	0,	0,	NULL,		NULL,			0},
	{"Password",				dalType_string,	 	0,	0,	NULL,		NULL,			0},
	{"Encapsulation",			dalType_string,	 	0,	0,	NULL,		NULL,			0},
	{NULL,		0,	0,	0,	NULL},
};
#endif
...
#ifdef ZYXEL_TT_CUSTOMIZATION
zcfgRet_t zcfgFeDalWanBackUpEdit(struct json_object *Jobj, char *replyMsg){
	zcfgRet_t ret = ZCFG_SUCCESS;
	objIndex_t TTbackupIid;
	struct json_object *TTbackupObj = NULL;
	bool VLANEnable = false,sysGwEnable = false;
	int VLANID = 0;
	int VLANPriority = 0;
	const char *Username;
	const char *Password;
	...
	VLANEnable = json_object_get_boolean(json_object_object_get(Jobj, "VLANEnable"));
	VLANID = json_object_get_int(json_object_object_get(Jobj, "VLANID"));
	VLANPriority = json_object_get_int(json_object_object_get(Jobj, "VLANPriority"));
	...
	sysGwEnable = json_object_get_boolean(json_object_object_get(Jobj, "sysGwEnable"));

    IID_INIT(TTbackupIid);
	ret = zcfgFeObjJsonGet(RDM_OID_TTPPPBACKUP, &TTbackupIid, &TTbackupObj);
	if(ret == ZCFG_SUCCESS){
		    json_object_object_add(TTbackupObj, "VLANEnable", json_object_new_boolean(VLANEnable));
			json_object_object_add(TTbackupObj, "VLANID", json_object_new_int(VLANID));
			json_object_object_add(TTbackupObj, "VLANPriority", json_object_new_int(VLANPriority));
			json_object_object_add(TTbackupObj, "Username", json_object_new_string(Username));
			json_object_object_add(TTbackupObj, "Password", json_object_new_string(Password));
			json_object_object_add(TTbackupObj, "Encapsulation", json_object_new_string(Encapsulation));
			json_object_object_add(TTbackupObj, "sysGwEnable", json_object_new_boolean(sysGwEnable));

		ret = zcfgFeObjJsonSet(RDM_OID_TTPPPBACKUP, &TTbackupIid, TTbackupObj, NULL);
		zcfgFeJsonObjFree(TTbackupObj);
	}

	return ret;
}

zcfgRet_t zcfgFeDalWanBackUpGet(struct json_object *Jobj, struct json_object *Jarray, char *replyMsg){
	zcfgRet_t ret = ZCFG_SUCCESS;
	objIndex_t TTbackupIid;
	struct json_object *TTbackupObj = NULL;
    ...
    IID_INIT(TTbackupIid);
	if(zcfgFeObjJsonGet(RDM_OID_TTPPPBACKUP, &TTbackupIid, &TTbackupObj) == ZCFG_SUCCESS){
				json_object_object_add(paramJobj, "VLANEnable", JSON_OBJ_COPY(json_object_object_get(TTbackupObj, "VLANEnable")));
				json_object_object_add(paramJobj, "VLANID", JSON_OBJ_COPY(json_object_object_get(TTbackupObj, "VLANID")));
				json_object_object_add(paramJobj, "VLANPriority", JSON_OBJ_COPY(json_object_object_get(TTbackupObj, "VLANPriority")));
				json_object_object_add(paramJobj, "Username", JSON_OBJ_COPY(json_object_object_get(TTbackupObj, "Username")));
				json_object_object_add(paramJobj, "Password", JSON_OBJ_COPY(json_object_object_get(TTbackupObj, "Password")));
				json_object_object_add(paramJobj, "Encapsulation", JSON_OBJ_COPY(json_object_object_get(TTbackupObj, "Encapsulation")));
				json_object_object_add(paramJobj, "sysGwEnable", JSON_OBJ_COPY(json_object_object_get(TTbackupObj, "sysGwEnable")));
		        zcfgFeJsonObjFree(TTbackupObj);
	}
	else{
    		    json_object_object_add(paramJobj, "VLANEnable", json_object_new_boolean(1));
			    json_object_object_add(paramJobj, "VLANID", json_object_new_int(35));
			    json_object_object_add(paramJobj, "VLANPriority", json_object_new_int(0));
			    json_object_object_add(paramJobj, "Username", json_object_new_string("ttnetpppoe@ttnet"));
			    json_object_object_add(paramJobj, "Password", json_object_new_string("ttnetpppoe"));
			    json_object_object_add(paramJobj, "Encapsulation", json_object_new_string("PPPoE"));
				json_object_object_add(paramJobj, "sysGwEnable", json_object_new_boolean(1));
	}
...
zcfgRet_t zcfgFeDalWanBackUp(const char *method, struct json_object *Jobj, struct json_object *Jarray, char *replyMsg){
	zcfgRet_t ret = ZCFG_SUCCESS;
	int arrLen=0;
	int idx=0;
	struct json_object *obj = NULL;
	...
	char BAND[8] = {0};
		...
		if(!strncmp(method, "PUT", strlen("PUT"))) {
			ret = zcfgFeDalWanBackUpEdit(Jobj, replyMsg);
		}
		else if(!strncmp(method, "GET", strlen("GET")))
			ret = zcfgFeDalWanBackUpGet(Jobj, Jarray, replyMsg);
		else
			printf("Unknown method:%s\n", method);
