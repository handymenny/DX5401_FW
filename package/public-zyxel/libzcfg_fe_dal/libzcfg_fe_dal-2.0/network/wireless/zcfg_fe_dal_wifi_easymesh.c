...
zcfgRet_t zcfgFeDalWifiEasyMeshEdit(struct json_object *Jobj, char *replyMsg)
{
...
#ifdef ZYXEL_WEB_GUI_SHOW_MESH_ROLE_SELECT_OPTIONS
	objIndex_t objIid;
	...
	if(zcfgFeObjJsonGet(RDM_OID_EASY_MESH, &EasyMeshIid, &EasyMeshObj) == ZCFG_SUCCESS){
...
#endif
			{
				...
				if(EasyMeshEnable){
					...
					if(zcfgFeObjJsonGet(RDM_OID_OPERATION_MODE, &objIid, &operationMode) == ZCFG_SUCCESS) {
						OPmode = json_object_get_int(json_object_object_get(operationMode, "Mode"));
						if(Controller == 2){
							if(OPmode != 1){
								json_object_object_add(operationMode, "Mode", json_object_new_int(1));
								json_object_object_add(operationMode, "Band", json_object_new_int(0));
								json_object_object_add(operationMode, "Scan", json_object_new_int(0));
								zcfgFeObjJsonBlockedSet(RDM_OID_OPERATION_MODE, &objIid, operationMode, NULL);
								if(operationMode) zcfgFeJsonObjFree(operationMode);
							}
						}
						else if(Controller == 1){
							if(OPmode != 0){
								json_object_object_add(operationMode, "Mode", json_object_new_int(0));
								json_object_object_add(operationMode, "Band", json_object_new_int(0));
								json_object_object_add(operationMode, "Scan", json_object_new_int(0));
								zcfgFeObjJsonBlockedSet(RDM_OID_OPERATION_MODE, &objIid, operationMode, NULL);
								if(operationMode) zcfgFeJsonObjFree(operationMode);
		...
		}else if(!strcmp(json_object_get_string(json_object_object_get(Jobj, "cmdtype")), "CLI")){
				json_object_object_add(EasyMeshObj, "Enable", json_object_new_boolean(EasyMeshEnable));
				zcfgFeObjJsonBlockedSet(RDM_OID_EASY_MESH, &EasyMeshIid, EasyMeshObj, NULL);
		}
		zcfgFeJsonObjFree(EasyMeshObj);
