...
#include <pwd.h>
#include <zos_api.h>
#if defined(MSTC_TTNET_MULTI_USER) || defined(MSTC_RETAIL_MULTI_USER)
...
extern dal_param_t DHCP_ACL_POOL_param[];
extern dal_param_t DHCP_ACL_param[];
extern dal_param_t DHCP_ACL_DEF_ACT_param[];
#endif
...
extern dal_param_t Fsecure_param[];
#endif
...
#ifdef CONFIG_TT_PORTMIRROR
extern dal_param_t TT_PORT_MIRROR_param[];
#endif
#ifdef CONFIG_TT_PORTMIRROR_E
extern dal_param_t TT_PORT_MIRROR_param[];
#endif
#ifdef CONFIG_TT_PACKETCAPTURE
...
extern dal_param_t WanBackUp_param[];
#endif
...
#endif
#ifdef CONFIG_ABZQ_CUSTOMIZATION
extern zcfgRet_t zcfgFeDalDHCPAclPool(const char *method, struct json_object *Jobj, struct json_object *Jarray, char *replyMsg);
extern zcfgRet_t zcfgFeDalDHCPAcl(const char *method, struct json_object *Jobj, struct json_object *Jarray, char *replyMsg);
extern zcfgRet_t zcfgFeDalDHCPAclDefAct(const char *method, struct json_object *Jobj, struct json_object *Jarray, char *replyMsg);
#endif
#ifdef CONFIG_ZCFG_BE_MODULE_ZYXEL_VLAN_GROUP
...
#endif
extern zcfgRet_t zcfgFeDalPPPoEQuickSet(const char *method, struct json_object *Jobj, struct json_object *Jarray, char *replyMsg);
...
#endif
#ifdef CONFIG_ZCFG_BE_MODULE_ZYXEL_FSECURE
extern zcfgRet_t zcfgFeDalFsecure(const char *method, struct json_object *Jobj, struct json_object *Jarray, char *replyMsg);
#endif
...
#endif
extern zcfgRet_t zcfgFeDalWanBackUp(const char *method, struct json_object *Jobj, struct json_object *Jarray, char *replyMsg);
#endif
#ifdef ZYXEL_VPNLITE
...
#ifdef CONFIG_ABZQ_CUSTOMIZATION
extern void zcfgFeDalShowDHCPAclPool(struct json_object *Jarray);
extern void zcfgFeDalShowDHCPAcl(struct json_object *Jarray);
extern void zcfgFeDalShowDHCPAclDefAct(struct json_object *Jarray);
#endif
...
#endif
#ifdef CONFIG_ZCFG_BE_MODULE_ZYXEL_FSECURE
extern void zcfgFeDalShowFsecure(struct json_object *Jarray);
#endif
...
#ifdef ACBD_CUSTOMIZATION
extern dal_param_t portBindingSwitch_param[];
extern zcfgRet_t zcfgFeDalPortBindingSwitch(const char *method, struct json_object *Jobj, struct json_object *Jarray, char *replyMsg);
extern zcfgRet_t zcfgFeDalMgmtDefault(const char *method, struct json_object *Jobj, struct json_object *Jarray, char *replyMsg);
extern void zcfgFeDalShowMgmtDefault(struct json_object *Jarray);																 
#endif

#ifdef CONFIG_PACKAGE_OWAMP
extern dal_param_t TWAMP_param[];
extern zcfgRet_t zcfgFeDalTWAMP(const char *method, struct json_object *Jobj, struct json_object *Jarray, char *replyMsg);
extern zcfgRet_t zcfgFeDalShowTWAMP(struct json_object* Jarray);
#endif
...
#ifdef ACBD_CUSTOMIZATION
{"tr69default",		MGMT_param,			zcfgFeDalMgmtDefault,	zcfgFeDalShowMgmt,			"get",				"TR-069_Client",												0},
#endif 
...
#endif
#ifdef CONFIG_ABZQ_CUSTOMIZATION
{"dhcp_acl_pool",		DHCP_ACL_POOL_param, 	zcfgFeDalDHCPAclPool,		NULL,		"get|edit",	"Home_Networking"},//Home_Networking
{"dhcp_acl",		DHCP_ACL_param,	    zcfgFeDalDHCPAcl,	    zcfgFeDalShowDHCPAcl,	    "get|add|edit|delete",	"Home_Networking"},//Home_Networking
{"dhcp_acl_default_action", DHCP_ACL_DEF_ACT_param, zcfgFeDalDHCPAclDefAct,	zcfgFeDalShowDHCPAclDefAct,	"get|edit",	"Home_Networking"},//Home_Networking
#endif
#ifdef CONFIG_ZCFG_BE_MODULE_ZYXEL_VLAN_GROUP
...
#endif // #ifdef CONFIG_ZCFG_BE_MODULE_WIFI
{"wanbackup",	WanBackUp_param, zcfgFeDalWanBackUp,	NULL,	"edit|get",  "wanbackup"},
#endif
#ifdef CONFIG_ZCFG_BE_MODULE_QOS
...
#endif
{"pppoe_setting",	NETWORK_MAP_param,	zcfgFeDalPPPoEQuickSet,			NULL,			"edit|get",				"",															hidden_extender_dalcmd},
...
#endif
#ifdef CONFIG_ZCFG_BE_MODULE_ZYXEL_FSECURE
{"fsecure", 		Fsecure_param,	zcfgFeDalFsecure,	zcfgFeDalShowFsecure,						"get|edit",	"Parental_Control"},
#endif
...
#ifdef CONFIG_TT_PORTMIRROR
{"ttportmirror", TT_PORT_MIRROR_param, zcfgFeDalTTPortMirror, 	NULL,						"edit|get",				""},
#endif
#ifdef CONFIG_TT_PORTMIRROR_E
{"ttportmirror", TT_PORT_MIRROR_param, zcfgFeDalTTPortMirror, 	NULL,						"edit|get",				""},
#endif
...
#ifdef ZYXEL_WEB_WIRELESS_NEW_VERSION_ONE
#ifdef CONFIG_ZCFG_BE_MODULE_WIFI
{"WIFI_CBT",		WIFI_CBT_param,		zcfgFeDalCbtWifi,		NULL,		"edit",		"",																					0},
{"WIFI_ADVANCED_CBT",		WIFI_ADV_CBT_param,		zcfgFeDalCbtWifiAdv,		NULL,		"edit",		"",																	0},
#endif // #ifdef CONFIG_ZCFG_BE_MODULE_WIFI
#endif
#ifdef CONFIG_ZCFG_BE_MODULE_MOS_USER_CONFIG
...
#endif
#ifdef ACBD_CUSTOMIZATION
{"portBindingSwitch",  portBindingSwitch_param, zcfgFeDalPortBindingSwitch,    NULL,                        "edit|get",             "",                                  0},
#endif
#ifdef CONFIG_PACKAGE_OWAMP
{"TWAMP",			TWAMP_param,		zcfgFeDalTWAMP,			zcfgFeDalShowTWAMP,			"edit|get",				"",												0},
#endif
...
#if defined(ZYXEL_EASYMESH_R2) && defined(BROADCOM_PLATFORM)
		/* hidden traffic separation preset bridge */
		if(!strncmp(X_ZYXEL_SrvName, "brts",4)){
			continue;
		}
#endif
...
#ifdef ZYXEL_TT_CUSTOMIZATION
		X_TT_SrvName = json_object_get_string(json_object_object_get(ipIfaceObj, "X_TT_SrvName"));
		if(strstr(X_TT_SrvName,"_DSL") != NULL ){
		    json_object_object_add(Jobj, "BindName", json_object_new_string(X_TT_SrvName));
			json_object_object_add(Jobj, "BindType", json_object_new_string("DSL"));
		}
		else if(strstr(X_TT_SrvName,"_WAN") != NULL ){
		    json_object_object_add(Jobj, "BindName", json_object_new_string(X_TT_SrvName));
			json_object_object_add(Jobj, "BindType", json_object_new_string("ETH"));
										...
										tmp_string = strrchr(X_TT_SrvName,'_');
										strncpy(newName, X_TT_SrvName, strlen(X_TT_SrvName)-strlen(tmp_string));	
										newName[strlen(X_TT_SrvName)-strlen(tmp_string)] = '\0';
...
zcfgRet_t getmultiuserList(struct json_object **Jarray, const char *userName){
...
#endif
	int TTNETMultiUser = 1,Internet_WANpass = 1,IPTV_DSLpass = 1;
	...
	bool isEUI641 = true,isManual1 = true,enblIPv6LinkLocalAddress = true;
	bool isEUI642 = true,isManual2 = true,enblIPv6IdentifierAddress = true;
	bool isip6PrefixTypeDelegated = true,isIPv6PrefixWAN = true,enblPrefixStatic = true;
	bool isIPv6PrefixAddress = true,isip6LanAddrAssign = true,enblip6DNSAssign = true;
	bool isipv6IntfIDStart = true,isipv6IntfIDEnd = true,enblipv6DomainName = true;
	bool issysLanFirDns6State = true,issysLanSecDns6State = true,enblsysLanThiDns6State = true;
	bool isdnsServerAssign=true;
...
#ifdef TT_HGW
	struct json_object *InternetJarray = NULL;
    struct json_object *managementJarray = NULL;
    struct json_object *VoIPJarray = NULL;
    struct json_object *IPTVJarray = NULL;
	objIndex_t InternetIid,managementIid,VoIPIid,IPTVIid;
	int Internetv = 1,managementv = 1,VoIPv = 1,IPTVv = 1;
	bool InternetLimitation = false,managementLimitation = false,VoIPLimitation = false,IPTVLimitation = false;
	bool InternetLock = false,managementLock = false,VoIPLock = false,IPTVLock = false;
	int Internetpass = 1,managementpass = 1,VoIPpass = 1,IPTVpass = 1;
#endif
...
#ifdef TT_HGW	
	IID_INIT(InternetIid);
	if ( zcfgFeObjJsonGet(RDM_OID_INTERNET, &InternetIid,&InternetJarray) == ZCFG_SUCCESS)
	{
		Internetv = Jgeti(InternetJarray, "VisibilityLevel");
		InternetLimitation = Jgetb(InternetJarray, "Limitation");
		InternetLock = Jgetb(InternetJarray, "Lock");
	}
	IID_INIT(managementIid);
	if ( zcfgFeObjJsonGet(RDM_OID_MANAGEMENT, &managementIid,&managementJarray) == ZCFG_SUCCESS)
	{
	...
	IID_INIT(VoIPIid);
	if ( zcfgFeObjJsonGet(RDM_OID_VO_IP, &VoIPIid,&VoIPJarray) == ZCFG_SUCCESS)
	{
		VoIPv = Jgeti(VoIPJarray, "VisibilityLevel");
		VoIPLimitation = Jgetb(VoIPJarray, "Limitation");
		VoIPLock = Jgetb(VoIPJarray, "Lock");
	}
	IID_INIT(IPTVIid);
	if ( zcfgFeObjJsonGet(RDM_OID_IPTV, &IPTVIid,&IPTVJarray) == ZCFG_SUCCESS)
	{
		IPTVv = Jgeti(IPTVJarray, "VisibilityLevel");
		IPTVLimitation = Jgetb(IPTVJarray, "Limitation");
		IPTVLock = Jgetb(IPTVJarray, "Lock");
	}
#else	
	...
	*Jarray = json_object_new_array();
	...
	json_object_object_add(Jobj, "TTNETMultiUser", json_object_new_int(1));

	char TTuserame[512] = {0};
	struct json_object *TTuserobj = NULL;
	objIndex_t TTuserIid;
	IID_INIT(TTuserIid);	
					
	if (userName != NULL && strlen(userName) > 0){
		while(zcfgFeObjJsonGetNext(RDM_OID_TT_USER, &TTuserIid,&TTuserobj) == ZCFG_SUCCESS){
			strcpy(TTuserame, Jgets(TTuserobj, "Username"));
			if(strcmp(userName, TTuserame) == 0){
				curloginLevel = Jgeti(TTuserobj, "Level");
#ifdef TT_HGW
				strncpy(password, Jgets(TTuserobj, "Password"), sizeof(password)-1);
#endif
				json_object_put(TTuserobj);
				break;
				}
			json_object_put(TTuserobj);
			}
	...
	if(password != NULL)
	{
		...
		decopassword = base64_decode((unsigned char*)password, &len);
	}
#endif
	...
	if (userName != NULL && strlen(userName) > 0){
		while(zcfgFeObjJsonGetNext(RDM_OID_ZY_LOG_CFG_GP_ACCOUNT, &useraccountIid,&useraccountobj) == ZCFG_SUCCESS){
			strcpy(useraccountname, Jgets(useraccountobj, "Username"));
			if(strcmp(userName, useraccountname) == 0){
#ifndef TT_HGW
				strncpy(password, Jgets(useraccountobj, "Password"), sizeof(password)-1);
#endif
				strcpy(defpassword, Jgets(useraccountobj, "DefaultPassword"));
	...
	TtuserIid1.level = 1;
	TtuserIid1.idx[0] = 1; 
	if(zcfgFeObjJsonGet(RDM_OID_TT_USER, &TtuserIid1,&Ttuserobj1) == ZCFG_SUCCESS){
		strcpy(Ttuserame1, Jgets(Ttuserobj1, "Username"));
	...
	TtuserIid2.level = 1;
	TtuserIid2.idx[0] = 2; 
	if(zcfgFeObjJsonGet(RDM_OID_TT_USER, &TtuserIid2,&Ttuserobj2) == ZCFG_SUCCESS){
		strcpy(Ttuserame2, Jgets(Ttuserobj2, "Username"));
...
#ifdef TT_HGW
	json_object_object_add(Jobj, "Internetv", json_object_new_int(Internetv));
	json_object_object_add(Jobj, "InternetLimitation", json_object_new_boolean(InternetLimitation));
	json_object_object_add(Jobj, "Internet_DSLLock", json_object_new_boolean(InternetLock));
	...
	json_object_object_add(Jobj, "VoIPv", json_object_new_int(VoIPv));
	json_object_object_add(Jobj, "VoIPLimitation", json_object_new_boolean(VoIPLimitation));
	json_object_object_add(Jobj, "VoIPLock", json_object_new_boolean(VoIPLock));
	json_object_object_add(Jobj, "IPTVv", json_object_new_int(IPTVv));
	json_object_object_add(Jobj, "IPTVLimitation", json_object_new_boolean(IPTVLimitation));
	json_object_object_add(Jobj, "IPTVLock", json_object_new_boolean(IPTVLock));
#else
...
#ifdef TT_HGW	
	json_object_object_add(Jobj, "Internetpass", json_object_new_int(Internetpass));
	json_object_object_add(Jobj, "managementpass", json_object_new_int(managementpass));
	json_object_object_add(Jobj, "VoIPpass", json_object_new_int(VoIPpass));
	json_object_object_add(Jobj, "IPTVpass", json_object_new_int(IPTVpass));
#else
...
#ifdef TT_HGW
	json_object_object_add(Jobj, "password", json_object_new_string(decopassword));
#else
	...
	while(zcfgFeObjJsonGetNext(RDM_OID_IP_IFACE, &ipinterfaceIid,&ipinterfaceJarray) == ZCFG_SUCCESS){
		...
		if(isAddFromTr69 == true){
			strcpy(tr69wanname,Jgets(ipinterfaceJarray, "X_TT_SrvName"));	
#ifndef TT_HGW			
...
#ifdef TT_HGW
	if(!strcmp(wanname, "Internet_WAN"))
	{
		if(curloginLevel == 1)
		{
			if (Internetv == 2 || Internetv == 3)
			{
				zcfgLog(ZCFG_LOG_INFO, "%s(): Internet_WAN VisibilityLevel is equal to 2 or 3\n", __FUNCTION__);
			}
			else if(InternetLock==false)
			{
				if(InternetLimitation ==true)
				{
					//iswanenable=false;
					issrv_name=false;
					istypeSelect=false;
				...
				else if(InternetLimitation ==false)
				{
					//iswanenable=false;
					isdns=false;
					isipDns1=false;
	...
	else if(!strcmp(wanname, "Management_WAN"))
	{
		if(curloginLevel == 1)
		{
	...
	else if(!strcmp(wanname, "VoIP_WAN"))
	{
		if(curloginLevel == 1)
		{
			if (VoIPv == 2 || VoIPv == 3)
			...
			else if(VoIPLock==false)
			{
				if(VoIPLimitation ==true)
				{
					iswanenable=false;
					issrv_name=false;
					istypeSelect=false;
	...
	else if(!strcmp(wanname, "IPTV_WAN"))
	{
		if(curloginLevel == 1)
		{
			...
			else if(IPTVLock==false)
			{
				if(IPTVLimitation ==true)
				{
					iswanenable=false;
					issrv_name=false;
					istypeSelect=false;
	...
	if(curloginLevel == 1)
	{
		...
		else if (LANSetupLimitation==true)
		{
			if(LANSetupLock ==false)
			{
				...
				isEUI641 = false;
				isManual1 = false;
				enblIPv6LinkLocalAddress = false;
				isEUI642 = false;
				isManual2 = false;
				enblIPv6IdentifierAddress = false;
				isip6PrefixTypeDelegated = false;
				isIPv6PrefixWAN = false;
				enblPrefixStatic = false;
				isIPv6PrefixAddress = false;
				isip6LanAddrAssign = false;
				enblip6DNSAssign = false;
				isipv6IntfIDStart = false;
				isipv6IntfIDEnd = false;
				enblipv6DomainName = false;
				issysLanFirDns6State = false;
				issysLanSecDns6State = false;
				enblsysLanThiDns6State = false;
				isdnsServerAssign = false;
				//Admin will see the limited version of the page
			}
			else if(LANSetupLock ==true)
			{
	...
	json_object_object_add(Jobj, "isEUI641", json_object_new_boolean(isEUI641));
	json_object_object_add(Jobj, "isManual1", json_object_new_boolean(isManual1));	
	json_object_object_add(Jobj, "enblIPv6LinkLocalAddress", json_object_new_boolean(enblIPv6LinkLocalAddress));	
	json_object_object_add(Jobj, "isEUI642", json_object_new_boolean(isEUI642));
	json_object_object_add(Jobj, "isManual2", json_object_new_boolean(isManual2));
	json_object_object_add(Jobj, "enblIPv6IdentifierAddress", json_object_new_boolean(enblIPv6IdentifierAddress));
	json_object_object_add(Jobj, "isip6PrefixTypeDelegated", json_object_new_boolean(isip6PrefixTypeDelegated));
	json_object_object_add(Jobj, "isIPv6PrefixWAN", json_object_new_boolean(isIPv6PrefixWAN));
	json_object_object_add(Jobj, "enblPrefixStatic", json_object_new_boolean(enblPrefixStatic));
	json_object_object_add(Jobj, "isIPv6PrefixAddress", json_object_new_boolean(isIPv6PrefixAddress));
	json_object_object_add(Jobj, "isip6LanAddrAssign", json_object_new_boolean(isip6LanAddrAssign));
	json_object_object_add(Jobj, "enblip6DNSAssign", json_object_new_boolean(enblip6DNSAssign));
	json_object_object_add(Jobj, "isipv6IntfIDStart", json_object_new_boolean(isipv6IntfIDStart));
	json_object_object_add(Jobj, "isipv6IntfIDEnd", json_object_new_boolean(isipv6IntfIDEnd));
	json_object_object_add(Jobj, "enblipv6DomainName", json_object_new_boolean(enblipv6DomainName));
	json_object_object_add(Jobj, "issysLanFirDns6State", json_object_new_boolean(issysLanFirDns6State));
	json_object_object_add(Jobj, "issysLanSecDns6State", json_object_new_boolean(issysLanSecDns6State));
	json_object_object_add(Jobj, "enblsysLanThiDns6State", json_object_new_boolean(enblsysLanThiDns6State));
	json_object_object_add(Jobj, "isdnsServerAssign", json_object_new_boolean(isdnsServerAssign));
	...
	if((TTNETMultiUser == 1) && (LocalEnable == false))	/*__MSTC__, Linda, CPE User Definitions and Rights. */
	{
		enbllocallan = false;
	}
...
#ifdef TT_HGW
	json_object_put(InternetJarray);
	json_object_put(managementJarray);
	json_object_put(VoIPJarray);
	json_object_put(IPTVJarray);
#else	
...
zcfgRet_t getLoginUserInfo(struct json_object *Jobj, char *userName, char *level, char *remoteAddress){
	...
	if(json_object_get_type(Jobj) == json_type_object){
		...
		if(remoteAddress) strcpy(remoteAddress, json_object_get_string(json_object_object_get(Jobj, DAL_LOGIN_REMOTEADDR)));
	}
	else if(json_object_get_type(Jobj) == json_type_array){
		subJobj = json_object_array_get_idx(Jobj, 0);
		if(!subJobj)
		...
		if(remoteAddress) strcpy(remoteAddress, json_object_get_string(json_object_object_get(subJobj, DAL_LOGIN_REMOTEADDR)));
	}
	else
		return ZCFG_INTERNAL_ERROR;
...
{
	int i=0;
	char oldFile[64]={0}, newFile[64]={0};

	for(i=logFileRotate-1; i>0; i--) {
        zos_snprintf(oldFile, sizeof(oldFile), "%s%d", flashLogFilePath, i-1);
        zos_snprintf(newFile, sizeof(newFile), "%s%d", flashLogFilePath, i);
...
void copyLogToFlash(char *logFilePath, char *flashLogFilePath, unsigned int logFileSize, int logFileRotate)
{
	...
	FILE *dummyFP=NULL, *rotateFP=NULL;

	char log_0_file[128]={0};
	char log_1_file[128]={0};

    zos_snprintf(log_1_file, sizeof(log_1_file), "%s.%d", logFilePath, 1);

	if((rotateFP = ZOS_FOPEN(log_1_file,"r")) != NULL)
	{
		ZOS_FCLOSE(rotateFP);
		printf("move %s to flash\n", log_1_file);
		zos_snprintf(cmd, sizeof(cmd), "cp %s %s%d", log_1_file, flashLogFilePath, 1);
        ZOS_SYSTEM(cmd);
		printf("move %s to flash\n", logFilePath);
		zos_snprintf(cmd, sizeof(cmd), "cp %s %s%d", logFilePath, flashLogFilePath, 0);
        ZOS_SYSTEM(cmd);
		return;
	}


    zos_snprintf(log_0_file, sizeof(log_0_file), "%s%d", flashLogFilePath, 0);
	...
	if((dummyFP = ZOS_FOPEN(logFilePath,"r")) == NULL) 
	{
	...
	if((dummyFP = ZOS_FOPEN(log_0_file,"r")) == NULL) 
	{
		printf("move %s to flash\n", logFilePath);
        zos_snprintf(cmd, sizeof(cmd), "cp %s %s%d", logFilePath, flashLogFilePath, 0);
		ZOS_SYSTEM(cmd);
		// Clean /var/log/syslog.log. Do not delete it.
        zos_snprintf(cmd, sizeof(cmd), "> %s", logFilePath);
		ZOS_SYSTEM(cmd);
	}
	else
	{
        ...
        zos_snprintf(cmd, sizeof(cmd), "cp %s%d %s", flashLogFilePath, 0, tempFile);
        ZOS_SYSTEM(cmd);
		...
		if((fp = ZOS_FOPEN(logFilePath,"r")) != NULL)
		{
			...
			{ /* 150k -30k */
				...
				else
				{
                    moveAll = TRUE;
                    printf("save %s to flash\n", logFilePath);
                    zos_snprintf(cmd, sizeof(cmd), "cat %s %s >> %s%d", tempFile, logFilePath, flashLogFilePath, 0);
                    ZOS_SYSTEM(cmd);
				}
			}
			if(moveAll)
			...
			while (index < logRemainSize)
			{
                ...
                zos_snprintf(cmd, sizeof(cmd), "echo \"%s\" >> %s", buffer, tempFile);
				ZOS_SYSTEM(cmd);
				index += strlen(buffer);
			}
		}
        ...
        zos_snprintf(cmd, sizeof(cmd), "cp %s %s%d", tempFile, flashLogFilePath, 0);
		ZOS_SYSTEM(cmd);
...
void copySystemLogToFlash (bool uSaveSecurity)
{
    ...
    zos_snprintf(tmpfilename, sizeof(tmpfilename), "%s%s",LOG_FILE_PATH, FLASH_LOG_FILE_SYSLOG);
    zos_snprintf(tmpflashfilename, sizeof(tmpflashfilename), "%s%s",FLASH_LOG_FILE_PATH, FLASH_LOG_FILE_SYSLOG);

	copyLogToFlash(tmpfilename, tmpflashfilename, 200*1024, LOG_ROTATE);

	if ( uSaveSecurity == true )
	{
        zos_snprintf(tmpfilename, sizeof(tmpfilename), "%s%s",LOG_FILE_PATH, FLASH_LOG_FILE_SECURITY);
        zos_snprintf(tmpflashfilename, sizeof(tmpflashfilename), "%s%s",FLASH_LOG_FILE_PATH, FLASH_LOG_FILE_SECURITY);
		copyLogToFlash(tmpfilename, tmpflashfilename, 200*1024, LOG_ROTATE);
	}
