...
#if 1  //TTNET Feature Request: Interface Down and Factory Reset Reason Code to record the reason of CPE reset to default
...
#endif
#ifdef CONFIG_TT_CUSTOMIZATION_TR069  //TTNET Feature Request: Interface Down and Factory Reset Reason Code to record the reason of CPE reset to default
void SetFactoryResetCode_TT(const char *frCode)
{
	FILE *frfp=NULL;
	char frbuf[16] = {0};
	time_t timep;
	...
	if(frfp != NULL)
	{
		fprintf(frfp,"%s@%s", frCode, frbuf);	//for reset to default, X_TT_FR_ReasonCode: 1=GUI, 2=Telnet, 3=Button, 4=TR-069
		fclose(frfp);
		sync();
	}else
		return;
