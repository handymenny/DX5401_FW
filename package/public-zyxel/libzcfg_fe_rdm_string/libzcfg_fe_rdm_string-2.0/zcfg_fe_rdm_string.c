...
zcfgRet_t zcfgFeObjGetObjectDefaultValue(char *pObj_path_name,json_type type,char *pOut_value,int Out_max_len){
	int err = ZCFG_SUCCESS;
	zcfg_offset_t oid;
	objIndex_t objIid;
	char obj_path_name[256] = {0};
	char obj_path[256] = {0};
	char param_name[256] = {0};
    	objInfo_t *objInfo = NULL;
	bool found_flag = false;
	char *pParamName = NULL;
	ZOS_STRNCPY(obj_path_name,pObj_path_name,256);
	pParamName = strrchr(obj_path_name,'.');
	if(pParamName != NULL){
		ZOS_STRNCPY(param_name,pParamName+1,256);
		*pParamName = '\0';
		ZOS_STRNCPY(obj_path,obj_path_name,256);
	}else{
		zcfgLog(ZCFG_LOG_ERR,"Invalid Object parameter path\n");
		err = ZCFG_INTERNAL_ERROR;
	}
	if(err == ZCFG_SUCCESS){
		IID_INIT(objIid);
		oid = zcfgFeObjNameToObjId(obj_path,&objIid);
		if(oid != ZCFG_NO_SUCH_OBJECT && pOut_value != NULL && Out_max_len > 0){
			objInfo = GET_OBJ_INFO_BY_OID(oid);
			GET_PAMAMETERS_IN_OBJECT(objInfo, paramInfo, paramName){
				if(strcmp(paramName, param_name) == 0) {
					found_flag = true;
					break;
				}
			}
			if(found_flag){
				if(type == json_type_boolean){
					if(paramInfo->iniVal){
						ZOS_STRNCPY(pOut_value,"true",Out_max_len);
					}else{
						ZOS_STRNCPY(pOut_value,"false",Out_max_len);
					}
				}else if(type == json_type_int){
					ZOS_SNPRINTF(pOut_value,Out_max_len, "%d",paramInfo->iniVal);
				}else if(type == json_type_uint8 || type == json_type_uint16 || type == json_type_uint32 || type == json_type_ulong){
					ZOS_SNPRINTF(pOut_value,Out_max_len, "%u",paramInfo->iniVal);
				}else if(type == json_type_string || type == json_type_time || type == json_type_base64 || type == json_type_hex){
					if(objDefaultValPtr != NULL){
						ZOS_STRNCPY(pOut_value,objDefaultValPtr + paramInfo->iniVal,Out_max_len);
					}else{
						zcfgLog(ZCFG_LOG_ERR, "objDefaultValPtr = NULL\n");
						err = ZCFG_INTERNAL_ERROR;
					}
				}	
			}else{
				zcfgLog(ZCFG_LOG_ERR,"Invalid parameter name [%s]\n",param_name);
				err = ZCFG_INTERNAL_ERROR;
			}
		}else{
			zcfgLog(ZCFG_LOG_ERR,"Invalid handle\n");
			err = ZCFG_INTERNAL_ERROR;
		}
	}
  	return err;
