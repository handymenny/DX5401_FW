	...
	while(paramList->name != NULL) {
		...
		if(strcmp(paramList->name, "ModelName") == 0){
			...
			if (strstr(swVer, "ABLZ") != NULL) {
				...
				char newModelName[128] = {0};
				char *modelName = NULL;
				modelName = json_object_get_string(json_object_object_get(devInfoObj, "ModelName"));
				if(modelName != NULL){
					zos_snprintf(newModelName, sizeof(newModelName), "Wind3 HUB - %s", modelName);
					json_object_object_add(*tr98Jobj, paramList->name, json_object_new_string(newModelName));
					paramList++;
					continue;
				}
			}
