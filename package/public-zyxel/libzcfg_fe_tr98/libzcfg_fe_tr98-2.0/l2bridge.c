...
zcfgRet_t l2BrObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	...
	if(zcfgFeCheckQuickBridging(&brIid))
	{
		//zcfgLog()
...
#ifdef CONFIG_TT_CUSTOMIZATION
	if(zcfgFeCheckQuickBridging(&objIid))
	{
		//zcfgLog()
		return ZCFG_REQUEST_REJECT;
	}
...
zcfgRet_t l2BrFilterObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	...
	{
		objIndex_t objIid = {0};
		const char *bridingPath = json_object_get_string(json_object_object_get(brFilterJobj, "Bridge"));

		if(bridingPath && sscanf(bridingPath, "Bridging.Bridge.%hhu", &objIid.idx[0]))
		{
			objIid.level = 1;
			if(zcfgFeCheckQuickBridging(&objIid))
			{
				//zcfgLog("Cannot set quick bridging objects");
				json_object_put(brFilterJobj);
				return ZCFG_REQUEST_REJECT;
			}
		}
...
#ifdef CONFIG_TT_CUSTOMIZATION
	{
		struct json_object *brFilObj = NULL;

		if(oid == RDM_OID_BRIDGING_FILTER && zcfgFeObjJsonGetWithoutUpdate(oid, &objIid, &brFilObj) == ZCFG_SUCCESS)
		{
			const char *bridingPath = json_object_get_string(json_object_object_get(brFilObj, "Bridge"));
			objIndex_t brIid = {0};

			if(bridingPath && sscanf(bridingPath, "Bridging.Bridge.%hhu", &brIid.idx[0]))
			{
				brIid.level = 1;
				if(zcfgFeCheckQuickBridging(&brIid))
				{
					//zcfgLog("Cannot delete quick bridging objects");
					json_object_put(brFilObj);
					return ZCFG_REQUEST_REJECT;
				}
			}
			json_object_put(brFilObj);
...
// lanPortSn: lan port, 1~4
bool zcfgBrLanPortQueryExist(uint8_t bridgingSn, uint8_t lanPortSn)
{
	char bridgingBrPath[100] = {0};
	objIndex_t objIid;
	struct json_object *bridgingflObj = NULL;
	const char *bridgingflbr = NULL;
	bool exist = false;

	if(lanPortSn < 1 || lanPortSn > 4)
		return ZCFG_INTERNAL_ERROR;

	snprintf(bridgingBrPath, sizeof(bridgingBrPath), "Bridging.Bridge.%hhu", bridgingSn);

	IID_INIT(objIid);
	objIid.level = 1;
	objIid.idx[0] = lanPortSn;

	if(zcfgFeObjJsonGetWithoutUpdate(RDM_OID_BRIDGING_FILTER, &objIid, &bridgingflObj) == ZCFG_SUCCESS){
		bridgingflbr = json_object_get_string(json_object_object_get(bridgingflObj, "Bridge"));
		exist = (!strncmp(bridgingflbr, bridgingBrPath, sizeof(bridgingflbr))) ? true : false;

		json_object_put(bridgingflObj);
	}
