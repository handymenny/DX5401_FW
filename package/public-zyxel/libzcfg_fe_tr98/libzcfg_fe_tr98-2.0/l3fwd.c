...
int GetTTForwardingNumber(){
	int Count = 0;
	objIndex_t objIid_old;
	struct json_object *rtoldObj;
			
	IID_INIT(objIid_old);
	objIid_old.level = 2;
	objIid_old.idx[0] = 1;
	while(zcfgFeObjJsonGetNextWithoutUpdate(RDM_OID_TT_FORWARDING, &objIid_old, &rtoldObj) == ZCFG_SUCCESS) {
			Count++;
		zcfgFeJsonObjFree(rtoldObj);
	}
...
zcfgRet_t l3fwdObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	...
	*tr98Jobj = json_object_new_object();
	...
	int IPv4ForwardingNumber = 0;
		IPv4ForwardingNumber = GetTTForwardingNumber();
	...
	memset(X_ZYXEL_ActiveDefaultGateway, 0, sizeof(X_ZYXEL_ActiveDefaultGateway));
	memset(ipIfaceName, 0, sizeof(ipIfaceName));
	l3fwdGetDfGwByRtTable(X_ZYXEL_ActiveDefaultGateway, 6);
	if(ifNameToIpIfaceName((const char *)X_ZYXEL_ActiveDefaultGateway, ipIfaceName) == ZCFG_SUCCESS){
		char *tr98WanIpIface = translateWanIpIface(TRANS_WANIPIFACE_181TO98, ipIfaceName);
		if(tr98WanIpIface){
#ifndef TT_CUSTOMIZATION_V6_TTNET
			json_object_object_add(*tr98Jobj, "X_TTG_IPv6DefaultConnectionService", json_object_new_string(tr98WanIpIface));
#else
			json_object_object_add(*tr98Jobj, "X_TTNET_IPv6DefaultConnectionService", json_object_new_string(tr98WanIpIface));
#endif
			ZOS_FREE(tr98WanIpIface);
		}
	}
#endif
...
zcfgRet_t l3fwdObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	...
	const char *tr181_X_ZYXEL_ActiveV6DefaultGateway = json_object_get_string(json_object_object_get(routerObj, "X_ZYXEL_ActiveV6DefaultGateway"));
	...
	if(paramValue = json_object_object_get(tr98Jobj, "DefaultConnectionService")){
...
#ifdef TT_CUSTOMIZATION_V6
#ifndef TT_CUSTOMIZATION_V6_TTNET
	else if(paramValue = json_object_object_get(tr98Jobj, "X_TTG_IPv6DefaultConnectionService")){
#else
	else if(paramValue = json_object_object_get(tr98Jobj, "X_TTNET_IPv6DefaultConnectionService")){
#endif
		char X_ZYXEL_ActiveV6DefaultGateway[513] = {0};
		const char *tr98IPv6DefaultConnectionService = json_object_get_string(paramValue);
		if(strlen(tr98IPv6DefaultConnectionService) && (!strstr(tr98IPv6DefaultConnectionService, "WANDevice") || !strstr(tr98IPv6DefaultConnectionService, "WANConnectionDevice"))){
			ret = ZCFG_INVALID_PARAM_VALUE;
		}else if(strlen(tr98IPv6DefaultConnectionService)){
			ZOS_STRNCPY(DefaultConnectionService, json_object_get_string(paramValue), sizeof(DefaultConnectionService));
			printf("DefaultConnectionService=%s\n", DefaultConnectionService);
			char *tr181WanIpIface = translateWanIpIface(TRANS_WANIPIFACE_98TO181, DefaultConnectionService);
			if(tr181WanIpIface) {
				if(strlen(tr181_X_ZYXEL_ActiveV6DefaultGateway) == 0)
					ZOS_STRNCPY(X_ZYXEL_ActiveV6DefaultGateway, tr181WanIpIface, sizeof(X_ZYXEL_ActiveV6DefaultGateway));
				else if(!strstr(tr181_X_ZYXEL_ActiveV6DefaultGateway, tr181WanIpIface))
					snprintf(X_ZYXEL_ActiveV6DefaultGateway, sizeof(X_ZYXEL_ActiveV6DefaultGateway), "%s,%s", tr181_X_ZYXEL_ActiveV6DefaultGateway, tr181WanIpIface);
				else
					ZOS_STRNCPY(X_ZYXEL_ActiveV6DefaultGateway, tr181_X_ZYXEL_ActiveV6DefaultGateway, sizeof(X_ZYXEL_ActiveV6DefaultGateway));

				ZOS_FREE(tr181WanIpIface);
			}else{
				ret = ZCFG_INVALID_PARAM_VALUE;
			}
		}
		...
		if(ret == ZCFG_SUCCESS){
			printf("X_ZYXEL_ActiveV6DefaultGateway=%s\n", X_ZYXEL_ActiveV6DefaultGateway);
			json_object_object_add(routerObj, "X_ZYXEL_ActiveV6DefaultGateway", json_object_new_string(X_ZYXEL_ActiveV6DefaultGateway));
		}else{
			json_object_put(routerObj);
			return ret;
		}
...
zcfgRet_t l3fwdFwdTbObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
    ...
    sscanf(tr98FullPathName, "InternetGatewayDevice.Layer3Forwarding.Forwarding.%d", &GetFwdIndex);
	/*mapping InternetGatewayDevice.Layer3Forwarding to Device.Routing.Router.1*/
	IID_INIT(objIid);
	objIid.level = 2;
	objIid.idx[0] = 1;
	objIid.idx[1] = GetFwdIndex;

	if((ret = feObjJsonGet(RDM_OID_TT_FORWARDING, &objIid, &ipv4FwdObj, updateFlag)) != ZCFG_SUCCESS)
		return ret;
#else
