...
#ifdef TT_HGW
#define WPS_DEVICENAME "EX3301-T0"
#else
...
#define TR98_PARM_LAN_DEV_WLAN_CFG "{\
		...
		\"X_CUSTOM_OperatingFrequencyBand\": { \"2.4GHz\": { }, \"5GHz\": { } },\
		\"X_CUSTOM_OperatingChannelBandwidth\": { \"20MHz\": { }, \"40MHz\": { }, \"80MHz\": { } , \"160MHz\": { } },\
		}"
...
zcfgRet_t lanDevHostObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	...
	*tr98Jobj = json_object_new_object();
	...
	while(paramList->name != NULL) {
#ifdef ACEN_CUSTOMIZATION
		if(!strcmp(paramList->name, "X_CUSTOM_SignalStrength")) {
		...
		if(!strcmp(paramList->name, "X_CUSTOM_RSSI")) {
			paramValue = json_object_object_get(hostJobj, "X_ZYXEL_RSSI");
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
		...
		if(!strcmp(paramList->name, "X_CUSTOM_HostType")) {
			paramValue = json_object_object_get(hostJobj, "X_ZYXEL_HostType");
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
		}

		if(!strcmp(paramList->name, "X_CUSTOM_SNR")) {
			paramValue = json_object_object_get(hostJobj, "X_ZYXEL_SNR");
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
		}

		if(!strcmp(paramList->name, "X_CUSTOM_LastDataDownlinkRate")) {
			paramValue = json_object_object_get(hostJobj, "X_ZYXEL_LastDataDownlinkRate");
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
		}

		if(!strcmp(paramList->name, "X_CUSTOM_LastDataUplinkRate")) {
			paramValue = json_object_object_get(hostJobj, "X_ZYXEL_LastDataUplinkRate");
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
		}
		...
		else if(!strncmp(paramList->name, "IPv6Address", (strlen(paramList->name) >= strlen("IPv6Address")) ? strlen(paramList->name) : strlen("IPv6Address"))){
			paramValue = json_object_object_get(hostJobj, "IPAddress6");
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
		}
		else if(!strncmp(paramList->name, "IPv6LinkLocal", (strlen(paramList->name) >= strlen("IPv6LinkLocal")) ? strlen(paramList->name) : strlen("IPv6LinkLocal"))){
			paramValue = json_object_object_get(hostJobj, "IPLinkLocalAddress6");
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
		}
...
#ifdef ACEN_CUSTOMIZATION
		else if(!strcmp(paramList->name, "X_CUSTOM_OperatingFrequencyBand")) {
			paramValue = json_object_object_get(hostJobj, "X_ZYXEL_ConnectionType");
...
#ifdef ACEN_CUSTOMIZATION
		else if(!strcmp(paramList->name, "X_CUSTOM_OperatingChannelBandwidth")) {
#else
...
#ifdef ACEN_CUSTOMIZATION
	const char *opBand = json_object_get_string(json_object_object_get(tr98Jobj, "X_CUSTOM_OperatingFrequencyBand"));
	const char *opBandwidth = json_object_get_string(json_object_object_get(tr98Jobj, "X_CUSTOM_OperatingChannelBandwidth"));
#else
...
#ifdef ACEN_CUSTOMIZATION
			json_object_object_add(tr98Jobj, "X_CUSTOM_OperatingChannelBandwidth", json_object_new_string("20MHz"));
#else
...
#ifdef ACEN_CUSTOMIZATION
	opBandwidth = json_object_get_string(json_object_object_get(tr98Jobj, "X_CUSTOM_OperatingChannelBandwidth"));
#else
...
zcfgRet_t lanDevWlanCfgObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
...
#endif
#ifdef ABPY_CUSTOMIZATION
	bool changedoperatingstandards = false;
#endif
	...
	if(multiJobj){
#if (defined(ZYXEL_WLAN_BLOCK_OPEN) || defined(ZYXEL_CONFIG_MAX_WLAN_CLIENT))
		tmpObj = wifiJobj;
		wifiJobj = NULL;
	...
	while(paramList->name != NULL) {
		...
		if(!strcmp(paramList->name, "RadioEnabled")) {
...
#ifdef ABPY_CUSTOMIZATION
			if(!changedoperatingstandards){
				const char *oldvalue = json_object_get_string(json_object_object_get(radioJobj, "OperatingStandards"));
				const char *newvalue = json_object_get_string(paramValue);
				if(strcmp(oldvalue, newvalue)){
					char tmpstr[256]={0};
					char map[256]={0};
					int i=0, j=0;
				
					strncpy(map,"11", sizeof(map)-1);
					strncpy(tmpstr, newvalue, sizeof(tmpstr)-1);	
					for (i = j = 0; tmpstr[i] != '\0'; i++) {
						if (tmpstr[i] != ',')
							tmpstr[j++] = tmpstr[i];
					}
					tmpstr[j] = '\0';
					strncat(map, tmpstr,sizeof(map)-strlen(map)-1);
					json_object_object_add(radioJobj, "X_ZYXEL_Wireless_Mode", json_object_new_string(map));
					changedoperatingstandards = true;
				}
			...
			if(!changedoperatingstandards){
				const char *oldvalue = json_object_get_string(json_object_object_get(radioJobj, "X_ZYXEL_Wireless_Mode"));
				const char *newvalue = json_object_get_string(paramValue);
				if(strcmp(oldvalue, newvalue)){
					char tmpstr[256]={0};
					char map[256]={0};
					...
					for (i = j = 0; tmpstr[i] != '\0'; i++) {
						if (tmpstr[i] == 'a' && (tmpstr[i + 1] == 'c' || tmpstr[i + 1] == 'x')) {
							map[j++] = tmpstr[i];
							map[j++] = tmpstr[i + 1];
							i++;
						}
					...
					map[j] = '\0';
					json_object_object_add(radioJobj, "OperatingStandards", json_object_new_string(map));
					changedoperatingstandards = true;
				}
				json_object_object_add(radioJobj, "X_ZYXEL_Wireless_Mode", JSON_OBJ_COPY(paramValue));
			}
			paramList++;
			continue;
		}
...
#ifdef ACEN_CUSTOMIZATION
		else if(!strcmp(paramList->name, "X_CUSTOM_SignalStrength")) {
			paramValue = json_object_object_get(assocDevJobj, "SignalStrength");
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
		}
		else if(!strcmp(paramList->name, "X_CUSTOM_RSSI")) {
			paramValue = json_object_object_get(assocDevJobj, "SignalStrength");
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
...
#ifdef ACEN_CUSTOMIZATION
		/*special case*/
		if(!strcmp(paramList->name, "X_CUSTOM_AutoGenPSK")) {
			paramValue = json_object_object_get(apSecJobj, "X_ZYXEL_AutoGenPSK");
			printf("### get X_ZYXEL_AutoGenPSK =%d\n", json_object_get_boolean(json_object_object_get(apSecJobj, "X_ZYXEL_AutoGenPSK")));
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
		}
...
#ifdef ACEN_CUSTOMIZATION
	setAutoGenPsk = json_object_get_boolean(json_object_object_get(tr98Jobj, "X_CUSTOM_AutoGenPSK"));
#else
	...
	while(paramList->name != NULL) {
		...
		if(paramValue != NULL) {
			...
			if (!strcmp(paramList->name, "X_CUSTOM_AutoGenPSK")){
				paramValue = json_object_object_get(tr98Jobj, paramList->name);
				if(paramValue != NULL) {
					tr181ParamValue = json_object_object_get(apSecJobj, "X_ZYXEL_AutoGenPSK");
					if(tr181ParamValue != NULL) {
						json_object_object_add(apSecJobj, "X_ZYXEL_AutoGenPSK", JSON_OBJ_COPY(paramValue));
						paramList++;
						continue;
					}
				}
...
zcfgRet_t wifiNeiborDiagResultObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	...
	iid.level = 1;
#ifdef ACEN_CUSTOMIZATION
	sscanf(tr98FullPathName, "InternetGatewayDevice.X_CUSTOM_NeighboringWiFiDiagnostic.Result.%hhu", &iid.idx[0]);
#else
...
#ifdef TT_CUSTOMIZATION_V6
// InternetGatewayDevice.LANDevice.i.X_TT_LANIPv6HostConfigManagement
zcfgRet_t lanV6HostCfgMgmtGet(char *tr98Path, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	struct json_object *ttLanV6HostCfgMgmtObj = NULL, *paramObj;
	objIndex_t objIid;
	zcfgRet_t rst = ZCFG_NO_SUCH_OBJECT;
	...
	sscanf(tr98Path, "InternetGatewayDevice.LANDevice.%hhu.X_TTG_LANIPv6HostConfigManagement", &objIid.idx[0]);
#else
	sscanf(tr98Path, "InternetGatewayDevice.LANDevice.%hhu.X_TTNET_LANIPv6HostConfigManagement", &objIid.idx[0]);
#endif
	if((rst = zcfgFeObjJsonGet(RDM_OID_TT_LAN_V6_HOST_CFG_MGMT, &objIid, &ttLanV6HostCfgMgmtObj)) != ZCFG_SUCCESS){
	...
	*tr98Jobj = json_object_new_object();
	while(parmList->name){
			paramObj = json_object_object_get(ttLanV6HostCfgMgmtObj, parmList->name);
			if(paramObj){
					json_object_object_add(*tr98Jobj, parmList->name, JSON_OBJ_COPY(paramObj));
			}
			parmList++;
	...
	json_object_put(ttLanV6HostCfgMgmtObj);

	return ZCFG_SUCCESS;
}

// InternetGatewayDevice.LANDevice.i.X_TT_LANIPv6HostConfigManagement
zcfgRet_t lanV6HostCfgMgmtSet(char *tr98Path, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	struct json_object *ttLanV6HostCfgMgmtObj = NULL, *paramObj;
	objIndex_t objIid;
	const char *tr98SitePrefixConfigType = NULL, *tr181SitePrefixConfigType = NULL;
	...
	tr98SitePrefixConfigType = json_object_get_string(json_object_object_get(tr98Jobj, "SitePrefixConfigType"));
	tr181SitePrefixConfigType = json_object_get_string(json_object_object_get(ttLanV6HostCfgMgmtObj, "SitePrefixConfigType"));
	...
	while(parmList && parmList->name){
			...
			if(!strncmp(parmList->name, "DNSServers", (strlen(parmList->name) >= strlen("DNSServers")) ? strlen(parmList->name) : strlen("DNSServers")))
			{
				char *dnssvr = json_object_get_string(paramObj);
				if(dnssvr && !strstr(dnssvr, ":")){
					rst = ZCFG_INVALID_PARAM_VALUE;

					goto out;
				}
				json_object_object_add(ttLanV6HostCfgMgmtObj, parmList->name, json_object_new_string(dnssvr));
			}
			else if(!strncmp(parmList->name, "ULAPrefix", (strlen(parmList->name) >= strlen("ULAPrefix")) ? strlen(parmList->name) : strlen("ULAPrefix")))
			{
				char *ulaprefix = json_object_get_string(paramObj);
				if(ulaprefix && !strstr(ulaprefix, ":")){
					rst = ZCFG_INVALID_PARAM_VALUE;

					goto out;
				}
				json_object_object_add(ttLanV6HostCfgMgmtObj, parmList->name, json_object_new_string(ulaprefix));
			}
			else if(!strncmp(parmList->name, "DNSServers", (strlen(parmList->name) >= strlen("DNSServers")) ? strlen(parmList->name) : strlen("DNSServers")))
			{
				char *dnssvr = json_object_get_string(paramObj);
				if(dnssvr && !strstr(dnssvr, ":")){
					rst = ZCFG_INVALID_PARAM_VALUE;
			...
			else if(!strncmp(parmList->name, "PrefixDelegationWANConnection", (strlen(parmList->name) >= strlen("PrefixDelegationWANConnection")) ? strlen(parmList->name) : strlen("PrefixDelegationWANConnection")))
			{
				char prefixDelegationWAN[130] = {0}, ipifacePath[MAX_TR181_PATHNAME] = {0};
				strncpy(prefixDelegationWAN, json_object_get_string(paramObj), parmList->len);
				if(zcfgFe98To181IpIface(prefixDelegationWAN, ipifacePath) == ZCFG_SUCCESS){
					json_object_object_add(ttLanV6HostCfgMgmtObj, parmList->name, json_object_new_string(ipifacePath));
				}
			}
			else if(!strncmp(parmList->name, "SitePrefixConfigType", (strlen(parmList->name) >= strlen("SitePrefixConfigType")) ? strlen(parmList->name) : strlen("SitePrefixConfigType")))
			{
				char *prefixConfigType = json_object_get_string(paramObj);
				if(!strlen(prefixConfigType) || (strncmp(prefixConfigType, "Static", (strlen(prefixConfigType) >= strlen("Static")) ? strlen(prefixConfigType) : strlen("Static")) && strncmp(prefixConfigType, "Auto", (strlen(prefixConfigType) >= strlen("Auto")) ? strlen(prefixConfigType) : strlen("Auto"))))
				{
					rst = ZCFG_INVALID_PARAM_VALUE;

					goto out;
				}
				json_object_object_add(ttLanV6HostCfgMgmtObj, parmList->name, json_object_new_string(prefixConfigType));
			}
			else if(!strncmp(parmList->name, "SitePrefix", (strlen(parmList->name) >= strlen("SitePrefix")) ? strlen(parmList->name) : strlen("SitePrefix")))
			{
				char *prefix = json_object_get_string(paramObj);
				if(prefix && !strstr(prefix, ":")){
					// might require valid ipv6 prefix correctness
					rst = ZCFG_INVALID_PARAM_VALUE;

					goto out;
				}

				if(((!tr98SitePrefixConfigType && strncmp(tr181SitePrefixConfigType, "Static", (strlen(tr181SitePrefixConfigType) >= strlen("Static")) ? strlen(tr181SitePrefixConfigType) : strlen("Static"))) ||
						(tr98SitePrefixConfigType && strncmp(tr98SitePrefixConfigType, "Static", (strlen(tr98SitePrefixConfigType) >= strlen("Static")) ? strlen(tr98SitePrefixConfigType) : strlen("Static")))) && strlen(prefix))
				{
					rst = ZCFG_REQUEST_REJECT;

					goto out;
				}
				json_object_object_add(ttLanV6HostCfgMgmtObj, parmList->name, json_object_new_string(prefix));
			}else
				json_object_object_add(ttLanV6HostCfgMgmtObj, parmList->name, JSON_OBJ_COPY(paramObj));

			parmList++;
	}

	if(multiJobj){
			zcfgFeJsonMultiObjAppend(RDM_OID_TT_LAN_V6_HOST_CFG_MGMT, &objIid, multiJobj, ttLanV6HostCfgMgmtObj);
	}else{
			zcfgRet_t rst = zcfgFeObjJsonSet(RDM_OID_TT_LAN_V6_HOST_CFG_MGMT, &objIid, ttLanV6HostCfgMgmtObj, NULL);
			json_object_put(ttLanV6HostCfgMgmtObj);
