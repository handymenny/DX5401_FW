	...
	while(1) {
		if((fp = fopen(RET_FILE, "r")) != NULL) {
			if(fgets(result, sizeof(result), fp) != NULL ){
				/* #170091, when controller received agent response, controller would create 1905ReturnValue at zyMAP1905_service.c
				 * but file didn't unlink, so next round, even if agent didn't send response, controller would get same result.
				 * Unlink file after getting the result, __ZYXEL__ Danny
				 */
				fclose(fp);
				unlink(RET_FILE);
				break;
