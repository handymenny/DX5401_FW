...
extern char FeActiveV4GW[];
extern char FeActiveV6GW[];
#endif
		...
		if(strstr(dslChannelObj->LinkEncapsulationUsed, "ATM") != NULL) {
...
#ifdef CONFIG_TT_CUSTOMIZATION
				strcpy(atmLinkObj->LinkType, "EoA");
				strcpy(atmLinkObj->Encapsulation, "LLC");
#endif
			...
			*idx = objIid.idx[0];
			...
			if((ret = zcfgFeObjStructGet(RDM_OID_ETH_LINK, &objIid, (void **)&ethLinkObj)) == ZCFG_SUCCESS) {
				...
				strcpy(ethLinkObj->LowerLayers, "PTM.Link.1");
...
void  getDSLLineModulationType(char *tr98FullPathName,bool updateFlag, char *ModulationType){
		...
		strcpy(tr98TmpLineName, tr98FullPathName);
		...
		*ptr = '\0';
		...
		if((ret = zcfgFe98To181MappingNameGet(tr98TmpLineName, dslChannelObj)) == ZCFG_SUCCESS) {
			...
			if((ret = feObjJsonGet(dslLineOid, &dslLineIid, &dslLine, updateFlag)) == ZCFG_SUCCESS){
				...
				strcpy(ModulationType,json_object_get_string(json_object_object_get(dslLine, "X_ZYXEL_ModulationType")));
				json_object_put(dslLine);
			}
			json_object_put(dslChannel);
...
void  getPTMLinkStatus(char *tr98FullPathName,bool updateFlag, char *LinkStatus){
		...
		*ptr = '\0';
		...
		if((ret = zcfgFe98To181MappingNameGet(tr98TmpName, ptmLink)) == ZCFG_SUCCESS) {
#ifdef CONFIG_TT_CUSTOMIZATION_TR069 //InternetGatewayDevice.WANDevice.i.WANConnectionDevice.i mapping to eth link
				...
				strcpy(ptmLink, json_object_get_string(json_object_object_get(ethLinkJobj, "LowerLayers")));
			...
			strcpy(LinkStatus, json_object_get_string(json_object_object_get(ptmLinkJobj, "Status")));
...
#ifdef CONFIG_TT_CUSTOMIZATION //InternetGatewayDevice.WANDevice.i.WANConnectionDevice.i mapping to eth link
	struct json_object *ethLinkJobj = NULL;
	objIndex_t ethLinkIid;
	IID_INIT(ethLinkIid);
	sscanf(ptmLink, "Ethernet.Link.%hhu", &ethLinkIid.idx[0]);
...
zcfgRet_t WANIpConnObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	...
	char ttV6ParmName[40] = {0};
	struct json_object *ipIfaceTtV6Obj = NULL;
#endif
#if 0
	int i;
	char pmPath[256] = {0};
...
#ifdef TT_CUSTOMIZATION_V6
	if(feObjJsonGet(RDM_OID_IP_IFACE_TT_V6, &ipIfaceIid, &ipIfaceTtV6Obj, updateFlag) != ZCFG_SUCCESS)
		goto ipConnFree;
#endif
	...
	*tr98Jobj = json_object_new_object();
	...
	while(paramList->name != NULL) {
...
#ifdef TT_HGW
		else if(!strcmp(paramList->name, "X_TTNET_802-1p")) {
					if (vlanTermJobj){
		...
		else if(!strcmp(paramList->name, "X_TTNET_VLANID")) {
					if (vlanTermJobj){
...
#ifdef TT_CUSTOMIZATION_V6
		memset(ttV6ParmName, 0, sizeof(ttV6ParmName));
#ifndef TT_CUSTOMIZATION_V6_TTNET
		if(sscanf(paramList->name, "X_TTG_%s", ttV6ParmName) && (paramValue = json_object_object_get(ipIfaceTtV6Obj, ttV6ParmName))) {
#else
		if(sscanf(paramList->name, "X_TTNET_%s", ttV6ParmName) && (paramValue = json_object_object_get(ipIfaceTtV6Obj, ttV6ParmName))) {
#endif
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
...
#ifdef TT_CUSTOMIZATION_V6
	json_object_put(ipIfaceTtV6Obj);
#endif
...
#ifdef TT_CUSTOMIZATION_V6
	if(zcfgFeObjJsonGetWithoutUpdate(RDM_OID_IP_IFACE_TT_V6, &ipIfaceIid, &ipIfaceTtV6Obj) != ZCFG_SUCCESS) {
		json_object_put(ipIfaceJobj);
		json_object_put(vlanTermJobj);
		json_object_put(natJobj);
		json_object_put(natSessionJobj);
...
#ifdef TT_CUSTOMIZATION_V6
		if(ipIfaceTtV6Obj){
			tmpObj = ipIfaceTtV6Obj;
			ipIfaceTtV6Obj = NULL;
			ipIfaceTtV6Obj = zcfgFeJsonMultiObjAppend(RDM_OID_IP_IFACE_TT_V6, &ipIfaceIid, multiJobj, tmpObj);
			json_object_put(tmpObj);
			zcfgFeJsonMultiObjAddParam(multiJobj, RDM_OID_IP_IFACE_TT_V6, &ipIfaceIid, "apply", json_object_new_boolean(true));
		}
#endif
	...
	while(paramList->name != NULL) {
		...
		if(paramValue != NULL) {
			...
			if(!strcmp(paramList->name, "Name")){
				...
				tr181ParamValue = json_object_object_get(ipIfaceJobj, "X_TTNET_802_1p");
			}
			else {
			...
			if(tr181ParamValue != NULL) {
...
#if 1 //multiObj RDM_OID_ROUTING_ROUTER issue
					const char *activeDefaultGt = NULL;
					if(strlen(FeActiveV4GW))
						activeDefaultGt = FeActiveV4GW;
					else
						activeDefaultGt = json_object_get_string(json_object_object_get(rtJobj, "X_ZYXEL_ActiveDefaultGateway"));
#else
...
#if 1 //multiObj RDM_OID_ROUTING_ROUTER issue
					else if(strlen(FeActiveV4GW)){
						json_object_object_add(rtJobj, "X_ZYXEL_ActiveDefaultGateway", json_object_new_string(FeActiveV4GW));
					}
#endif
				}
				...
				if(!strcmp(paramList->name, "Name")){
					...
					if(vlanTermJobj){
						json_object_object_add(ipIfaceJobj, "X_TTNET_802_1p", JSON_OBJ_COPY(paramValue));
						json_object_object_add(vlanTermJobj, "X_ZYXEL_VLANPriority", JSON_OBJ_COPY(paramValue));
					}
				}
				else if(!strcmp(paramList->name, "X_TTNET_VLANID")){
					if(vlanTermJobj){
						json_object_object_add(ipIfaceJobj, "X_TTNET_VLANID", JSON_OBJ_COPY(paramValue));
					...
					unsigned int X_TTNET_Bootp_Flag = 0;
					X_TTNET_Bootp_Flag = json_object_get_int(paramValue);
					if(X_TTNET_Bootp_Flag < 0 || X_TTNET_Bootp_Flag > 1){
...
#ifdef TT_HGW
					if(ipv4setflag1 == true)
						json_object_object_add(vlanTermJobj, "X_ZYXEL_VLANPriority", json_object_new_int(-1));
					else	
						json_object_object_add(vlanTermJobj, "X_ZYXEL_VLANPriority", JSON_OBJ_COPY(paramValue));
#endif
...
#ifdef TT_HGW
					if(ipv4setflag1 == true)
						json_object_object_add(vlanTermJobj, "VLANID", json_object_new_int(-1));
					else
#endif
						json_object_object_add(vlanTermJobj, "VLANID", JSON_OBJ_COPY(paramValue));
					paramList++;
...
#ifdef TT_CUSTOMIZATION_V6
#ifndef TT_CUSTOMIZATION_V6_TTNET
			else if(sscanf(paramList->name, "X_TTG_%s", ttV6ParmName))
#else
			else if(sscanf(paramList->name, "X_TTNET_%s", ttV6ParmName))
#endif
			{
				struct json_object *ttV6ParamValue = NULL;
				if(!strncmp(ttV6ParmName, "IPv6_AddressingType", sizeof(ttV6ParmName)))
				{
					const char *addrType = json_object_get_string(paramValue);
					if(!strncmp(addrType, "DHCPv6", strlen(addrType)) || !strncmp(addrType, "SLAAC", strlen(addrType)))
					{
						json_object_object_add(ipIfaceTtV6Obj, ttV6ParmName, json_object_new_string(addrType));
					}
				}
				else if(!strncmp(ttV6ParmName, "IPv6_DNSServers", sizeof(ttV6ParmName)))
				{
					struct json_object *tr98IpV6DNSOverrideAllowedParam = json_object_object_get(tr98SpvObj, "IPv6_DNSOverrideAllowed");
					bool tr98IpV6DNSOverrideAllowed = json_object_get_boolean(tr98IpV6DNSOverrideAllowedParam);
					bool tr181IpV6DNSOverrideAllowed = json_object_get_boolean(json_object_object_get(ipIfaceTtV6Obj, "IPv6_DNSOverrideAllowed"));

					if(tr98IpV6DNSOverrideAllowed || (!tr98IpV6DNSOverrideAllowedParam && tr181IpV6DNSOverrideAllowed))
					{
						json_object_object_add(ipIfaceTtV6Obj, ttV6ParmName, JSON_OBJ_COPY(paramValue));
					}
				}
				else
				{
					ttV6ParamValue = json_object_object_get(ipIfaceTtV6Obj, ttV6ParmName);
					if(zcfgJsonObjectValueCmp(paramValue, ttV6ParamValue, paramList->type)){
						json_object_object_add(ipIfaceTtV6Obj, ttV6ParmName, JSON_OBJ_COPY(paramValue));
					}
				}
				memset(ttV6ParmName, 0, sizeof(ttV6ParmName));
				...
				const char *activeV6DefaultGt = NULL;
				const char *new_activeV6DefaultGt = NULL;
#if 1 //multiObj RDM_OID_ROUTING_ROUTER issue
				if(strlen(FeActiveV6GW))
					activeV6DefaultGt = FeActiveV6GW;
				else
					activeV6DefaultGt = json_object_get_string(json_object_object_get(rtJobj, "X_ZYXEL_ActiveV6DefaultGateway"));
#else
				activeV6DefaultGt = json_object_get_string(json_object_object_get(rtJobj, "X_ZYXEL_ActiveV6DefaultGateway"));
#endif
...
#if 1 //multiObj RDM_OID_ROUTING_ROUTER issue
				else if(strlen(FeActiveV6GW)){
					json_object_object_add(rtJobj, "X_ZYXEL_ActiveV6DefaultGateway", json_object_new_string(FeActiveV6GW));
				}
#endif
			}
			else if(!strcmp(paramList->name, "X_ZYXEL_IPv6Enable")) {
...
#ifdef TT_CUSTOMIZATION_V6
		if(ipIfaceTtV6Obj){
			if((ret = zcfgFeObjJsonSet(RDM_OID_IP_IFACE_TT_V6, &ipIfaceIid, ipIfaceTtV6Obj, NULL)) != ZCFG_SUCCESS) {
				zcfgLog(ZCFG_LOG_ERR, "%s: Set Device.IP.Interface.i TT IPv6 object Fail\n", __FUNCTION__);
			}
			else {
				zcfgLog(ZCFG_LOG_ERR, "%s: Set Device.IP.Interface.i TT IPv6 object Success\n", __FUNCTION__);
			}
			json_object_put(ipIfaceTtV6Obj);
		}
#endif
	...
	*ptr = '\0';
	...
	if(wanIpConnType == WAN_IPCONN_EINTELVLIP) {
		...
		if(strstr(wanIntfObj, "Ethernet") != NULL || strstr(wanIntfObj, "Optical") != NULL){//ETH or PON
			strcpy(ethLinkPathName, wanLinkName);
		}
...
*/
	...
	while(paramList->name != NULL) {
		...
		if(!strcmp(paramList->name, "ExternalIPAddress")){
			...
			attrValue = zcfgFeParamAttrGetByName(RDM_OID_IP_IFACE, "X_TTNET_802-1p");
		}
		else if(!strcmp(paramList->name, "X_TTNET_VLANID")){
			attrValue = zcfgFeParamAttrGetByName(RDM_OID_IP_IFACE, "X_TTNET_VLANID");
		}
#endif		
#ifndef MSTC_TTNET_MULTI_USER
		else if(!strcmp(paramList->name, "X_ZYXEL_DMZHost_Enable") || !strcmp(paramList->name, "X_ZYXEL_DMZHost") || !strcmp(paramList->name, "X_ZYXEL_NAT_ALG") || 
...
zcfgRet_t WANIpConnObjAttrSet(char *tr98FullPathName, int handler, char *paramName, int newNotify, struct json_object *multiAttrJobj)
{
	...
	if(paramNamePtr) {
		...
		if(!strcmp(paramList->name, "ExternalIPAddress")){
			...
			rdmOid = RDM_OID_IP_IFACE;
			strncpy(attrParamName, "X_TTNET_802-1p", sizeof(attrParamName)-1);
		}
		else if(!strcmp(paramList->name, "X_TTNET_VLANID")){
			rdmOid = RDM_OID_IP_IFACE;
			strncpy(attrParamName, "X_TTNET_VLANID", sizeof(attrParamName)-1);
		}
#endif		
...
zcfgRet_t WANPppConnObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	...
	if(zcfgFeObjJsonGetWithoutUpdate(RDM_OID_IP_IFACE_TT_V6, &ipIfaceIid, &ipIfaceTtV6Obj) != ZCFG_SUCCESS) {
...
#ifdef CONFIG_TT_CUSTOMIZATION_TR069
		{
			if(json_object_object_get(ipIfaceJobj, "X_ZYXEL_IfName") != NULL){
				strcpy(X_ZYXEL_IfName, json_object_get_string(json_object_object_get(ipIfaceJobj, "X_ZYXEL_IfName")));
			}
	...
	while(paramList->name != NULL) {
		...
		if(paramValue != NULL) {
...
#ifdef TT_HGW
			if(!strcmp(paramList->name, "X_TTNET_802-1p")){
							if(vlanTermJobj){
			...
			if(!strcmp(paramList->name, "X_TTNET_VLANID")){
							if(vlanTermJobj){
...
#endif
				json_object_object_add(pppIfaceJobj, paramList->name, json_object_new_boolean(tr98SetDefaultIface));
				paramList++;
				continue;
			}
...
#endif
				json_object_object_add(pppIfaceJobj, paramList->name, json_object_new_boolean(tr98SetV6DefaultIface));
				paramList++;
				continue;
			}
...
#endif
			{
				...
				{
					const char *addrType = json_object_get_string(paramValue);
					if(!strncmp(addrType, "DHCPv6", (strlen(addrType) >= strlen("DHCPv6")) ? strlen(addrType) : strlen("DHCPv6")) || !strncmp(addrType, "SLAAC", (strlen(addrType) >= strlen("SLAAC")) ? strlen(addrType) : strlen("SLAAC")))
					{
						json_object_object_add(ipIfaceTtV6Obj, ttV6ParmName, json_object_new_string(addrType));
					}
				}
...
#ifdef TT_HGW
        else if(strcmp(paramList->name, "X_TTNET_802-1p") == 0){
			ret = zcfgFeMultiParamAttrAppend(RDM_OID_IP_IFACE, multiAttrJobj, "X_TTNET_802-1p", attrValue);
		}
		else if(strcmp(paramList->name, "X_TTNET_VLANID") == 0){
			ret = zcfgFeMultiParamAttrAppend(RDM_OID_IP_IFACE, multiAttrJobj, "X_TTNET_VLANID", attrValue);
		}
#endif		
		else{
	...
	*tr98Jobj = json_object_new_object();
	...
	while(paramList->name != NULL) {
		...
		if(!strcmp(paramList->name, "PortMappingEnabled")) { 
			...
			paramValue = json_object_object_get(portMapJobj, "RemoteHost");
			if(paramValue != NULL) {
				json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
		...
		else if(!strcmp(paramList->name, "X_ZYXEL_SetOriginatingIP")) {
			paramValue = json_object_object_get(portMapJobj, "X_ZYXEL_SetRemoteIP");
			if(paramValue != NULL) {
				json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
...
zcfgRet_t WANPortMappingSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	...
	sscanf(portMap, "NAT.PortMapping.%hhu", &objIid.idx[0]);
								...
								{
									strcpy(New_SrvName,ipIfaceObj->X_ZYXEL_SrvName);
									strcpy(New_TT_SrvName,ipIfaceObj->X_TT_SrvName);
	...
	while(paramList->name != NULL) {
		...
		if(paramValue != NULL) {
			...
			if(!strcmp(paramList->name, "PortMappingEnabled")) { 
				...
				json_object_object_add(portMapJobj, "RemoteHost", JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
			}
			else if(!strcmp(paramList->name, "X_ZYXEL_SetOriginatingIP")) {
				json_object_object_add(portMapJobj, "X_ZYXEL_SetRemoteIP", JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
			}
		}
	...
	while(paramList->name != NULL) {
		...
		if(paramValue != NULL) {
			...
			if(!strcmp(paramList->name, "PortMappingEnabled")) { 
				...
				json_object_object_add(referportMapJobj, "RemoteHost", JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
			}
			else if(!strcmp(paramList->name, "X_ZYXEL_SetOriginatingIP")) {
				json_object_object_add(referportMapJobj, "X_ZYXEL_SetRemoteIP", JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
			}
		}
						...
						if(strstr(New_SrvName,"_VDSL") != NULL){
								...
								zcfgFeObjStructGetWithoutUpdate(RDM_OID_NAT_PORT_MAPPING, &referobjIid, (void **)&natPortMapObj);
									...
									while(zcfgFeObjStructGetNextWithoutUpdate(RDM_OID_IP_IFACE, &ipIfaceIid, (void **)&ipIfaceObj) == ZCFG_SUCCESS) {
										if(!strcmp(ipIfaceObj->X_TT_SrvName,New_TT_SrvName) && strcmp(ipIfaceObj->X_ZYXEL_SrvName,New_SrvName) && strstr(ipIfaceObj->X_ZYXEL_SrvName,"_ADSL"))
										{
												sprintf(newreferinterface, "IP.Interface.%u", ipIfaceIid.idx[0]);
												strcpy(natPortMapObj->Interface, newreferinterface);
												natPortMapObj->X_ZYXEL_REFERENCE = -1;
												zcfgFeObjStructFree(ipIfaceObj);
												break;
										}
								...
								zcfgFeObjStructGetWithoutUpdate(RDM_OID_NAT_PORT_MAPPING, &referobjIid, (void **)&natPortMapObj);
									...
									IID_INIT(ipIfaceIid);
									while(zcfgFeObjStructGetNextWithoutUpdate(RDM_OID_IP_IFACE, &ipIfaceIid, (void **)&ipIfaceObj) == ZCFG_SUCCESS) {
										if(!strcmp(ipIfaceObj->X_TT_SrvName,New_TT_SrvName) && strcmp(ipIfaceObj->X_ZYXEL_SrvName,New_SrvName) && strstr(ipIfaceObj->X_ZYXEL_SrvName,"_VDSL"))
										{
												sprintf(newreferinterface, "IP.Interface.%u", ipIfaceIid.idx[0]);
												strcpy(natPortMapObj->Interface, newreferinterface);
												natPortMapObj->X_ZYXEL_REFERENCE = newreferindex;
...
int WANPortMappingAttrGet(char *tr98FullPathName, int handler, char *paramName)
{
	...
	while(paramList->name != NULL) {
		...
		if(strcmp(paramList->name, "PortMappingEnabled") == 0){
			...
			attrValue = zcfgFeParamAttrGetByName(devInfoOid, "RemoteHost");
		}else if(strcmp(paramList->name, "X_ZYXEL_SetOriginatingIP") == 0){
			attrValue = zcfgFeParamAttrGetByName(devInfoOid, "X_ZYXEL_SetRemoteIP");
		}else{
			attrValue = zcfgFeParamAttrGetByName(devInfoOid, paramList->name);
		}
...
zcfgRet_t WANPortMappingAttrSet(char *tr98FullPathName, int handler, char *paramName, int newNotify, struct json_object *multiAttrJobj)
{
	...
	while(paramList->name != NULL) {
		...
		if(strcmp(paramList->name, "PortMappingEnabled") == 0){
			...
			ret = zcfgFeMultiParamAttrAppend(devInfoOid, multiAttrJobj, "RemoteHost", attrValue);
		}else if(strcmp(paramList->name, "X_ZYXEL_SetOriginatingIP") == 0){
			ret = zcfgFeMultiParamAttrAppend(devInfoOid, multiAttrJobj, "X_ZYXEL_SetRemoteIP", attrValue);
		}else{
			ret = zcfgFeMultiParamAttrAppend(devInfoOid, multiAttrJobj, paramList->name, attrValue);
		}
		if( ret != ZCFG_SUCCESS){
...
/*  
 *   TR98 Object Name : InternetGatewayDevice.X_CUSTOM_ActiveWANDevice.i
 *
 *   Related object in TR181:
 *   Device.IP.Interface.i
 *   Device.IP.Interface.i.IPv4Address.i
...
zcfgRet_t RUNNINGWANConnObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	...
	sscanf(tr98FullPathName, "InternetGatewayDevice.X_CUSTOM_ActiveWANDevice.%d", &runningwanInstance);
	IID_INIT(ipIfaceIid);
...
#endif //ACEN_CUSTOMIZATION //hardcode for ACEN_CUSTOMIZATION
				snprintf(tr98FullPathName, TR98_MAX_OBJ_NAME_LENGTH, "InternetGatewayDevice.X_CUSTOM_ActiveWANDevice.%d", runningwanInstance);
			}
		}	
