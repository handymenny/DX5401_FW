...
#endif
{
    ...
    for(addindex = 0; addindex < MAX_ACS_IPIFACE; addindex++){ 
		...
		strcpy(TTSrvName,SrvName);
		if(wantype == 1){
			strcat(SrvName,"_ETHWAN");
			strcat(TTSrvName,"_WAN");
		}
		else if(wantype == 2){
			strcat(SrvName,"_ADSL");
			strcat(TTSrvName,"_DSL");
		}
		else if(wantype == 3){
