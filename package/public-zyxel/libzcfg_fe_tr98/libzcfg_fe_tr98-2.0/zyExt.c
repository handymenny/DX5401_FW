 ...
 *   Device.X_CUSTOM_Gpon_Info >> ACEN_CUSTOMIZATION
 */
...
#endif
#if defined(ISKON_CUSTOMIZATION) || defined(ZYXEL_SP_DOMAIN_TIMER) || defined(GFIBER_CUSTOMIZATION)
	...
	*tr98Jobj = json_object_new_object();
	...
	while(paramList->name != NULL){
		if(!strcmp(paramList->name, "ConfigFile")) {
			...
			if (fp == NULL){
				strncpy(configfile, "NONE", sizeof(configfile));
			}
			else{
                ...
                const uint8_t *s = tmpstr;
                int len = 2*bytes;

				for(i = 0; i < len; i+=2){
                   configfile[i] = nibble_to_hex_char(*s >> 4);
				   configfile[i+1] = nibble_to_hex_char(*s++ & 0xF);
				}
				configfile[i] = '\0';
			}
			fclose(fp);
			...
			json_object_object_add(*tr98Jobj, paramList->name, json_object_new_string(configfile));
			paramList++;
			continue;
		}
		else
	...
	while(paramList->name != NULL){
		if(!strcmp(paramList->name, "ConfigFile")) {
			...
			if(paramValue != NULL){
				...
				{
				    ZOS_SYSTEM("rm -rf /tmp/zcfg_config.tgz");
...
/* InternetGatewayDevice.X_CUSTOM_WLANConfiguration.i  for ACEN */
...
zcfgRet_t zyExtWLANConfigurationObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	...
	IID_INIT(apIid);
	apIid.level= 1;
	sscanf(tr98FullPathName, "InternetGatewayDevice.X_CUSTOM_WLANConfiguration.%hhu", &apIid.idx[0]);
	...
	*tr98Jobj = json_object_new_object();
	...
	if(paramValue != NULL) {
		if(apIid.idx[0] <= 4)
			json_object_object_add(*tr98Jobj, "X_CUSTOM_PSK24", JSON_OBJ_COPY(paramValue));
		else
			json_object_object_add(*tr98Jobj, "X_CUSTOM_PSK5", JSON_OBJ_COPY(paramValue));
	}else{
...
/* InternetGatewayDevice.X_CUSTOM_AirtimeFairness for ACEN*/
...
zcfgRet_t zyExtAccountResetPasswordObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	...
	*tr98Jobj = json_object_new_object();
	...
	while(paramList->name != NULL) {
		...
		if (!strcmp(paramList->name, "X_CUSTOM_ResetDefaultPassword")){
			paramValue = json_object_object_get(loginCfgGpAccoutObj, "ResetDefaultPassword");
				json_object_object_add(*tr98Jobj, "X_CUSTOM_ResetDefaultPassword", JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
			}
...
/*
 *   TR98 Object Name : InternetGatewayDevice.X_ZYXEL_EXT.TWAMPReflector
 *
 *   Related object in TR181: 
 *   Device.X_ZYXEL_EXT.X_ZYXEL_TWAMPReflector
 */
zcfgRet_t zyExtTwampdObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *twampJobj = NULL;
	struct json_object *paramValue = NULL;
	tr98Parameter_t *paramList = NULL;
	...
	/* Get related tr181 objects */
	IID_INIT(objIid);
	if ((ret = feObjJsonGet(RDM_OID_ZY_TWAMP_REFLECTOR, &objIid, &twampJobj, updateFlag)) != ZCFG_SUCCESS)
		return ret;
	...
	*tr98Jobj = json_object_new_object();
	...
	while (paramList->name != NULL) {
		/* Write parameter value from tr181 objects to tr98 object */
		paramValue = json_object_object_get(twampJobj, paramList->name);
		if (paramValue != NULL) {
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
 ...
 *   Device.X_ZYXEL_EXT.X_ZYXEL_TWAMPReflector
 */
zcfgRet_t zyExtTwampdObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *twampJobj = NULL;
	...
	/* Get related tr181 objects */
	IID_INIT(objIid);
	if ((ret = zcfgFeObjJsonGet(RDM_OID_ZY_TWAMP_REFLECTOR, &objIid, &twampJobj)) != ZCFG_SUCCESS)
		return ret;
	
	if (multiJobj) {
		tmpObj = twampJobj;
		twampJobj = NULL;
		twampJobj = zcfgFeJsonMultiObjAppend(RDM_OID_ZY_TWAMP_REFLECTOR, &objIid, multiJobj, tmpObj);
	}
	...
	while (paramList->name != NULL) {
		...
		paramValue = json_object_object_get(tr98Jobj, paramList->name);
		if (paramValue != NULL) {
			tr181ParamValue = json_object_object_get(twampJobj, paramList->name);
			if (tr181ParamValue != NULL) {
				json_object_object_add(twampJobj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
			}
		}
	...
	if (multiJobj)
		...
		if ((ret = zcfgFeObjJsonSet(RDM_OID_ZY_TWAMP_REFLECTOR, &objIid, twampJobj, NULL)) != ZCFG_SUCCESS ) {
			json_object_put(twampJobj);
			return ret;
		}
		json_object_put(twampJobj);
...
#endif /*#ifdef CONFIG_ZCFG_BE_MODULE_TWAMP*/
		...
		if(paramValue != NULL){
			...
			if(!strlen(IGMPInterfaceList))
				...
				while((ret = zcfgFeObjStructGetNext(RDM_OID_IP_IFACE, &ipIfaceIid, (void **)&ipIface)) == ZCFG_SUCCESS) {
					...
					if(strstr(ipIface->X_TT_SrvName, "IPTV_") != NULL){
...
void getADSLIPInterface(char *MatchInterface, char *NTPInterface){
	...
	if(zcfgFeObjStructGetWithoutUpdate(RDM_OID_IP_IFACE, &matchIpifaceIid, (void **)&matchIpiface) == ZCFG_SUCCESS){
		if(!strstr(matchIpiface->X_TT_SrvName,"_DSL") || strstr(matchIpiface->X_ZYXEL_SrvName,"_ADSL") ){
	...
	while(zcfgFeObjStructGetNextWithoutUpdate(RDM_OID_IP_IFACE, &ipifaceIid, (void **)&ipiface) == ZCFG_SUCCESS){
		if(!strcmp(matchIpiface->X_TT_SrvName,ipiface->X_TT_SrvName) && strstr(ipiface->X_ZYXEL_SrvName,"_ADSL")){
...
void getVDSLIPInterface(char *MatchInterface, char *NTPInterface){
	...
	sscanf(MatchInterface, "IP.Interface.%hhu", &matchIpifaceIid.idx[0]);
	if(zcfgFeObjStructGetWithoutUpdate(RDM_OID_IP_IFACE, &matchIpifaceIid, (void **)&matchIpiface) == ZCFG_SUCCESS){
		if(!strstr(matchIpiface->X_TT_SrvName,"_DSL") || strstr(matchIpiface->X_ZYXEL_SrvName,"_VDSL") ){
			strcpy(NTPInterface,MatchInterface);
			zcfgFeObjStructFree(matchIpiface);
			return ;
		}
	...
	IID_INIT(ipifaceIid);
	while(zcfgFeObjStructGetNextWithoutUpdate(RDM_OID_IP_IFACE, &ipifaceIid, (void **)&ipiface) == ZCFG_SUCCESS){
		if(!strcmp(matchIpiface->X_TT_SrvName,ipiface->X_TT_SrvName) && strstr(ipiface->X_ZYXEL_SrvName,"_VDSL")){
			sprintf(NTPInterface,"IP.Interface.%d",ipifaceIid.idx[0]);
			zcfgFeObjStructFree(ipiface);
			return;
		}	
...
/* InternetGatewayDevice.X_ZYXEL_Mesh for ACEN_CUSTOMIZATION*/ 
...
/* InternetGatewayDevice.X_ZYXEL_EXT.EasyMeshTs */
zcfgRet_t zyExtEasyMeshTsObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *EasyMeshTsObj = NULL;
	struct json_object *paramValue = NULL;
	tr98Parameter_t *paramList = NULL;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = feObjJsonGet(RDM_OID_EASY_MESH_TS, &objIid, &EasyMeshTsObj, updateFlag)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	*tr98Jobj = json_object_new_object();
	...
	while(paramList->name != NULL){
		/*Write parameter value from tr181 objects to tr98 object*/
		paramValue = json_object_object_get(EasyMeshTsObj, paramList->name);
		if(paramValue != NULL){
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
	...
	json_object_put(EasyMeshTsObj);

	return ZCFG_SUCCESS;
}

/* InternetGatewayDevice.X_ZYXEL_EXT.EasyMeshTs */
zcfgRet_t zyExtEasyMeshTsObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *EasyMeshTsObj = NULL;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = zcfgFeObjJsonGet(RDM_OID_EASY_MESH_TS, &objIid, &EasyMeshTsObj)) != ZCFG_SUCCESS){
		return ret;
	}

	if(multiJobj){
		tmpObj = EasyMeshTsObj;
		EasyMeshTsObj = NULL;
		EasyMeshTsObj = zcfgFeJsonMultiObjAppend(RDM_OID_EASY_MESH_TS, &objIid, multiJobj, tmpObj);
	}
	...
	while(paramList->name != NULL){
		...
		paramValue = json_object_object_get(tr98Jobj, paramList->name);
		if(paramValue != NULL){
			tr181ParamValue = json_object_object_get(EasyMeshTsObj, paramList->name);
			if(tr181ParamValue != NULL){
				json_object_object_add(EasyMeshTsObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
			}
		}
	...
	if(multiJobj){
		...
		if((ret = zcfgFeObjJsonSet(RDM_OID_EASY_MESH_TS, &objIid, EasyMeshTsObj, NULL)) != ZCFG_SUCCESS ){
			json_object_put(EasyMeshTsObj);
			return ret;
		}
		json_object_put(EasyMeshTsObj);
...
// InternetGatewayDevice.X_TT.Configuration.IPv6Lan
zcfgRet_t ttcfgIpV6LanQuery(char *tr98Path, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	struct json_object *ttIpV6LanObj = NULL, *paramObj;
	objIndex_t objIid;
	zcfgRet_t rst = ZCFG_NO_SUCH_OBJECT;


	IID_INIT(objIid);
	if((rst = zcfgFeObjJsonGet(RDM_OID_IP_V6_LAN, &objIid, &ttIpV6LanObj)) != ZCFG_SUCCESS){
	...
	*tr98Jobj = json_object_new_object();
	while(parmList->name){
			paramObj = json_object_object_get(ttIpV6LanObj, parmList->name);
			if(paramObj){
					json_object_object_add(*tr98Jobj, parmList->name, JSON_OBJ_COPY(paramObj));
			}
			parmList++;
	...
	json_object_put(ttIpV6LanObj);

	return ZCFG_SUCCESS;
}

// InternetGatewayDevice.X_TT.Configuration.IPv6Lan
zcfgRet_t ttcfgIpV6LanSet(char *tr98Path, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	struct json_object *ttIpV6LanObj = NULL, *paramObj;
	objIndex_t objIid;
	struct tr98Parameter_s *parmList = NULL;
	...
	while(parmList && parmList->name){
			...
			json_object_object_add(ttIpV6LanObj, parmList->name, JSON_OBJ_COPY(paramObj));

			parmList++;
	}

	if(multiJobj){
			zcfgFeJsonMultiObjAppend(RDM_OID_IP_V6_LAN, &objIid, multiJobj, ttIpV6LanObj);
	}else{
			zcfgRet_t rst = zcfgFeObjJsonSet(RDM_OID_IP_V6_LAN, &objIid, ttIpV6LanObj, NULL);
			json_object_put(ttIpV6LanObj);
...
#ifdef CONFIG_ZCFG_BE_MODULE_ZYXEL_ONESSID
//ACEN_CUSTOMIZATION
/* InternetGatewayDevice.X_CUSTOM_ONESSID */
...
#endif //ifndef ACEN_CUSTOMIZATION
...
zcfgRet_t zyTTUsersObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = feObjJsonGet(RDM_OID_TT_USERS, &objIid, &UsersObj, updateFlag)) != ZCFG_SUCCESS){
		return ret;
	}
...
zcfgRet_t zyTTUserObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
    ...
    if((ret = feObjJsonGet(RDM_OID_TT_USER, &objIid, &UserObj, updateFlag)) != ZCFG_SUCCESS)
		return ret;
...
zcfgRet_t zyTTUserObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	...
	if((ret = zcfgFeObjJsonGet(RDM_OID_TT_USER, &objIid, &UserObj)) != ZCFG_SUCCESS)
		return ret;

	if(multiJobj){
		tmpObj = UserObj;
		UserObj = NULL;
		UserObj = zcfgFeJsonMultiObjAppend(RDM_OID_TT_USER, &objIid, multiJobj, tmpObj);
	}
	...
	if(multiJobj){
		...
		if((ret = zcfgFeObjJsonSet(RDM_OID_TT_USER, &objIid, UserObj, NULL)) != ZCFG_SUCCESS ){
			json_object_put(UserObj);
			return ret;
		}
		json_object_put(UserObj);
...
#if 0 //def MSTC_TTNET_MULTI_USER
	IID_INIT(TTUserIid);

	if((TTUserIid.idx[0]==1) || (TTUserIid.idx[0]==2))
		return ZCFG_REQUEST_REJECT;

	if((ret = zcfgFeObjStructAdd(RDM_OID_TT_USER, &TTUserIid, NULL)) != ZCFG_SUCCESS) {
...
//#ifdef TT_CUSTOMIZATION
// InternetGatewayDevice.X_TTNET.Configuration.QuickATABridge
zcfgRet_t ttcfgQuickBridgingQuery(char *tr98Path, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	struct json_object *ttcfgQuickBridgingObj = NULL, *paramObj;
	objIndex_t objIid;
	zcfgRet_t rst = ZCFG_NO_SUCH_OBJECT;

	IID_INIT(objIid);

	if((rst = zcfgFeObjJsonGet(RDM_OID_QUICK_ATA_BRIDGE, &objIid, &ttcfgQuickBridgingObj)) != ZCFG_SUCCESS){
		printf("%s: retrieve RDM_OID_TT_LAN_V6_HOST_CFG_MGMT object fail\n", __FUNCTION__);
	...
	*tr98Jobj = json_object_new_object();
	while(parmList->name){
		paramObj = json_object_object_get(ttcfgQuickBridgingObj, parmList->name);
		if(paramObj){
			json_object_object_add(*tr98Jobj, parmList->name, JSON_OBJ_COPY(paramObj));
		}
		parmList++;
	...
	json_object_put(ttcfgQuickBridgingObj);

	return ZCFG_SUCCESS;
}

// ok: q,n -> n,s
static bool zcfgQuickBridgingLanPortRsv(uint8_t lanPortSn, const char *rdmEnable, void *spvQueryHandle)
{
	bool rsvok = false;
	char pstr[5] = {0};
	char qbridgingPath[100] = {0};

	snprintf(pstr, sizeof(pstr), "%d", lanPortSn);
	if(strstr(rdmEnable, pstr))
		return true;

	rsvok = zcfgBrLanPortQueryExist(1, lanPortSn);
	if(!rsvok){
		return rsvok;
	}

	//struct json_object *zcfgFeHandRoutRetrieveTr98PvsSpv(struct json_object *multiJobj, const char *objpath)
	//zcfgFeHandRoutRetrieveTr98PvsSpv(spvQueryHandle, qbridgingPath);

	return rsvok;
}

static bool validQuickBridgingEnable(const char *spvEnable, const char *rdmEnable, void *spvQueryHandle)
{
	char *token, *tmp = NULL;
	int p;
	bool hasz = 0, valid = true;
	char enableStr[33] = {0};
	int n = 0;

	ZOS_STRNCPY(enableStr, spvEnable, sizeof(enableStr));

	token = strtok_r(enableStr, ",", &tmp);
	int running = token ? 1 : 0;

	while(running) {
		p = atoi(token);
		p = (0 <= p && 5 > p) ? p : -1;
		valid = (p == 0 && hasz) ? false : valid;
		hasz = (p == 0 && !hasz) ? true : hasz;

		valid = (p > 0) ? zcfgQuickBridgingLanPortRsv((uint8_t)p, rdmEnable, spvQueryHandle) : valid;

		token = strtok_r(NULL, ",", &tmp);
		running = (!token || p == -1 || !valid) ? 0 : 1;
		n++;
	}
...
static bool validQuickWanBridgingEnable(const char *spvEnable, const char *rdmEnable)
{
	char *token, *tmp = NULL;
	int p;
	bool hasz = 0, valid = true;
	...
	if((rdmEnable && spvEnable && rdmEnable[0] != '0' && spvEnable[0] != '0'))
		return false;
	...
	while(running) {
		...
		valid = (p > 0) ? zcfgBrLanPortQueryExist(1, (uint8_t)p) : valid;
...
// InternetGatewayDevice.X_TTNET.Configuration.QuickATABridge
zcfgRet_t ttcfgQuickBridgingSet(char *tr98Path, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	struct json_object *ttcfgQuickBridgingObj = NULL, *paramObj;
	objIndex_t objIid;
	zcfgRet_t rst = ZCFG_SUCCESS;
	...
	tr98Jobj = zcfgFeRetrieveSpv(tr98Path);

	if((rst = zcfgFeObjJsonGetWithoutUpdate(RDM_OID_QUICK_ATA_BRIDGE, &objIid, &ttcfgQuickBridgingObj)) != ZCFG_SUCCESS) {
		goto out;
	}

	//ttcfgQuickBridgingObj = json_object_new_object();
	...
	while(parmList && parmList->name){
		...
		if(!strncmp(parmList->name, "Enable", (strlen(parmList->name) >= strlen("Enable")) ? strlen(parmList->name) : strlen("Enable")))
		{
			const char *spvEnable, *rdmEnable;
			rdmEnable = json_object_get_string(json_object_object_get(ttcfgQuickBridgingObj, parmList->name));
			spvEnable = json_object_get_string(paramObj);
			if(!strncmp(rdmEnable, spvEnable, (strlen(rdmEnable) >= strlen(spvEnable)) ? strlen(rdmEnable) : strlen(spvEnable))){
				rst = ZCFG_INVALID_PARAM_VALUE;
				goto out;
			}else if(!validQuickBridgingEnable(spvEnable, rdmEnable, (void *)multiJobj)){
				rst = ZCFG_INVALID_PARAM_VALUE;
				goto out;
			}

			json_object_object_add(ttcfgQuickBridgingObj, parmList->name, JSON_OBJ_COPY(paramObj));
		}
	...
	if(multiJobj){
		zcfgFeJsonMultiObjAppend(RDM_OID_QUICK_ATA_BRIDGE, &objIid, multiJobj, ttcfgQuickBridgingObj);
		//zcfgFeJsonMultiObjAddParam(multiJobj, oid, &objIid, "partialset", json_object_new_boolean(true));
	}else{
		zcfgRet_t rst = zcfgFeObjJsonSet(RDM_OID_QUICK_ATA_BRIDGE, &objIid, ttcfgQuickBridgingObj, NULL);
		json_object_put(ttcfgQuickBridgingObj);
...
zcfgRet_t zyTTFirewallObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = feObjJsonGet(RDM_OID_TT_FIREWALL, &objIid, &FirewallObj, updateFlag)) != ZCFG_SUCCESS){
		return ret;
	}
...
zcfgRet_t zyTTFirewallObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = zcfgFeObjJsonGet(RDM_OID_TT_FIREWALL, &objIid, &FirewallObj)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	if(multiJobj){
		tmpObj = FirewallObj;
		FirewallObj = NULL;
		FirewallObj = zcfgFeJsonMultiObjAppend(RDM_OID_TT_FIREWALL, &objIid, multiJobj, tmpObj);
	}
	...
	if(multiJobj){
		...
		if((ret = zcfgFeObjJsonSet(RDM_OID_TT_FIREWALL, &objIid, FirewallObj, NULL)) != ZCFG_SUCCESS ){
			json_object_put(FirewallObj);
			return ret;
		}
		json_object_put(FirewallObj);
...
zcfgRet_t zyTTUPnPDeviceObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = feObjJsonGet(RDM_OID_TT_DEVICE, &objIid, &UPnPDeviceObj, updateFlag)) != ZCFG_SUCCESS){
		return ret;
	}
...
zcfgRet_t zyTTUPnPDeviceObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = zcfgFeObjJsonGet(RDM_OID_TT_DEVICE, &objIid, &UPnPDeviceObj)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	if(multiJobj){
		...
		UPnPDeviceObj = NULL;
		UPnPDeviceObj = zcfgFeJsonMultiObjAppend(RDM_OID_TT_DEVICE, &objIid, multiJobj, tmpObj);
	}
	...
	if(multiJobj){
		...
		if((ret = zcfgFeObjJsonSet(RDM_OID_TT_DEVICE, &objIid, UPnPDeviceObj, NULL)) != ZCFG_SUCCESS ){
			json_object_put(UPnPDeviceObj);
			return ret;
		}
		json_object_put(UPnPDeviceObj);
...
#else
/* InternetGatewayDevice.X_TT.Configuration.GUI.AccessRights.Page.NetworkSetting.Broadband.Broadband.Internet */
zcfgRet_t zyTTInetObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *InetObj = NULL;
	struct json_object *paramValue = NULL;
	tr98Parameter_t *paramList = NULL;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = feObjJsonGet(RDM_OID_INTERNET, &objIid, &InetObj, updateFlag)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	*tr98Jobj = json_object_new_object();
	...
	while(paramList->name != NULL){
		/*Write parameter value from tr181 objects to tr98 object*/
		paramValue = json_object_object_get(InetObj, paramList->name);
		if(paramValue != NULL){
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
	...
	json_object_put(InetObj);

	return ZCFG_SUCCESS;
}

zcfgRet_t zyTTInetObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *InetObj = NULL;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = zcfgFeObjJsonGet(RDM_OID_INTERNET, &objIid, &InetObj)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	if(multiJobj){
		tmpObj = InetObj;
		InetObj = NULL;
		InetObj = zcfgFeJsonMultiObjAppend(RDM_OID_INTERNET, &objIid, multiJobj, tmpObj);
	}
	...
	while(paramList->name != NULL){
		...
		paramValue = json_object_object_get(tr98Jobj, paramList->name);
		if(paramValue != NULL){
			tr181ParamValue = json_object_object_get(InetObj, paramList->name);
			if(tr181ParamValue != NULL){
				json_object_object_add(InetObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
			}
		}
	...
	if(multiJobj){
		...
		if((ret = zcfgFeObjJsonSet(RDM_OID_INTERNET, &objIid, InetObj, NULL)) != ZCFG_SUCCESS ){
			json_object_put(InetObj);
			return ret;
		}
		json_object_put(InetObj);
...
zcfgRet_t zyTTInetObjNotify(char *tr98ObjName, char *tr181ParamName, struct json_object *tr181ParamVal, int handler, struct json_object **tr98NotifyInfo)
{
	bool found = false;
	char tr98Notify[256] = "";
	tr98Parameter_t *paramList = NULL;
...
int zyTTInetObjAttrGet(char *tr98FullPathName, int handler, char *paramName)
{
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
	uint32_t InetOid = 0;
	...
	IID_INIT(InetIid);
	snprintf(tr181Obj, sizeof(tr181Obj),"Device.%s", mapObj);
	InetOid = zcfgFeObjNameToObjId(tr181Obj, &InetIid);
	...
	while(paramList->name != NULL) {
		...
		attrValue = zcfgFeParamAttrGetByName(InetOid, paramList->name);
		if(attrValue < 0 ) {
			attrValue = 0;
		}
		break;
...
zcfgRet_t zyTTInetObjAttrSet(char *tr98FullPathName, int handler, char *paramName, int newNotify, struct json_object *multiAttrJobj)
{
	zcfgRet_t ret = ZCFG_SUCCESS;
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
	...
	while(paramList->name != NULL) {
		...
		/*Write new parameter attribute from tr98 object to tr181 objects*/
		attrValue = zcfgFeNotifyAttrValSet(attrValue, newNotify);
		ret = zcfgFeMultiParamAttrAppend(InetOid, multiAttrJobj, paramList->name, attrValue);
		if( ret != ZCFG_SUCCESS){
			zcfgLog(ZCFG_LOG_ERR, "%s(): set %d %s attribute fail\n", __FUNCTION__, InetOid, paramList->name);
		}
		break;
	}
...
/* InternetGatewayDevice.X_TT.Configuration.GUI.AccessRights.Page.NetworkSetting.Broadband.Broadband.Management */
zcfgRet_t zyTTMamgObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *MamgObj = NULL;
	struct json_object *paramValue = NULL;
	tr98Parameter_t *paramList = NULL;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = feObjJsonGet(RDM_OID_MANAGEMENT, &objIid, &MamgObj, updateFlag)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	*tr98Jobj = json_object_new_object();
	...
	while(paramList->name != NULL){
		/*Write parameter value from tr181 objects to tr98 object*/
		paramValue = json_object_object_get(MamgObj, paramList->name);
		if(paramValue != NULL){
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
	...
	json_object_put(MamgObj);

	return ZCFG_SUCCESS;
}

zcfgRet_t zyTTMamgObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *MamgObj = NULL;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = zcfgFeObjJsonGet(RDM_OID_MANAGEMENT, &objIid, &MamgObj)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	if(multiJobj){
		tmpObj = MamgObj;
		MamgObj = NULL;
		MamgObj = zcfgFeJsonMultiObjAppend(RDM_OID_MANAGEMENT, &objIid, multiJobj, tmpObj);
	}
	...
	while(paramList->name != NULL){
		...
		paramValue = json_object_object_get(tr98Jobj, paramList->name);
		if(paramValue != NULL){
			tr181ParamValue = json_object_object_get(MamgObj, paramList->name);
			if(tr181ParamValue != NULL){
				json_object_object_add(MamgObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
			}
		}
	...
	if(multiJobj){
		...
		if((ret = zcfgFeObjJsonSet(RDM_OID_MANAGEMENT, &objIid, MamgObj, NULL)) != ZCFG_SUCCESS ){
			json_object_put(MamgObj);
			return ret;
		}
		json_object_put(MamgObj);
...
zcfgRet_t zyTTMamgObjNotify(char *tr98ObjName, char *tr181ParamName, struct json_object *tr181ParamVal, int handler, struct json_object **tr98NotifyInfo)
{
	bool found = false;
	char tr98Notify[256] = "";
	tr98Parameter_t *paramList = NULL;
...
int zyTTMamgObjAttrGet(char *tr98FullPathName, int handler, char *paramName)
{
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
	uint32_t MamgOid = 0;
	...
	IID_INIT(MamgIid);
	snprintf(tr181Obj, sizeof(tr181Obj),"Device.%s", mapObj);
	MamgOid = zcfgFeObjNameToObjId(tr181Obj, &MamgIid);
	...
	while(paramList->name != NULL) {
		...
		attrValue = zcfgFeParamAttrGetByName(MamgOid, paramList->name);
		if(attrValue < 0 ) {
			attrValue = 0;
		}
		break;
...
zcfgRet_t zyTTMamgObjAttrSet(char *tr98FullPathName, int handler, char *paramName, int newNotify, struct json_object *multiAttrJobj)
{
	zcfgRet_t ret = ZCFG_SUCCESS;
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
	...
	while(paramList->name != NULL) {
		...
		/*Write new parameter attribute from tr98 object to tr181 objects*/
		attrValue = zcfgFeNotifyAttrValSet(attrValue, newNotify);
		ret = zcfgFeMultiParamAttrAppend(MamgOid, multiAttrJobj, paramList->name, attrValue);
		if( ret != ZCFG_SUCCESS){
			zcfgLog(ZCFG_LOG_ERR, "%s(): set %d %s attribute fail\n", __FUNCTION__, MamgOid, paramList->name);
		}
		break;
	}
...
/* InternetGatewayDevice.X_TT.Configuration.GUI.AccessRights.Page.NetworkSetting.Broadband.Broadband.VoIP */
zcfgRet_t zyTTVoIPObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *VoIPObj = NULL;
	struct json_object *paramValue = NULL;
	tr98Parameter_t *paramList = NULL;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = feObjJsonGet(RDM_OID_VO_IP, &objIid, &VoIPObj, updateFlag)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	*tr98Jobj = json_object_new_object();
	...
	while(paramList->name != NULL){
		/*Write parameter value from tr181 objects to tr98 object*/
		paramValue = json_object_object_get(VoIPObj, paramList->name);
		if(paramValue != NULL){
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
...
zcfgRet_t zyTTVoIPObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *VoIPObj = NULL;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = zcfgFeObjJsonGet(RDM_OID_VO_IP, &objIid, &VoIPObj)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	if(multiJobj){
		tmpObj = VoIPObj;
		VoIPObj = NULL;
		VoIPObj = zcfgFeJsonMultiObjAppend(RDM_OID_VO_IP, &objIid, multiJobj, tmpObj);
	}
	...
	while(paramList->name != NULL){
		...
		paramValue = json_object_object_get(tr98Jobj, paramList->name);
		if(paramValue != NULL){
			tr181ParamValue = json_object_object_get(VoIPObj, paramList->name);
			if(tr181ParamValue != NULL){
				json_object_object_add(VoIPObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
			}
		}
	...
	if(multiJobj){
		...
		if((ret = zcfgFeObjJsonSet(RDM_OID_VO_IP, &objIid, VoIPObj, NULL)) != ZCFG_SUCCESS ){
			json_object_put(VoIPObj);
			return ret;
		}
		json_object_put(VoIPObj);
...
zcfgRet_t zyTTVoIPObjNotify(char *tr98ObjName, char *tr181ParamName, struct json_object *tr181ParamVal, int handler, struct json_object **tr98NotifyInfo)
{
	bool found = false;
	char tr98Notify[256] = "";
	tr98Parameter_t *paramList = NULL;
...
int zyTTVoIPObjAttrGet(char *tr98FullPathName, int handler, char *paramName)
{
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
	uint32_t VoIPOid = 0;
	...
	IID_INIT(VoIPIid);
	snprintf(tr181Obj, sizeof(tr181Obj),"Device.%s", mapObj);
	VoIPOid = zcfgFeObjNameToObjId(tr181Obj, &VoIPIid);
	...
	while(paramList->name != NULL) {
		...
		attrValue = zcfgFeParamAttrGetByName(VoIPOid, paramList->name);
		if(attrValue < 0 ) {
			attrValue = 0;
		}
		break;
...
zcfgRet_t zyTTVoIPObjAttrSet(char *tr98FullPathName, int handler, char *paramName, int newNotify, struct json_object *multiAttrJobj)
{
	zcfgRet_t ret = ZCFG_SUCCESS;
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
	...
	while(paramList->name != NULL) {
		...
		/*Write new parameter attribute from tr98 object to tr181 objects*/
		attrValue = zcfgFeNotifyAttrValSet(attrValue, newNotify);
		ret = zcfgFeMultiParamAttrAppend(VoIPOid, multiAttrJobj, paramList->name, attrValue);
		if( ret != ZCFG_SUCCESS){
			zcfgLog(ZCFG_LOG_ERR, "%s(): set %d %s attribute fail\n", __FUNCTION__, VoIPOid, paramList->name);
		}
		break;
	}
...
/* InternetGatewayDevice.X_TT.Configuration.GUI.AccessRights.Page.NetworkSetting.Broadband.Broadband.IPTV */
zcfgRet_t zyTTIPTVObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *IPTVObj = NULL;
	struct json_object *paramValue = NULL;
	tr98Parameter_t *paramList = NULL;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = feObjJsonGet(RDM_OID_IPTV, &objIid, &IPTVObj, updateFlag)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	*tr98Jobj = json_object_new_object();
	...
	while(paramList->name != NULL){
		/*Write parameter value from tr181 objects to tr98 object*/
		paramValue = json_object_object_get(IPTVObj, paramList->name);
		if(paramValue != NULL){
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
	...
	json_object_put(IPTVObj);

	return ZCFG_SUCCESS;
}

zcfgRet_t zyTTIPTVObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *IPTVObj = NULL;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = zcfgFeObjJsonGet(RDM_OID_IPTV, &objIid, &IPTVObj)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	if(multiJobj){
		tmpObj = IPTVObj;
		IPTVObj = NULL;
		IPTVObj = zcfgFeJsonMultiObjAppend(RDM_OID_IPTV, &objIid, multiJobj, tmpObj);
	}
	...
	while(paramList->name != NULL){
		...
		paramValue = json_object_object_get(tr98Jobj, paramList->name);
		if(paramValue != NULL){
			tr181ParamValue = json_object_object_get(IPTVObj, paramList->name);
			if(tr181ParamValue != NULL){
				json_object_object_add(IPTVObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
			}
		}
	...
	if(multiJobj){
		...
		if((ret = zcfgFeObjJsonSet(RDM_OID_IPTV, &objIid, IPTVObj, NULL)) != ZCFG_SUCCESS ){
			json_object_put(IPTVObj);
			return ret;
		}
		json_object_put(IPTVObj);
...
zcfgRet_t zyTTIPTVObjNotify(char *tr98ObjName, char *tr181ParamName, struct json_object *tr181ParamVal, int handler, struct json_object **tr98NotifyInfo)
{
	bool found = false;
	char tr98Notify[256] = "";
	tr98Parameter_t *paramList = NULL;
...
int zyTTIPTVObjAttrGet(char *tr98FullPathName, int handler, char *paramName)
{
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
	uint32_t IPTVOid = 0;
	...
	IID_INIT(IPTVIid);
	snprintf(tr181Obj, sizeof(tr181Obj),"Device.%s", mapObj);
	IPTVOid = zcfgFeObjNameToObjId(tr181Obj, &IPTVIid);
	...
	while(paramList->name != NULL) {
		...
		attrValue = zcfgFeParamAttrGetByName(IPTVOid, paramList->name);
		if(attrValue < 0 ) {
			attrValue = 0;
		}
		break;
...
zcfgRet_t zyTTIPTVObjAttrSet(char *tr98FullPathName, int handler, char *paramName, int newNotify, struct json_object *multiAttrJobj)
{
	zcfgRet_t ret = ZCFG_SUCCESS;
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
	...
	while(paramList->name != NULL) {
		...
		/*Write new parameter attribute from tr98 object to tr181 objects*/
		attrValue = zcfgFeNotifyAttrValSet(attrValue, newNotify);
		ret = zcfgFeMultiParamAttrAppend(IPTVOid, multiAttrJobj, paramList->name, attrValue);
		if( ret != ZCFG_SUCCESS){
			zcfgLog(ZCFG_LOG_ERR, "%s(): set %d %s attribute fail\n", __FUNCTION__, IPTVOid, paramList->name);
		}
		break;
	}
...
zcfgRet_t zyTTTR069ObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	...
	struct json_object *tr181ParamValue = NULL;
	tr98Parameter_t *paramList = NULL;
	objIndex_t cwmpobjIid;
	struct json_object *cwmpObj = NULL;
	...
	while(paramList->name != NULL){
		...
		if(paramValue != NULL){
#ifdef TT_HGW			
			if(!strcmp(paramList->name, "VisibilityLevel")) {
               	IID_INIT(cwmpobjIid);
				if(zcfgFeObjJsonGet(RDM_OID_CWMP, &cwmpobjIid, &cwmpObj) == ZCFG_SUCCESS){
					json_object_object_add(cwmpObj, "VisibilityLevel", JSON_OBJ_COPY(paramValue));
					zcfgFeObjJsonSetWithoutApply(RDM_OID_CWMP, &cwmpobjIid, cwmpObj, NULL);
				}
				json_object_object_add(TR069Obj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
			}
			if(!strcmp(paramList->name, "Limitation")) {
               	IID_INIT(cwmpobjIid);
				if(zcfgFeObjJsonGet(RDM_OID_CWMP, &cwmpobjIid, &cwmpObj) == ZCFG_SUCCESS){
					json_object_object_add(cwmpObj, "Limitation", JSON_OBJ_COPY(paramValue));
					zcfgFeObjJsonSetWithoutApply(RDM_OID_CWMP, &cwmpobjIid, cwmpObj, NULL);
				}
				json_object_object_add(TR069Obj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
			...
			if(!strcmp(paramList->name, "Lock")) {
               	IID_INIT(cwmpobjIid);
				if(zcfgFeObjJsonGet(RDM_OID_CWMP, &cwmpobjIid, &cwmpObj) == ZCFG_SUCCESS){
					json_object_object_add(cwmpObj, "Lock", JSON_OBJ_COPY(paramValue));
					zcfgFeObjJsonSetWithoutApply(RDM_OID_CWMP, &cwmpobjIid, cwmpObj, NULL);
				}
				json_object_object_add(TR069Obj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
	...
	while(paramList->name != NULL){
		...
		if(paramValue != NULL){
			...
			if(!strcmp(paramList->name, "VisibilityLevel")) {
				...
				if(zcfgFeObjJsonGet(RDM_OID_TT_PORT_MIRROR, &portmirrorobjIid, &portmirrorObj) == ZCFG_SUCCESS){
					json_object_object_add(portmirrorObj, "VisibilityLevel", JSON_OBJ_COPY(paramValue));
					zcfgFeObjJsonSetWithoutApply(RDM_OID_TT_PORT_MIRROR, &portmirrorobjIid, portmirrorObj, NULL);
				}
			...
			if(!strcmp(paramList->name, "Limitation")) {
               	IID_INIT(portmirrorobjIid);
				if(zcfgFeObjJsonGet(RDM_OID_TT_PORT_MIRROR, &portmirrorobjIid, &portmirrorObj) == ZCFG_SUCCESS){
					json_object_object_add(portmirrorObj, "Limitation", JSON_OBJ_COPY(paramValue));
					zcfgFeObjJsonSetWithoutApply(RDM_OID_TT_PORT_MIRROR, &portmirrorobjIid, portmirrorObj, NULL);
				}
				json_object_object_add(EngdebugObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
			...
			if(!strcmp(paramList->name, "Lock")) {
               	IID_INIT(portmirrorobjIid);
				if(zcfgFeObjJsonGet(RDM_OID_TT_PORT_MIRROR, &portmirrorobjIid, &portmirrorObj) == ZCFG_SUCCESS){
					json_object_object_add(portmirrorObj, "Lock", JSON_OBJ_COPY(paramValue));
					zcfgFeObjJsonSetWithoutApply(RDM_OID_TT_PORT_MIRROR, &portmirrorobjIid, portmirrorObj, NULL);
				}
				json_object_object_add(EngdebugObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
	...
	while(paramList->name != NULL){
		...
		if(paramValue != NULL){
			...
			if(!strcmp(paramList->name, "VisibilityLevel")) {
				...
				if(zcfgFeObjJsonGet(RDM_OID_PACKET_CAPTURE, &packetcaptureobjIid, &packetcaptureObj) == ZCFG_SUCCESS){
					json_object_object_add(packetcaptureObj, "VisibilityLevel", JSON_OBJ_COPY(paramValue));
					zcfgFeObjJsonSetWithoutApply(RDM_OID_PACKET_CAPTURE, &packetcaptureobjIid, packetcaptureObj, NULL);
				}
			...
			if(!strcmp(paramList->name, "Limitation")) {
				IID_INIT(packetcaptureobjIid);
				if(zcfgFeObjJsonGet(RDM_OID_PACKET_CAPTURE, &packetcaptureobjIid, &packetcaptureObj) == ZCFG_SUCCESS){
					json_object_object_add(packetcaptureObj, "Limitation", JSON_OBJ_COPY(paramValue));
					zcfgFeObjJsonSetWithoutApply(RDM_OID_PACKET_CAPTURE, &packetcaptureobjIid, packetcaptureObj, NULL);
				}
				json_object_object_add(CaptureObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
			...
			if(!strcmp(paramList->name, "Lock")) {
				IID_INIT(packetcaptureobjIid);
				if(zcfgFeObjJsonGet(RDM_OID_PACKET_CAPTURE, &packetcaptureobjIid, &packetcaptureObj) == ZCFG_SUCCESS){
					json_object_object_add(packetcaptureObj, "Lock", JSON_OBJ_COPY(paramValue));
					zcfgFeObjJsonSetWithoutApply(RDM_OID_PACKET_CAPTURE, &packetcaptureobjIid, packetcaptureObj, NULL);
				}
				json_object_object_add(CaptureObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
...
/* InternetGatewayDevice.X_TT.Configuration.QuickWANBridg */
zcfgRet_t zyTTQuickWanBridgeObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *obj = NULL;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = feObjJsonGet(RDM_OID_QUICK_WAN_BRIDGE, &objIid, &obj, updateFlag)) != ZCFG_SUCCESS){
		return ret;
	}
...
zcfgRet_t zyTTQuickWanBridgeObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	zcfgRet_t ret = ZCFG_SUCCESS;
	objIndex_t objIid;
	struct json_object *obj = NULL;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = zcfgFeObjJsonGet(RDM_OID_QUICK_WAN_BRIDGE, &objIid, &obj)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	if(multiJobj){
		tmpObj = obj;
		obj = NULL;
		obj = zcfgFeJsonMultiObjAppend(RDM_OID_QUICK_WAN_BRIDGE, &objIid, multiJobj, tmpObj);
	}
	...
	while(paramList->name != NULL){
		...
		if(paramValue != NULL){
			...
			if(tr181ParamValue != NULL){
				const char *spvEnable, *rdmEnable;
				rdmEnable = json_object_get_string(tr181ParamValue);
				spvEnable = json_object_get_string(paramValue);
				if(!validQuickWanBridgingEnable(spvEnable, rdmEnable)){
					ret = ZCFG_INVALID_PARAM_VALUE;
					goto out;
				}
	...
	if(multiJobj){
		...
		if((ret = zcfgFeObjJsonSet(RDM_OID_QUICK_WAN_BRIDGE, &objIid, obj, NULL)) != ZCFG_SUCCESS ){
			json_object_put(obj);
			return ret;
		}
		json_object_put(obj);
...
zcfgRet_t zyTTQuickWanBridgeObjNotify(char *tr98ObjName, char *tr181ParamName, struct json_object *tr181ParamVal, int handler, struct json_object **tr98NotifyInfo)
{
	bool found = false;
	char tr98Notify[256] = "";
	tr98Parameter_t *paramList = NULL;
	...
	paramList = tr98Obj[handler].parameter;
	while(paramList->name != NULL) {
		if(strncmp(tr181ParamName, paramList->name, (strlen(tr181ParamName) >= strlen(paramList->name)) ? strlen(tr181ParamName) : strlen(paramList->name)) == 0) {
			found = true;
			snprintf(tr98Notify, sizeof(tr98Notify), "%s.%s", tr98ObjName, paramList->name);
			break;
		}
...
int zyTTQuickWanBridgeObjAttrGet(char *tr98FullPathName, int handler, char *paramName)
{
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
	uint32_t oid = 0;
	...
	paramList = tr98Obj[handler].parameter;
	while(paramList->name != NULL) {
		if(strncmp(paramList->name, paramName, (strlen(paramList->name) >= strlen(paramName)) ? strlen(paramList->name) : strlen(paramName))){
			paramList++;
			continue;
		}
...
zcfgRet_t zyTTQuickWanBridgeObjAttrSet(char *tr98FullPathName, int handler, char *paramName, int newNotify, struct json_object *multiAttrJobj)
{
	zcfgRet_t ret = ZCFG_SUCCESS;
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
...
#ifdef TT_HGW
/* InternetGatewayDevice.X_TT.Configuration.SIP.DNS-SRV */
zcfgRet_t zyTTDnsSrvObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *DnsSrvObj = NULL;
	struct json_object *paramValue = NULL;
	tr98Parameter_t *paramList = NULL;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = feObjJsonGet(RDM_OID_DNS_SRV, &objIid, &DnsSrvObj, updateFlag)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	*tr98Jobj = json_object_new_object();
	...
	while(paramList->name != NULL){
		/*Write parameter value from tr181 objects to tr98 object*/
		paramValue = json_object_object_get(DnsSrvObj, paramList->name);
		if(paramValue != NULL){
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
	...
	json_object_put(DnsSrvObj);

	return ZCFG_SUCCESS;
}

zcfgRet_t zyTTDnsSrvObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *DnsSrvObj = NULL;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = zcfgFeObjJsonGet(RDM_OID_DNS_SRV, &objIid, &DnsSrvObj)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	if(multiJobj){
		tmpObj = DnsSrvObj;
		DnsSrvObj = NULL;
		DnsSrvObj = zcfgFeJsonMultiObjAppend(RDM_OID_DNS_SRV, &objIid, multiJobj, tmpObj);
	}
	...
	while(paramList->name != NULL){
		...
		paramValue = json_object_object_get(tr98Jobj, paramList->name);
		if(paramValue != NULL){
			tr181ParamValue = json_object_object_get(DnsSrvObj, paramList->name);
			if(tr181ParamValue != NULL){
				json_object_object_add(DnsSrvObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
			}
		}
	...
	if(multiJobj){
		...
		if((ret = zcfgFeObjJsonSet(RDM_OID_DNS_SRV, &objIid, DnsSrvObj, NULL)) != ZCFG_SUCCESS ){
			json_object_put(DnsSrvObj);
			return ret;
		}
		json_object_put(DnsSrvObj);
...
zcfgRet_t zyTTDnsSrvObjNotify(char *tr98ObjName, char *tr181ParamName, struct json_object *tr181ParamVal, int handler, struct json_object **tr98NotifyInfo)
{
	bool found = false;
	char tr98Notify[256] = "";
	tr98Parameter_t *paramList = NULL;
...
int zyTTDnsSrvObjAttrGet(char *tr98FullPathName, int handler, char *paramName)
{
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
	uint32_t DnsSrvOid = 0;
	...
	IID_INIT(DnsSrvIid);
	snprintf(tr181Obj, sizeof(tr181Obj),"Device.%s", mapObj);
	DnsSrvOid = zcfgFeObjNameToObjId(tr181Obj, &DnsSrvIid);
	...
	while(paramList->name != NULL) {
		...
		attrValue = zcfgFeParamAttrGetByName(DnsSrvOid, paramList->name);
		if(attrValue < 0 ) {
			attrValue = 0;
		}
		break;
...
zcfgRet_t zyTTDnsSrvObjAttrSet(char *tr98FullPathName, int handler, char *paramName, int newNotify, struct json_object *multiAttrJobj)
{
	zcfgRet_t ret = ZCFG_SUCCESS;
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
	...
	while(paramList->name != NULL) {
		...
		/*Write new parameter attribute from tr98 object to tr181 objects*/
		attrValue = zcfgFeNotifyAttrValSet(attrValue, newNotify);
		ret = zcfgFeMultiParamAttrAppend(DnsSrvOid, multiAttrJobj, paramList->name, attrValue);
		if( ret != ZCFG_SUCCESS){
			zcfgLog(ZCFG_LOG_ERR, "%s(): set %d %s attribute fail\n", __FUNCTION__, DnsSrvOid, paramList->name);
		}
		break;
	}
...
zcfgRet_t zyTTPORTMIRRORObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = feObjJsonGet(RDM_OID_TT_PORT_MIRROR, &objIid, &portmirrorObj, updateFlag)) != ZCFG_SUCCESS){
		return ret;
	}
...
zcfgRet_t zyTTPORTMIRRORObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	...
	/*Get related tr181 objects*/
	IID_INIT(portmirrorobjIid);
	if((ret = zcfgFeObjJsonGet(RDM_OID_TT_PORT_MIRROR, &portmirrorobjIid, &portmirrorObj)) != ZCFG_SUCCESS){
		return ret;
	}
		...
		if(multiJobj){
			if(portmirrorObj){
				...
				portmirrorObj = NULL;
				portmirrorObj = zcfgFeJsonMultiObjAppend(RDM_OID_TT_PORT_MIRROR, &portmirrorobjIid, multiJobj, tmpObj);
				json_object_put(tmpObj);
			}
		}
	...
	while(paramList->name != NULL){
		...
		if(paramValue != NULL){
			if(!strcmp(paramList->name, "MonitorInterface")) {
				...
				if(tr181WanIpIface) {
					json_object_object_add(portmirrorObj, paramList->name, json_object_new_string(tr181WanIpIface));
...
#ifdef TT_HGW
            if(!strcmp(paramList->name, "VisibilityLevel")) {
               	IID_INIT(EngdebugobjIid);
				if(zcfgFeObjJsonGet(RDM_OID_ENGDEBUG, &EngdebugobjIid, &EngdebugObj) == ZCFG_SUCCESS){
					json_object_object_add(EngdebugObj, "VisibilityLevel", JSON_OBJ_COPY(paramValue));
					zcfgFeObjJsonSetWithoutApply(RDM_OID_ENGDEBUG, &EngdebugobjIid, EngdebugObj, NULL);
				}
				json_object_object_add(portmirrorObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
			...
			if(!strcmp(paramList->name, "Limitation")) {
				IID_INIT(EngdebugobjIid);
				if(zcfgFeObjJsonGet(RDM_OID_ENGDEBUG, &EngdebugobjIid, &EngdebugObj) == ZCFG_SUCCESS){
					json_object_object_add(EngdebugObj, "Limitation", JSON_OBJ_COPY(paramValue));
					zcfgFeObjJsonSetWithoutApply(RDM_OID_ENGDEBUG, &EngdebugobjIid, EngdebugObj, NULL);
				}
				json_object_object_add(portmirrorObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
			...
			if(!strcmp(paramList->name, "Lock")) {
				IID_INIT(EngdebugobjIid);
				if(zcfgFeObjJsonGet(RDM_OID_ENGDEBUG, &EngdebugobjIid, &EngdebugObj) == ZCFG_SUCCESS){
					json_object_object_add(EngdebugObj, "Lock", JSON_OBJ_COPY(paramValue));
					zcfgFeObjJsonSetWithoutApply(RDM_OID_ENGDEBUG, &EngdebugobjIid, EngdebugObj, NULL);
				}
				json_object_object_add(portmirrorObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
	...
	if(multiJobj){
		...
		if(portmirrorObj){
			if((ret = zcfgFeObjJsonSet(RDM_OID_TT_PORT_MIRROR, &portmirrorobjIid, portmirrorObj, NULL)) != ZCFG_SUCCESS ) {
				zcfgLog(ZCFG_LOG_ERR, "%s : Set RDM_OID_TT_PORT_MIRROR Fail\n", __FUNCTION__);
			}
			else {
				zcfgLog(ZCFG_LOG_DEBUG, "%s : Set RDM_OID_TT_PORT_MIRROR Success\n", __FUNCTION__);
			}
			json_object_put(portmirrorObj);
		}
	}
	...
	while(paramList->name != NULL){
		...
		if(paramValue != NULL){
...
#ifdef TT_HGW
            if(!strcmp(paramList->name, "VisibilityLevel")) {
               	IID_INIT(CaptureobjIid);
				if(zcfgFeObjJsonGet(RDM_OID_CAPTURE, &CaptureobjIid, &CaptureObj) == ZCFG_SUCCESS){
					json_object_object_add(CaptureObj, "VisibilityLevel", JSON_OBJ_COPY(paramValue));
					zcfgFeObjJsonSetWithoutApply(RDM_OID_CAPTURE, &CaptureobjIid, CaptureObj, NULL);
				}
				json_object_object_add(packetcaptureObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
			...
			if(!strcmp(paramList->name, "Limitation")) {
				IID_INIT(CaptureobjIid);
				if(zcfgFeObjJsonGet(RDM_OID_CAPTURE, &CaptureobjIid, &CaptureObj) == ZCFG_SUCCESS){
					json_object_object_add(CaptureObj, "Limitation", JSON_OBJ_COPY(paramValue));
					zcfgFeObjJsonSetWithoutApply(RDM_OID_CAPTURE, &CaptureobjIid, CaptureObj, NULL);
				}
				json_object_object_add(packetcaptureObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
			...
			if(!strcmp(paramList->name, "Lock")) {
				IID_INIT(CaptureobjIid);
				if(zcfgFeObjJsonGet(RDM_OID_CAPTURE, &CaptureobjIid, &CaptureObj) == ZCFG_SUCCESS){
					json_object_object_add(CaptureObj, "Lock", JSON_OBJ_COPY(paramValue));
					zcfgFeObjJsonSetWithoutApply(RDM_OID_CAPTURE, &CaptureobjIid, CaptureObj, NULL);
				}
				json_object_object_add(packetcaptureObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
...
#ifdef TT_HGW
/* InternetGatewayDevice.X_TT.CWMP */
zcfgRet_t zyTTCWMPObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *CWMPObj = NULL;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = feObjJsonGet(RDM_OID_CWMP, &objIid, &CWMPObj, updateFlag)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	*tr98Jobj = json_object_new_object();
	...
	while(paramList->name != NULL){
		/*Write parameter value from tr181 objects to tr98 object*/
		paramValue = json_object_object_get(CWMPObj, paramList->name);
		if(paramValue != NULL){
			json_object_object_add(*tr98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
	...
	json_object_put(CWMPObj);

	return ZCFG_SUCCESS;
}

zcfgRet_t zyTTCWMPObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *CWMPObj = NULL;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = zcfgFeObjJsonGet(RDM_OID_CWMP, &objIid, &CWMPObj)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	if(multiJobj){
		tmpObj = CWMPObj;
		CWMPObj = NULL;
		CWMPObj = zcfgFeJsonMultiObjAppend(RDM_OID_CWMP, &objIid, multiJobj, tmpObj);
	}
	...
	while(paramList->name != NULL){
		...
		if(paramValue != NULL){
			if(!strcmp(paramList->name, "VisibilityLevel")) {
               	IID_INIT(tr069objIid);
				if(zcfgFeObjJsonGet(RDM_OID_TR069, &tr069objIid, &tr069Obj) == ZCFG_SUCCESS){
					json_object_object_add(tr069Obj, "VisibilityLevel", JSON_OBJ_COPY(paramValue));
					zcfgFeObjJsonSetWithoutApply(RDM_OID_TR069, &tr069objIid, tr069Obj, NULL);
				}
				json_object_object_add(CWMPObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
			}
			if(!strcmp(paramList->name, "Limitation")) {
               	IID_INIT(tr069objIid);
				if(zcfgFeObjJsonGet(RDM_OID_TR069, &tr069objIid, &tr069Obj) == ZCFG_SUCCESS){
					json_object_object_add(tr069Obj, "Limitation", JSON_OBJ_COPY(paramValue));
					zcfgFeObjJsonSetWithoutApply(RDM_OID_TR069, &tr069objIid, tr069Obj, NULL);
				}
				json_object_object_add(CWMPObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
			...
			if(!strcmp(paramList->name, "Lock")) {
               	IID_INIT(tr069objIid);
				if(zcfgFeObjJsonGet(RDM_OID_TR069, &tr069objIid, &tr069Obj) == ZCFG_SUCCESS){
					json_object_object_add(tr069Obj, "Lock", JSON_OBJ_COPY(paramValue));
					zcfgFeObjJsonSetWithoutApply(RDM_OID_TR069, &tr069objIid, tr069Obj, NULL);
				}
				json_object_object_add(CWMPObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
			...
			tr181ParamValue = json_object_object_get(CWMPObj, paramList->name);
			if(tr181ParamValue != NULL){
				json_object_object_add(CWMPObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
	...
	if(multiJobj){
		...
		if((ret = zcfgFeObjJsonSet(RDM_OID_CWMP, &objIid, CWMPObj, NULL)) != ZCFG_SUCCESS ){
			json_object_put(CWMPObj);
			return ret;
		}
		json_object_put(CWMPObj);
...
zcfgRet_t zyTTCWMPObjNotify(char *tr98ObjName, char *tr181ParamName, struct json_object *tr181ParamVal, int handler, struct json_object **tr98NotifyInfo)
{
	bool found = false;
	char tr98Notify[256] = "";
	tr98Parameter_t *paramList = NULL;
...
int zyTTCWMPObjAttrGet(char *tr98FullPathName, int handler, char *paramName)
{
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
	uint32_t CWMPOid = 0;
	...
	IID_INIT(CWMPIid);
	snprintf(tr181Obj, sizeof(tr181Obj),"Device.%s", mapObj);
	CWMPOid = zcfgFeObjNameToObjId(tr181Obj, &CWMPIid);
	...
	while(paramList->name != NULL) {
		...
		attrValue = zcfgFeParamAttrGetByName(CWMPOid, paramList->name);
		if(attrValue < 0 ) {
			attrValue = 0;
		}
		break;
...
zcfgRet_t zyTTCWMPObjAttrSet(char *tr98FullPathName, int handler, char *paramName, int newNotify, struct json_object *multiAttrJobj)
{
	zcfgRet_t ret = ZCFG_SUCCESS;
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
	...
	while(paramList->name != NULL) {
		...
		/*Write new parameter attribute from tr98 object to tr181 objects*/
		attrValue = zcfgFeNotifyAttrValSet(attrValue, newNotify);
		ret = zcfgFeMultiParamAttrAppend(CWMPOid, multiAttrJobj, paramList->name, attrValue);
		if( ret != ZCFG_SUCCESS){
			zcfgLog(ZCFG_LOG_ERR, "%s(): set %d %s attribute fail\n", __FUNCTION__, CWMPOid, paramList->name);
		}
		break;
	}
		...
		if(paramValue != NULL){
			...
			if(!strcmp(paramList->name, "Enable")) {
				...
				if(AccountIndex == 1 || AccountIndex == 2){
					...
					if(zcfgFeObjJsonGet(RDM_OID_ZY_LOG_CFG_GP_ACCOUNT, &tempIid, &tempobj) == ZCFG_SUCCESS){
						setvalue = json_object_get_string(paramValue);
						tr181value = json_object_get_string(json_object_object_get(tempobj, "Password"));
						if(strcmp(setvalue,tr181value)){
						if(!checkTTPassword(setvalue,tr181value)){
							zcfgFeJsonObjFree(tempobj);
							ret = ZCFG_INVALID_PARAM_VALUE;
							strncpy(paramfault,"Password", sizeof(paramfault));
							return ret;
						}	
						}
						if(!strcmp(setvalue, "")){
...
zcfgRet_t zyTTWanConnDiagObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = feObjJsonGet(RDM_OID_TTNET_CONN_DIAG, &objIid, &wanConnDiagObj, updateFlag)) != ZCFG_SUCCESS){
		return ret;
	}
...
zcfgRet_t zyTTWanConnDiagObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = zcfgFeObjJsonGet(RDM_OID_TTNET_CONN_DIAG, &objIid, &wanConnDiagObj)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	if(multiJobj){
		...
		wanConnDiagObj = NULL;
		wanConnDiagObj = zcfgFeJsonMultiObjAppend(RDM_OID_TTNET_CONN_DIAG, &objIid, multiJobj, tmpObj);
	}
	...
	if(multiJobj){
		...
		if((ret = zcfgFeObjJsonSet(RDM_OID_TTNET_CONN_DIAG, &objIid, wanConnDiagObj, NULL)) != ZCFG_SUCCESS ){
			json_object_put(wanConnDiagObj);
			return ret;
		}
		json_object_put(wanConnDiagObj);
...
/* InternetGatewayDevice.X_ZYXEL_FSecure */
zcfgRet_t zyExtFsecureObjGet(char *tr98FullPathName, int handler, struct json_object **ret98Jobj, bool updateFlag)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = feObjJsonGet(RDM_OID_F_SECURE, &objIid, &fsecureObj, updateFlag)) != ZCFG_SUCCESS){
		return ret;
	}

	/*fill up tr98 object from related tr181 objects*/
	*ret98Jobj = json_object_new_object();
	paramList = tr98Obj[handler].parameter;
	while(paramList->name != NULL){
		/*Write parameter value from tr181 objects to tr98 object*/

		paramValue = json_object_object_get(fsecureObj, paramList->name);
		if(paramValue != NULL){
			json_object_object_add(*ret98Jobj, paramList->name, JSON_OBJ_COPY(paramValue));
			paramList++;
			continue;
		}
	...
	json_object_put(fsecureObj);

	return ZCFG_SUCCESS;
}

zcfgRet_t zyExtFsecureObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *fsecureObj = NULL;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = zcfgFeObjJsonGet(RDM_OID_F_SECURE, &objIid, &fsecureObj)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	if(multiJobj){
		tmpObj = fsecureObj;
		fsecureObj = NULL;
		fsecureObj = zcfgFeJsonMultiObjAppend(RDM_OID_F_SECURE, &objIid, multiJobj, tmpObj);
	}
	...
	while(paramList->name != NULL){
		...
		if(paramValue != NULL){
			tr181ParamValue = json_object_object_get(fsecureObj, paramList->name);
			if(tr181ParamValue != NULL){
				json_object_object_add(fsecureObj, paramList->name, JSON_OBJ_COPY(paramValue));
				paramList++;
				continue;
			}
		}
	...
	if(multiJobj){
		...
		if((ret = zcfgFeObjJsonSet(RDM_OID_F_SECURE, &objIid, fsecureObj, NULL)) != ZCFG_SUCCESS ){
			json_object_put(fsecureObj);
			return ret;
		}
		json_object_put(fsecureObj);
...
#endif // CONFIG_ZCFG_BE_MODULE_ZYXEL_FSECURE
...
zcfgRet_t usrIntfObjGet(char *tr98FullPathName, int handler, struct json_object **tr98Jobj, bool updateFlag)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *UserInterfaceObj = NULL;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = feObjJsonGet(RDM_OID_USR_INTF, &objIid, &UserInterfaceObj, updateFlag)) != ZCFG_SUCCESS){
		return ret;
	}
...
zcfgRet_t usrIntfObjSet(char *tr98FullPathName, int handler, struct json_object *tr98Jobj, struct json_object *multiJobj, char *paramfault)
{
	zcfgRet_t ret;
	objIndex_t objIid;
	struct json_object *UserInterfaceObj = NULL;
	...
	/*Get related tr181 objects*/
	IID_INIT(objIid);
	if((ret = zcfgFeObjJsonGet(RDM_OID_USR_INTF, &objIid, &UserInterfaceObj)) != ZCFG_SUCCESS){
		return ret;
	}
	...
	if(multiJobj){
		tmpObj = UserInterfaceObj;
		UserInterfaceObj = NULL;
		UserInterfaceObj = zcfgFeJsonMultiObjAppend(RDM_OID_USR_INTF, &objIid, multiJobj, tmpObj);
	}
	...
	if(multiJobj){
		...
		if((ret = zcfgFeObjJsonSet(RDM_OID_USR_INTF, &objIid, UserInterfaceObj, NULL)) != ZCFG_SUCCESS ){
			json_object_put(UserInterfaceObj);
			return ret;
		}
		json_object_put(UserInterfaceObj);
...
zcfgRet_t usrIntfObjNotify(char *tr98ObjName, char *tr181ParamName, struct json_object *tr181ParamVal, int handler, struct json_object **tr98NotifyInfo)
{
	bool found = false;
	char tr98Notify[256] = "";
	tr98Parameter_t *paramList = NULL;
...
int usrIntfObjAttrGet(char *tr98FullPathName, int handler, char *paramName)
{
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
	uint32_t UserInterfaceOid = 0;
...
zcfgRet_t usrIntfObjAttrSet(char *tr98FullPathName, int handler, char *paramName, int newNotify, struct json_object *multiAttrJobj)
{
	zcfgRet_t ret = ZCFG_SUCCESS;
	char tr181Obj[128] = {0};
	char mapObj[128] = {0};
