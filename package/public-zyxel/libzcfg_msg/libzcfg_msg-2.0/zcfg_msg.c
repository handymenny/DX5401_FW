...
)
{
    ...
    if(socketPathExtId != 0)
        snprintf(server_addr.sun_path, sizeof(server_addr.sun_path)-1, "%s%u-%u%u", ZCFG_ADDRESS_PREFIX, eid, seqid, socketPathExtId);
    else
...
int zcfg_msg_socketPathExtIdSet(int id) {
    socketPathExtId = id;
    return 0;
}
