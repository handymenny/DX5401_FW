...
/*
 * CheckRedirect():
 * support _zcfg_system redirect (>).
 * @param [in] origCmd: the string before >
 * @param [in] rediCmd: the string after >
 * @param [in/out] background: char &
 * @param [in] cmd: full command
 *
 * Support format:
 *  >[file path]
 * 1>[file path]
 ...
 */
int checkRedirect(char *origCmd,char *rediCmd, int *background, const char *cmd){
	char *p = NULL, *q = NULL;
	int i = 0;
	int stdNum = -1;

	strncpy(origCmd, cmd, ZOS_CMD_MAX_LEN+1);
	while(i<ZOS_CMD_MAX_LEN && cmd[i]!='\0'){
		if(origCmd[i] == '>'){
			p = strchr(&origCmd[i], '>');
			if(p != NULL){
				i = p - origCmd;
				if((origCmd[i-1] != ' ') && (origCmd[i-1] != '&') && (origCmd[i-1] != '0') && (origCmd[i-1] != '1') && (origCmd[i-1] != '2')){
					stdNum = -1; //error, don't need to redirect
					break;
				}
				...
				strncpy(rediCmd, p, ZOS_CMD_MAX_LEN);
 ...
 * runDup2():
 * @param [in] rediCmd: the string after >
 * @param [in] stdNum: file descriptor number
 */
int runDup2(char *rediCmd, int stdNum){
	int fd = -1;
	if(stdNum != -1){
		fd = open(rediCmd, O_CREAT | O_RDWR);
		if (fd == -1) {
			ZLOG_ERROR("fail to open() %s", rediCmd);
			return fd;
		}
...
static int _zcfg_system(const char *cmd)
{
	...
	int stdNum = -1;
	int fd = -1;
	char origCmd[ZOS_CMD_MAX_LEN+1] = {0};
	char rediCmd[ZOS_CMD_MAX_LEN+1] = {0};

	stdNum = checkRedirect(origCmd, rediCmd, &background, cmd);
	strncpy(buf, origCmd, ZOS_CMD_MAX_LEN);
	while(i<ZOS_CMD_MAX_LEN && origCmd[i]!='\0')
	{	
		if(buf[i] != ' '){
			if(buf[i] == '\"'){
				argv[j++] = &buf[++i];
