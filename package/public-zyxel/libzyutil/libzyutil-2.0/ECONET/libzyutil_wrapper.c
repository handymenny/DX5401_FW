...
zcfgRet_t zyUtilIGetFirmwareVersion(char *fwversion)
{
	...
	if(strcmp((char *)trx_header_buff.swVersionInt, (char *)trx_header_buff.swVersionExt)){ 						
		snprintf(version, sizeof(version),"%s_%s",(char *)trx_header_buff.swVersionInt,(char *)trx_header_buff.swVersionExt);				
		ZOS_STRNCPY(fwversion, version, 63);
	}
	else{
