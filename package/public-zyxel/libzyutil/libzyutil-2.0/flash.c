    ...
    for (erase.start = 0; ((erase.start < mtd.size) && (buf_size_to_write > 0)); erase.start += mtd.erasesize)
    {
        ...
        if (!meet_bad)
        {
            ...
            if (result != unit_to_use)
            {
                ...
                cur_ofs = lseek(devFd, (Z_OFF_T)(-result), SEEK_CUR);
...
#ifdef ZYXEL_QUICK_STARTUP
#define SMTMODE_FILE "/data/SMTMode_Startup"
/*
 *   return
 * 		0 for failed , 1 for SMT Image
 */
int zyUtilChkSMTMode(void){
	int ret = 0;
	FILE *fptr = NULL;
	
	unsigned char smt_value[4] = {0};
	if((fptr = fopen(SMTMODE_FILE, "r")) == NULL) {
		printf("Can't fopen \"%s\"\n", SMTMODE_FILE);
		return 0;
	}
	fread(smt_value, 1, sizeof(smt_value) ,fptr);
	fclose(fptr);	
	ret = atoi(smt_value);
	return ret;
}
